...
export function generateErrorMessage(
  ...
  switch (status) {
    ...
    case AlignmentStatus.NullabilityMismatch:
      ...
      return `Nullability mismatch for field '${fieldPath}': GraphQL field is ${
        graphqlField.nullable ? 'nullable' : 'non-null'
      }, database column is ${dbColumn.nullable ? 'nullable' : 'non-null'}, API field is ${
        apiField.nullable ? 'nullable' : 'non-null'
      }.`;
 ...
 */
function generateDatabaseMigrationSuggestion(
  fieldPath: string,
  graphqlField: FieldReference
): string {
 ...
 */
function generateApiResolverSuggestion(
  fieldPath: string,
  dbColumn: DatabaseColumn | undefined
): string {
  const parts = fieldPath.split('.');
 ...
 */
function generateNullabilitySuggestion(
  graphqlField: FieldReference,
  dbColumn: DatabaseColumn | undefined,
  apiField: ApiField | undefined
): string {
 ...
 */
function mapGraphQLTypeToPg(graphqlType: string): string {
 ...
 */
function mapPgTypeToRust(pgType: string): string {
 ...
 */
export function isComputedField(fieldPath: string, computedFields: string[]): boolean {
 ...
 */
export function getRequiredAction(status: AlignmentStatus): string {
  switch (status) {
    case AlignmentStatus.Aligned:

...
export async function loadConfig(configPath) {
    ...
    try {
        ...
        if (rawConfig.sources?.directory && rawConfig.sources?.extensions) {
            graphqlPaths = rawConfig.sources.extensions.map((ext) => {
        ...
        if (rawConfig.validation?.computedFields && Array.isArray(rawConfig.validation.computedFields)) {
            computedFields = rawConfig.validation.computedFields.map((fieldPath) => ({
                fieldPath,
                sourceColumns: [],
...
export async function saveConfig(config, configPath) {
    ...
    try {
        ...
        throw new Error(`Failed to save config to ${path}: ${error instanceof Error ? error.message : String(error)}`);
    }
}

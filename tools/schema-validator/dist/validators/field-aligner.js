 ...
 */
export function alignField(graphqlField, dbColumn, apiField, fieldPath, sourceFile, sourceLine, sourceColumn, typeComparator) {
...
export function computeAlignmentStatus(graphqlField, dbColumn, apiField, typeComparator) {
    ...
    const typeComparison = typeComparator.compareTypes(graphqlField.graphqlType, dbColumn.pgType, apiField.graphqlType);
 ...
 */
export function generateSuggestion(status, fieldPath, graphqlField, dbColumn, apiField, typeComparator) {
    switch (status) {
        case AlignmentStatus.Aligned:
            return undefined;
        case AlignmentStatus.MissingDb:
...
function generateNullabilitySuggestion(graphqlField, dbColumn, apiField) {
    ...
    if (dbColumn && !graphqlField.nullable && dbColumn.nullable) {
        suggestions.push(`Database column allows NULL but GraphQL requires non-null. Either:\n1. Add NOT NULL constraint to database column\n2. Make GraphQL field nullable`);
    }
    if (apiField && graphqlField.nullable !== apiField.nullable) {
        suggestions.push(`GraphQL and API nullability differ. Update API field to match GraphQL expectation.`);
    }

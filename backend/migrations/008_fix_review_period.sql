-- Migration 008: Fix review_period generation in create_review_with_goals
-- Feature: 023-reviews-creation-it
-- Date: 2025-10-06
-- Description: Auto-generate review_period string from dates or review type

DROP FUNCTION IF EXISTS hr_public.create_review_with_goals(UUID, UUID, hr_public.review_type, DATE, DATE, UUID[], JSONB, TEXT);
...
BEGIN
  ...
  -- Generate review_period string
  IF p_review_period_start IS NOT NULL THEN
    -- Use year from start date
    v_review_period := EXTRACT(YEAR FROM p_review_period_start)::TEXT || ' - ' ||
                       REPLACE(INITCAP(REPLACE(p_review_type::TEXT, '_', ' ')), 'Review', 'Review');
  ELSE
    -- Use current year and review type
    v_review_period := EXTRACT(YEAR FROM CURRENT_DATE)::TEXT || ' - ' ||
                       REPLACE(INITCAP(REPLACE(p_review_type::TEXT, '_', ' ')), 'Review', 'Review');
  END IF;
...
COMMENT ON FUNCTION hr_public.create_review_with_goals IS 'Create performance review with associated goals. Auto-generates review_period from dates/type.';

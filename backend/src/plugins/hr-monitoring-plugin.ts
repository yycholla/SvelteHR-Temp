...
export class HRMetricsCollector {
  ...
  recordRequest(type: 'query' | 'mutation' | 'subscription', responseTime: number, hasErrors: boolean = false): void {
    this.metrics.requests.total++;
    if (type === 'query') {
      this.metrics.requests.queries++;
    } else if (type === 'mutation') {
      this.metrics.requests.mutations++;
    }
    // Note: subscriptions are not tracked separately in our metrics schema
  ...
  recordHROperation(operation: keyof HRMetrics['hr'], count: number = 1): void {
    this.metrics.hr[operation] += count;
    this.storeMetricsInRedis();
  }

  // Record errors by type
  recordError(errorType: keyof HRMetrics['errors'], count: number = 1): void {
...
export class UserRateLimiter {
  ...
  async checkMultipleLimits(
    userId: string,
    checks: Array<{ operation: string; limit: number; windowMs: number }>
  ): Promise<{ allowed: boolean; failedChecks: string[]; resetTime: number }> {
    const results = await Promise.all(
      checks.map(check =>
        this.checkUserLimit(userId, check.operation, check.limit, check.windowMs)
...
export class GraphQLOperationAnalyzer {
  ...
  private static calculateComplexity(query: string, operations: string[]): number {
    let complexity = 1;
...
export function createHRRateLimitingMiddleware(
  redis: Redis,
  metricsCollector: HRMetricsCollector
) {
...
export function getHealthCheckData(metricsCollector: HRMetricsCollector, redis: Redis, pgPool: any) {
  return async () => {

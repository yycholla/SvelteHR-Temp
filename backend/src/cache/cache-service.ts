...
export class CacheService {
	 ...
	 */
	async set<T>(key: CacheKey | string, value: T, identifier?: string, ttl?: number): Promise<void> {
		try {
			const cacheKey = this.generateCacheKey(key, identifier);
...
class QueryParser {
	 ...
	 */
	getQueryHash(query: string, variables: Record<string, any>): string {
	 ...
	 */
	getCachePolicy(query: string): { cacheable: boolean; ttl: number } {
	 ...
	 */
	private isSubscription(query: string): boolean {
	 ...
	 */
	private hasSensitiveFields(query: string): boolean {
		const sensitivePatterns = [/password/i, /secret/i, /token/i, /auth/i, /sensitive/i];

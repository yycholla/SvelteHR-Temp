...
export class JWTAuthMiddleware {
   ...
   */
  verifyRefreshToken(token: string): { user_id: number, type: string } | null {
    try {
      return jwt.verify(token, this.jwtRefreshSecret) as { user_id: number, type: string };
    } catch (error) {
  ...
  private async isTokenBlacklisted(token: string): Promise<boolean> {
    try {
      ...
      this.logger.warn('Failed to check token blacklist', { error: error.message });
      return false;
    }
  }
 ...
 */
export function createJWTAuthMiddleware(redis: Redis, logger: winston.Logger): any {
...
export class SessionManager {
   ...
   */
  async trackActiveSession(userId: number, sessionId: string, deviceInfo?: any): Promise<void> {
    ...
    await this.redis.setex(sessionKey, JWT_REFRESH_EXPIRY, JSON.stringify(sessionData));
  ...
  async updateSessionActivity(userId: number, sessionId: string): Promise<void> {
    ...
    if (sessionData) {
      ...
      await this.redis.setex(sessionKey, JWT_REFRESH_EXPIRY, JSON.stringify(session));
    }
  }

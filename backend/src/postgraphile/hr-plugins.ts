            ...
            switch (field.fieldName) {
              ...
              case 'statusIndicator':
                return {
                  ...
                  type: build.graphql.GraphQLString,
                  description:
                    'Employee status indicator (Active, Inactive, On Leave, Terminated)',
                  resolve: async (
                    employee: any,
                    args: any,
                    context: Context,
                    ...
                    `;
                      ...
                      return {
                        ...
                        vacationDaysAvailable:
                          Math.max(0, row.vacation_days - row.vacation_used) ||
                          20,
                        sickDaysUsed: row.sick_used || 0,
                        sickDaysAvailable:
                          Math.max(0, row.sick_days - row.sick_used) || 10,
                        emergencyLeaveUsed: row.emergency_used || 0,
                        emergencyLeaveAvailable:
                          Math.max(
                            0,
                            row.emergency_days - row.emergency_used
                          ) || 5,
...
export const hrComputedFieldsPlugin = (redis?: Redis) =>
  new HRComputedFieldsPlugin(redis).createPlugin();
export const hrBusinessLogicPlugin = () =>
  new HRBusinessLogicPlugin().createPlugin();
export const hrPerformanceOptimizationPlugin = (redis?: Redis) =>
  new HRPerformanceOptimizationPlugin(redis).createPlugin();

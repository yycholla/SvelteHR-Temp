...
app.use(cors({
  ...
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Requested-With', 'Accept', 'Origin', 'X-Client-Name', 'X-Client-Version'],
  ...
  build.hook('postgraphile:http:handler', (req, { pgSettings }) => {
    ...
    if (token) {
      try {
        ...
        pgSettings['jwt.claims.employee_id'] = decoded.employee_id?.toString() || '';
        pgSettings['jwt.claims.role'] = decoded.role || 'hr_guest';
        pgSettings['jwt.claims.role_level'] = decoded.role_level?.toString() || '0';
        pgSettings['jwt.claims.department_id'] = decoded.department_id?.toString() || '';

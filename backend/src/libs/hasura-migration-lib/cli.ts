...
import { HasuraMigrationLib, MigrationConfig } from './index';
...
let migrationLib: HasuraMigrationLib;
...
async function migrateCommand(options: { dry?: boolean; target?: string }) {
  try {
    const lib = await initializeMigrationLib();
...
async function rollbackCommand(migrationId?: string) {
  try {
    const lib = await initializeMigrationLib();
...
async function generateCommand(name: string, options: { 
  up?: string; 
  down?: string;
  template?: string;
}) {

...
export class InputValidator {
    ...
    static validateRequired(value, fieldName) {
        if (value === null || value === undefined || value === '') {
            throw new HRValidationError(`${fieldName} is required`, ValidationErrorCodes.REQUIRED_FIELD, fieldName);
        }
    }
    static validateString(value, fieldName, minLength, maxLength) {
        this.validateRequired(value, fieldName);
        if (typeof value !== 'string') {
            throw new HRValidationError(`${fieldName} must be a string`, ValidationErrorCodes.INVALID_TYPE, fieldName);
        }
        if (minLength && value.length < minLength) {
            throw new HRValidationError(`${fieldName} must be at least ${minLength} characters long`, ValidationErrorCodes.VALUE_TOO_SHORT, fieldName, { minLength, actualLength: value.length });
        }
        if (maxLength && value.length > maxLength) {
            throw new HRValidationError(`${fieldName} cannot exceed ${maxLength} characters`, ValidationErrorCodes.VALUE_TOO_LONG, fieldName, { maxLength, actualLength: value.length });
        }
    }
    static validateNumber(value, fieldName, min, max) {
        ...
        if (isNaN(numValue)) {
            throw new HRValidationError(`${fieldName} must be a valid number`, ValidationErrorCodes.INVALID_TYPE, fieldName);
        }
        if (min !== undefined && numValue < min) {
            throw new HRValidationError(`${fieldName} must be at least ${min}`, ValidationErrorCodes.VALUE_OUT_OF_RANGE, fieldName, { min, max, actual: numValue });
        }
        if (max !== undefined && numValue > max) {
            throw new HRValidationError(`${fieldName} cannot exceed ${max}`, ValidationErrorCodes.VALUE_OUT_OF_RANGE, fieldName, { min, max, actual: numValue });
        }
    }
    static validateDate(value, fieldName) {
        ...
        if (isNaN(date.getTime())) {
            throw new HRValidationError(`${fieldName} must be a valid date`, ValidationErrorCodes.INVALID_FORMAT, fieldName);
        }
        return date;
    }
    static validateDateRange(startDate, endDate, startFieldName, endFieldName) {
        const start = this.validateDate(startDate, startFieldName);
        const end = this.validateDate(endDate, endFieldName);
        if (start >= end) {
            throw new HRValidationError(`${endFieldName} must be after ${startFieldName}`, ValidationErrorCodes.INVALID_FORMAT, endFieldName, { startDate: start, endDate: end });
        }
    }
    static validateEmail(value, fieldName) {
        this.validateString(value, fieldName);
        if (!ValidationPatterns.email.test(value)) {
            throw new HRValidationError(`${fieldName} must be a valid email address`, ValidationErrorCodes.INVALID_FORMAT, fieldName);
        }
    }
    static validateUUID(value, fieldName) {
        this.validateString(value, fieldName);
        if (!ValidationPatterns.uuid.test(value)) {
            throw new HRValidationError(`${fieldName} must be a valid UUID`, ValidationErrorCodes.INVALID_FORMAT, fieldName);
        }
    }
    static validatePattern(value, fieldName, pattern, errorMessage) {
        this.validateString(value, fieldName);
        if (!pattern.test(value)) {
            throw new HRValidationError(errorMessage || `${fieldName} has invalid format`, ValidationErrorCodes.INVALID_FORMAT, fieldName);
        }
    }
    // HR-specific validators
    static validateEmployeeId(value, fieldName = 'Employee ID') {
        this.validatePattern(value, fieldName, ValidationPatterns.employeeId, `${fieldName} must be in format EMP-XXXX (e.g., EMP-1001)`);
    }
    static validateProjectCode(value, fieldName = 'Project Code') {
        this.validatePattern(value, fieldName, ValidationPatterns.projectCode, `${fieldName} must be in format ABC-123 (e.g., HR-001)`);
    }
    static validatePassword(value, fieldName = 'Password') {
        this.validateString(value, fieldName, 8);
        if (!ValidationPatterns.passwordStrong.test(value)) {
            throw new HRValidationError(`${fieldName} must contain at least 8 characters including uppercase, lowercase, number, and special character`, ValidationErrorCodes.WEAK_PASSWORD, fieldName);
        }
    }
    static validateLeaveType(value, fieldName = 'Leave Type') {
        this.validateRequired(value, fieldName);
        ...
        if (!validLeaveTypes.includes(value)) {
            throw new HRValidationError(`${fieldName} must be one of: ${validLeaveTypes.join(', ')}`, ValidationErrorCodes.INVALID_FORMAT, fieldName, { validValues: validLeaveTypes });
        }
    }
    static validatePriority(value, fieldName = 'Priority') {
        this.validateRequired(value, fieldName);
        const validPriorities = ['LOW', 'NORMAL', 'HIGH', 'URGENT'];
        if (!validPriorities.includes(value)) {
            throw new HRValidationError(`${fieldName} must be one of: ${validPriorities.join(', ')}`, ValidationErrorCodes.INVALID_FORMAT, fieldName, { validValues: validPriorities });
        }
    }
    static validateStatus(value, fieldName, validStatuses) {
        this.validateRequired(value, fieldName);
        if (!validStatuses.includes(value)) {
            throw new HRValidationError(`${fieldName} must be one of: ${validStatuses.join(', ')}`, ValidationErrorCodes.INVALID_FORMAT, fieldName, { validValues: validStatuses });
        }
    }
    // Business rule validators
    static validateWorkingHours(hours, fieldName = 'Hours') {
        this.validateNumber(hours, fieldName, 0, 24);
        // Business rule: No more than 16 hours per day for safety
        if (hours > 16) {
            throw new HRValidationError('Cannot log more than 16 working hours per day for safety compliance', ValidationErrorCodes.BUSINESS_RULE_VIOLATION, fieldName, { maxHours: 16 });
        }
    }
    static validateLeaveDays(days, fieldName = 'Leave Days') {
        this.validateNumber(days, fieldName, 0.5, 365);
        // Business rule: Leave requests cannot exceed 90 days (except special cases)
        if (days > 90) {
            throw new HRValidationError('Leave requests cannot exceed 90 days. Please contact HR for extended leave arrangements', ValidationErrorCodes.BUSINESS_RULE_VIOLATION, fieldName, { maxDays: 90 });
        }
    }
    static validateGoalProgress(progress, fieldName = 'Progress') {
        this.validateNumber(progress, fieldName, 0, 100);
    }
    static validateRoleLevel(level, fieldName = 'Role Level') {
        this.validateNumber(level, fieldName, 0, 100);
        const validLevels = [20, 40, 60, 80, 100]; // Employee, Lead, Manager, HR, Admin
        if (!validLevels.includes(level)) {
            throw new HRValidationError(`${fieldName} must be one of: ${validLevels.join(', ')} (Employee: 20, Lead: 40, Manager: 60, HR: 80, Admin: 100)`, ValidationErrorCodes.INVALID_FORMAT, fieldName, { validValues: validLevels });
        }
    }
    // Future date validation for business rules
    static validateFutureDate(date, maxDaysInFuture, fieldName) {
        const now = new Date();
        const maxDate = new Date();
        maxDate.setDate(maxDate.getDate() + maxDaysInFuture);
        if (date > maxDate) {
            throw new HRValidationError(`${fieldName} cannot be more than ${maxDaysInFuture} days in the future`, ValidationErrorCodes.TIME_FUTURE_LIMIT, fieldName, { maxDaysInFuture, requestedDate: date });
        }
    }
    // Past date validation for business rules
    static validatePastDate(date, maxDaysInPast, fieldName) {
        const now = new Date();
        const minDate = new Date();
        minDate.setDate(minDate.getDate() - maxDaysInPast);
        if (date < minDate) {
            throw new HRValidationError(`${fieldName} cannot be more than ${maxDaysInPast} days in the past`, ValidationErrorCodes.TIME_PAST_LIMIT, fieldName, { maxDaysInPast, requestedDate: date });
        }
    }
}
...
export const ValidationSchemas = {
    ...
    user: {
        validateCreate: (args) => {
            ...
            if (args.phone) {
                InputValidator.validatePattern(args.phone, 'Phone', ValidationPatterns.phone, 'Phone number must be in valid format');
            }
        },
        validateUpdateRole: (args) => {
...
//# sourceMappingURL=hr-validation-plugin.js.map
...
import { ValidationSchemas, handleValidationError, ValidationErrorCodes, HRValidationError } from './hr-validation-plugin';
    ...
    `,
        resolvers: {
            Query: {
                ...
                myTimeEntries: async (_parent, args, context, _resolveInfo) => {
          ...
          `;
                ...
                timeEntrySummary: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const employeeId = args.employeeId || context.jwtClaims?.user_id;
                    const { rows } = await pgClient.query('SELECT * FROM hr_public.get_time_entry_stats($1, $2, $3)', [employeeId, args.startDate, args.endDate]);
                    return rows[0];
                },
                // Leave management resolvers
                myLeaveRequests: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
                    }
                    const { rows } = await pgClient.query('SELECT * FROM hr_public.get_my_leave_requests($1, $2, $3)', [userId, args.filter?.status, args.filter?.leaveType]);
                    return rows;
                },
                teamLeaveRequests: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    ...
                    const { rows } = await pgClient.query('SELECT * FROM hr_public.get_team_leave_requests($1, $2)', [managerId, args.status]);
                    return rows;
                },
                leaveBalanceQuery: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const { rows } = await pgClient.query('SELECT hr_public.get_leave_balance($1, $2, $3) as balance', [args.employeeId, args.leaveType, args.year]);
                    return rows[0]?.balance || 0;
                },
                // Goal management resolvers
                myGoals: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
                    }
                    const { rows } = await pgClient.query('SELECT * FROM hr_public.get_employee_goals($1, $2, $3)', [userId, args.filter?.status, args.filter?.includeCompleted]);
                    return rows;
                },
                // Notification resolvers
                myNotifications: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
                    }
                    const { rows } = await pgClient.query('SELECT * FROM hr_public.get_user_notifications($1, $2, $3, $4, $5)', [userId, args.status, args.notificationType, args.limit || 50, args.offset || 0]);
                    return rows;
                },
                notificationStats: async (_parent, _args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
                    }
                    const { rows } = await pgClient.query('SELECT * FROM hr_public.get_notification_stats($1)', [userId]);
                    return rows[0];
                },
                // Dashboard resolvers
                employeeDashboardData: async (_parent, _args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
            ...
            Mutation: {
                // Authentication mutations
                logout: async (_parent, _args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (userId) {
                        // Log logout action
                        await pgClient.query('INSERT INTO hr_public.security_audit_log (user_id, event_type, description, ip_address, success) VALUES ($1, $2, $3, $4, $5)', [userId, 'LOGOUT', 'User logged out', context.req?.ip || null, true]);
                    }
                    return {
                        success: true,
                ...
                submitTimeEntry: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
                    ...
                    const timeEntryQuery = await pgClient.query(`SELECT * FROM hr_public.time_entries WHERE id = $1 AND employee_id = $2`, [args.timeEntryId, userId]);
                    ...
                    const overlapQuery = await pgClient.query(`SELECT id FROM hr_public.time_entries
             WHERE employee_id = $1
             AND entry_date = $2
             AND id != $3
             ...
             AND (
               ...
               (start_time >= $4 AND end_time <= $5)
                    ...
                    const { rows } = await pgClient.query(`UPDATE hr_public.time_entries
             SET status = 'SUBMITTED', submitted_at = NOW(), updated_at = NOW()
             WHERE id = $1 AND employee_id = $2
             RETURNING *`, [args.timeEntryId, userId]);
                    // Log the submission for audit purposes
                    await pgClient.query('INSERT INTO hr_public.security_audit_log (user_id, event_type, description, success) VALUES ($1, $2, $3, $4)', [userId, 'TIME_ENTRY_SUBMIT', `Submitted time entry ${args.timeEntryId} for ${timeEntry.total_hours} hours`, true]);
                    return rows[0];
                },
                approveTimeEntry: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    // Verify manager role
                    if (context.jwtClaims?.role_level < 60) {
                        throw new Error('Manager access required');
             ...
             WHERE id = $1
             RETURNING *`, [args.timeEntryId, context.jwtClaims?.user_id]);
                    if (rows.length === 0) {
                        throw new Error('Time entry not found');
                    }
                    return rows[0];
                },
                rejectTimeEntry: async (_parent, args, context, _resolveInfo) => {
                    try {
                        // Input validation
                        ...
                        if (!managerId) {
                            throw new HRValidationError('Authentication required', ValidationErrorCodes.INSUFFICIENT_PERMISSION);
                        }
                        if (context.jwtClaims?.role_level < 60) {
                            throw new HRValidationError('Manager access required to reject time entries', ValidationErrorCodes.INSUFFICIENT_PERMISSION, undefined, { requiredRoleLevel: 60, actualRoleLevel: context.jwtClaims?.role_level });
                        }
                    }
                    catch (error) {
                    ...
                    const timeEntryQuery = await pgClient.query(`SELECT te.*, u.display_name as employee_name, p.name as project_name
             ...
             WHERE te.id = $1`, [args.timeEntryId]);
                    if (timeEntryQuery.rows.length === 0) {
                        throw new Error('Time entry not found');
                    }
                    const timeEntry = timeEntryQuery.rows[0];
                    ...
                    const managerCheck = await pgClient.query(`SELECT 1 FROM hr_public.users
             WHERE id = $1 AND (manager_id = $2 OR department_id IN (
               ...
               SET status = 'REJECTED',
                   ...
                   updated_at = NOW()
               WHERE id = $1
               RETURNING *`, [args.timeEntryId, managerId, args.reason.trim()]);
                        // Create notification for employee about rejection
                        await pgClient.query('SELECT hr_public.create_notification($1, $2, $3, $4, $5, $6)', [
                        ...
                        await pgClient.query('INSERT INTO hr_public.security_audit_log (user_id, event_type, description, success) VALUES ($1, $2, $3, $4)', [
                            managerId,
                            'TIME_ENTRY_REJECT',
                ...
                approveLeaveRequest: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const managerId = context.jwtClaims?.user_id;
                    // Verify manager role
                    if (context.jwtClaims?.role_level < 60) {
                    ...
                    const leaveQuery = await pgClient.query(`SELECT lr.*, u.display_name as employee_name
             FROM hr_public.leave_requests lr
             JOIN hr_public.users u ON lr.employee_id = u.id
             WHERE lr.id = $1`, [args.leaveRequestId]);
             ...
             WHERE id = $1 AND (manager_id = $2 OR department_id IN (
               SELECT department_id FROM hr_public.users WHERE id = $2
                    ...
                    const balanceQuery = await pgClient.query('SELECT hr_public.get_leave_balance($1, $2, $3) as balance', [leaveRequest.employee_id, leaveRequest.leave_type, currentYear]);
                    ...
                    const conflictQuery = await pgClient.query(`SELECT id FROM hr_public.leave_requests
             WHERE employee_id = $1
             AND id != $2
             AND status = 'APPROVED'
             AND (
               ...
               (start_date >= $3 AND end_date <= $4)
                    ...
                    try {
                        ...
                        const { rows } = await pgClient.query(`UPDATE hr_public.leave_requests
               SET status = 'APPROVED',
                   approved_by = $2,
                   approved_at = NOW(),
                   ...
                   updated_at = NOW()
               WHERE id = $1
               RETURNING *`, [args.leaveRequestId, managerId, availableBalance, availableBalance - requestedDays]);
                        // Update leave balance
                        await pgClient.query(`UPDATE hr_public.leave_balances
               SET used_days = used_days + $3,
                   updated_at = NOW()
               WHERE employee_id = $1 AND leave_type = $2 AND year = $4`, [leaveRequest.employee_id, leaveRequest.leave_type, requestedDays, currentYear]);
                        ...
                        await pgClient.query('INSERT INTO hr_public.security_audit_log (user_id, event_type, description, success) VALUES ($1, $2, $3, $4)', [managerId, 'LEAVE_APPROVED', `Approved ${requestedDays} days of ${leaveRequest.leave_type} leave for ${leaveRequest.employee_name} (${leaveRequest.start_date} to ${leaveRequest.end_date})`, true]);
                        await pgClient.query('COMMIT');
                        return rows[0];
                    }
                    catch (error) {
                ...
                rejectLeaveRequest: async (_parent, args, context, _resolveInfo) => {
                    try {
                        // Input validation
                        ...
                        if (context.jwtClaims?.role_level < 60) {
                            throw new HRValidationError('Manager access required to reject leave requests', ValidationErrorCodes.INSUFFICIENT_PERMISSION, undefined, { requiredRoleLevel: 60, actualRoleLevel: context.jwtClaims?.role_level });
                        }
                    }
                    catch (error) {
                        handleValidationError(error);
                    ...
                    const leaveQuery = await pgClient.query(`SELECT lr.*, u.display_name as employee_name, u.email as employee_email
             FROM hr_public.leave_requests lr
             JOIN hr_public.users u ON lr.employee_id = u.id
             WHERE lr.id = $1`, [args.leaveRequestId]);
                    if (leaveQuery.rows.length === 0) {
             ...
             WHERE id = $1 AND (manager_id = $2 OR department_id IN (
               ...
               SET status = 'REJECTED',
                   ...
                   updated_at = NOW()
               WHERE id = $1
               RETURNING *`, [args.leaveRequestId, managerId, args.reason.trim()]);
                        // Create notification for employee about rejection
                ...
                cancelLeaveRequest: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const { rows } = await pgClient.query(`UPDATE hr_public.leave_requests
             SET status = 'CANCELLED', updated_at = NOW()
             WHERE id = $1 AND (employee_id = $2 OR user_id = $2)
             RETURNING *`, [args.leaveRequestId, context.jwtClaims?.user_id]);
                    if (rows.length === 0) {
                ...
                updateGoalProgress: async (_parent, args, context, _resolveInfo) => {
                    const { pgClient } = context;
                    const userId = context.jwtClaims?.user_id;
                    if (!userId) {
                        throw new Error('Authentication required');
                    ...
                    const goalQuery = await pgClient.query(`SELECT g.*, u.display_name as employee_name, m.display_name as manager_name
             FROM hr_public.goals g
             JOIN hr_public.users u ON g.employee_id = u.id
             LEFT JOIN hr_public.users m ON g.manager_id = m.id
             WHERE g.id = $1`, [args.goalId]);
                    ...
                    try {
                        ...
                        const { rows } = await pgClient.query(`UPDATE hr_public.goals
               SET progress_percentage = $2,
                   ...
                   updated_at = NOW()
               WHERE id = $1
               RETURNING *`, [args.goalId, args.progress, args.notes, newStatus, completedAt, userId]);
                        // Create progress tracking record
                        await pgClient.query(`INSERT INTO hr_public.goal_progress_updates (
                goal_id, updated_by, old_progress, new_progress, notes, created_at
              ) VALUES ($1, $2, $3, $4, $5, NOW())`, [args.goalId, userId, goal.progress_percentage || 0, args.progress, args.notes]);
...
//# sourceMappingURL=hr-graphql-plugins.js.map
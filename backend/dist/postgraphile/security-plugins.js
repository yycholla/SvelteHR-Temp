...
export class QuerySecurityPlugin {
    ...
    createPlugin() {
        return (build) => {
            ...
            build.hook('postgraphile:http:handler', async (req, context) => {
            ...
            build.hook('GraphQLObjectType:fields:field', (field, build, context) => {
                ...
                if (SECURITY_CONFIG.sensitiveFields.some(sensitive => fieldName.toLowerCase().includes(sensitive.toLowerCase()))) {
     ...
     */
    checkForSuspiciousPatterns(query) {
     ...
     */
    canAccessSensitiveField(fieldName, userRole, userRoleLevel, userId, resource) {
        ...
        if (userRole === 'hr_admin' || userRoleLevel >= 80) {
            return !['personalPhone', 'personalEmail'].some(field => fieldName.toLowerCase().includes(field.toLowerCase()));
        }
        ...
        if (userRole === 'hr_employee' || userRoleLevel >= 20) {
            ...
            return (resourceUserId?.toString() === userId ||
...
export class MonitoringPlugin {
    ...
    createPlugin() {
        return (build) => {
            ...
            build.hook('GraphQLObject:fields:field', (field, build, context) => {
                const fieldName = field.fieldName;
                const originalResolve = field.resolve;
                if (originalResolve) {
                    field.resolve = async (root, args, context, info) => {
     ...
     */
    async recordFieldMetrics(fieldName, operation, duration) {
        try {
     ...
     * Record field errors
     */
    async recordFieldError(fieldName, operation, duration, error) {
        try {
    ...
    createPlugin() {
        return (build) => {
            ...
            build.hook('GraphQLObject:fields:field', (field, build, context) => {
                ...
                if (SECURITY_CONFIG.privilegedOperations.some(op => fieldName.toLowerCase().includes(op.toLowerCase()))) {
                    const originalResolve = field.resolve;
                    if (originalResolve) {
                        field.resolve = async (root, args, context, info) => {
                            const startTime = Date.now();
...
// Export plugins
export const querySecurityPlugin = (redis) => new QuerySecurityPlugin(redis).createPlugin();
export const monitoringPlugin = (redis) => new MonitoringPlugin(redis).createPlugin();
export const auditLoggingPlugin = (redis) => new AuditLoggingPlugin(redis).createPlugin();
// Security plugin bundle
export const securityPlugins = (redis) => [

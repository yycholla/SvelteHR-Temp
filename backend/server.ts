...
app.get('/health', async (req, res) => {
  try {
    ...
    try {
      ...
      health.services.hasura = hasuraHealthResponse.ok ? 'healthy' : 'unhealthy';
    } catch {
...
app.use('/graphql', createProxyMiddleware({
  ...
  onProxyReq: async (proxyReq, req, res) => {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        ...
        if (decoded.hasura) {
          ...
          proxyReq.setHeader('X-Hasura-Allowed-Roles', decoded.hasura.allowed_roles.join(','));
          
          if (decoded.hasura.user_email) {
            proxyReq.setHeader('X-Hasura-User-Email', decoded.hasura.user_email);
          }
          
          if (decoded.hasura.user_department_id) {
            proxyReq.setHeader('X-Hasura-User-Department-Id', decoded.hasura.user_department_id);
          }
          
          if (decoded.hasura.user_manager_id) {
            proxyReq.setHeader('X-Hasura-User-Manager-Id', decoded.hasura.user_manager_id);
          }
          
          if (decoded.hasura.user_role_level) {
            proxyReq.setHeader('X-Hasura-User-Role-Level', decoded.hasura.user_role_level);
          }
        }
      } catch (error) {
    ...
    if (req.headers['x-hasura-admin-secret']) {
      proxyReq.setHeader('X-Hasura-Admin-Secret', req.headers['x-hasura-admin-secret'] as string);
    }
  },
  onError: (err, req, res) => {
...
app.use('/admin/graphql', createProxyMiddleware({
  target: HASURA_URL,
  changeOrigin: true,
  pathRewrite: {
  ...
  onProxyReq: (proxyReq, req, res) => {
    ...
    proxyReq.setHeader('X-Hasura-Admin-Secret', process.env.HASURA_GRAPHQL_ADMIN_SECRET || 'admin-secret');

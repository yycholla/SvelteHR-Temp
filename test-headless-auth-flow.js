...
async function testAuthenticationFlow() {
  ...
  try {
    ...
    page.on('console', (msg) => {
      ...
      if (text.includes('Admin layout') || text.includes('redirect') || text.includes('Authentication')) {
    ...
    const rootVisits = navigationHistory.filter(nav => nav.url === 'http://localhost:5174/').length;

...
COMMENT ON COLUMN hr_public.notification_preferences.event_invites IS 'Notify when invited to events';
COMMENT ON COLUMN hr_public.notification_preferences.event_changes IS 'Notify when event details change';
COMMENT ON COLUMN hr_public.notification_preferences.event_reminders IS 'Send event reminders';
COMMENT ON COLUMN hr_public.notification_preferences.comment_mentions IS 'Notify when mentioned in comments';
COMMENT ON COLUMN hr_public.notification_preferences.waitlist_updates IS 'Notify on waitlist status changes';

...
class AuthDiagnosisRunner {
  ...
  detectAuthIssues(result) {
    ...
    const corsErrors = logs.filter(log =>
      ...
      log.text.includes('cross-origin')
    );
    ...
    if (url.includes('/login') && result.redirects.length > 2) {
      console.log(`   🔄 Potential Login Loop: Stuck on login page after ${result.redirects.length} redirects`);
    }

    // Check for authentication redirect patterns
    const authRedirects = result.redirects.filter(r =>

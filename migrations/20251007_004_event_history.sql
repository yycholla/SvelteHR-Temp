...
CREATE INDEX idx_history_change_type ON event_history(change_type);

-- Add table comment
COMMENT ON TABLE event_history IS E'@omit create,update,delete\nImmutable audit trail of event changes';
COMMENT ON COLUMN event_history.change_type IS 'Type of change: created, updated, deleted, ownership_transfer, attendee_added, attendee_removed';
COMMENT ON COLUMN event_history.old_value IS 'Previous field value as JSONB';
COMMENT ON COLUMN event_history.new_value IS 'New field value as JSONB';

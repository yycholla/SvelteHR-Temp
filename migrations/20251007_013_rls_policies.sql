...
DROP POLICY IF EXISTS attendees_select_policy ON event_attendees;
...
DROP POLICY IF EXISTS attendees_insert_policy ON event_attendees;
CREATE POLICY attendees_insert_policy ON event_attendees
  FOR INSERT WITH CHECK (
    employee_id = current_setting('app.current_user_id', true)::UUID OR
    event_id IN (
      SELECT id FROM events WHERE created_by = current_setting('app.current_user_id', true)::UUID
    )
  );

-- UPDATE: Users can only modify their own RSVP
DROP POLICY IF EXISTS attendees_update_policy ON event_attendees;
...
DROP POLICY IF EXISTS attendees_delete_policy ON event_attendees;
CREATE POLICY attendees_delete_policy ON event_attendees
  FOR DELETE USING (
    employee_id = current_setting('app.current_user_id', true)::UUID OR
    event_id IN (
      SELECT id FROM events WHERE created_by = current_setting('app.current_user_id', true)::UUID
...
DROP POLICY IF EXISTS waitlist_select_policy ON event_waitlist;
CREATE POLICY waitlist_select_policy ON event_waitlist
  FOR SELECT USING (
    event_id IN (SELECT id FROM events)
  );

-- INSERT: Users can add themselves to waitlist
DROP POLICY IF EXISTS waitlist_insert_policy ON event_waitlist;
CREATE POLICY waitlist_insert_policy ON event_waitlist
  FOR INSERT WITH CHECK (
    employee_id = current_setting('app.current_user_id', true)::UUID
  );

-- DELETE: Users can remove themselves from waitlist
DROP POLICY IF EXISTS waitlist_delete_policy ON event_waitlist;
CREATE POLICY waitlist_delete_policy ON event_waitlist
  FOR DELETE USING (
    employee_id = current_setting('app.current_user_id', true)::UUID
  );
...
DROP POLICY IF EXISTS comments_select_policy ON event_comments;
...
DROP POLICY IF EXISTS comments_insert_policy ON event_comments;
CREATE POLICY comments_insert_policy ON event_comments
  FOR INSERT WITH CHECK (
...
DROP POLICY IF EXISTS comments_update_policy ON event_comments;
...
DROP POLICY IF EXISTS comments_delete_policy ON event_comments;
...
DROP POLICY IF EXISTS notifications_update_policy ON event_notifications;
CREATE POLICY notifications_update_policy ON event_notifications
  FOR UPDATE USING (
    user_id = current_setting('app.current_user_id', true)::UUID
  );
...
DROP POLICY IF EXISTS prefs_select_policy ON notification_preferences;
CREATE POLICY prefs_select_policy ON notification_preferences
  FOR SELECT USING (
    user_id = current_setting('app.current_user_id', true)::UUID
  );

-- INSERT: Users can create their own preferences
DROP POLICY IF EXISTS prefs_insert_policy ON notification_preferences;
CREATE POLICY prefs_insert_policy ON notification_preferences
  FOR INSERT WITH CHECK (
    user_id = current_setting('app.current_user_id', true)::UUID
  );

-- UPDATE: Users can update their own preferences
DROP POLICY IF EXISTS prefs_update_policy ON notification_preferences;
CREATE POLICY prefs_update_policy ON notification_preferences
  FOR UPDATE USING (
    user_id = current_setting('app.current_user_id', true)::UUID
  );

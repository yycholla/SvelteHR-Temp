...
class FormsApiClient {
  ...
  async submitForm(formId: string, data: any, contextData?: FormContextData): Promise<ApiResponse<FormSubmission>> {
  ...
  async createFormFromTemplate(templateId: string, customizations?: Partial<Form>): Promise<ApiResponse<Form>> {
...
export const actions = {
  default: async ({ request, params, url }) => {
    ...
    const contextData = contextType && entityId ? {
      type: contextType,
      entityId: entityId
    } : undefined;
...
function applyValidation(schema: z.ZodType<any>, validation: FieldValidation): z.ZodType<any> {
  if (validation.minLength && schema instanceof z.ZodString) {
    schema = schema.min(validation.minLength, `Minimum ${validation.minLength} characters required`);
  }
  ...
  if (validation.pattern && schema instanceof z.ZodString) {
    schema = schema.regex(new RegExp(validation.pattern), validation.patternMessage || 'Invalid format');
  }
...
export class FormContextLoader {
  ...
  static async loadDefaultFormForContext(contextType: string): Promise<Form> {
    ...
    const template = templatesResponse.data.find(t => 
      t.category === contextType || 
      t.name.toLowerCase().includes(contextType.toLowerCase())
    );
...
<div class="performance-review-form">
  ...
  <form method="POST" use:enhance>
    ...
    <div class="form-actions">
      <button type="button" onclick="saveDraft($form)">
        Save Draft
      </button>
      <button type="submit" disabled={$submitting}>
...
describe('Form Schema Builder', () => {
  it('creates valid Zod schema from form definition', () => {
    ...
    // Invalid email
    expect(() => zodSchema.parse({ email: 'invalid', age: 25 }))
      .toThrow('Invalid email');
      
    // Age too low
    expect(() => zodSchema.parse({ email: 'test@example.com', age: 17 }))
      .toThrow('Minimum value: 18');
  });
});
```

...
describe('GraphQL Schema Contract Tests', function() {
  ...
  describe('Data Integrity', function() {
    ...
    it('should have proper foreign key constraints', async function() {
      ...
      expect(fkeys.some(fk => 
        fk.table === 'employees' && 
        fk.column === 'manager_id' && 
        fk.ref_table === 'employees'
      )).to.be.true;
    });
    ...
    it('should have proper check constraints', async function() {
      ...
      expect(constraints.some(c => 
        c.table === 'time_off_requests' && 
        c.name.includes('dates_valid')
      )).to.be.true;
      
      expect(constraints.some(c => 
        c.table === 'performance_reviews' && 
        c.name.includes('rating_valid')
      )).to.be.true;
    });
  });
  ...
  describe('Performance Indexes', function() {
    ...
    it('should have strategic indexes on key columns', async function() {
      ...
      expect(indexes.some(idx => 
        idx.table === 'employees' && 
        idx.definition.includes('department_id')
      )).to.be.true;
      
      expect(indexes.some(idx => 
        idx.table === 'time_off_requests' && 
        idx.definition.includes('employee_id')
      )).to.be.true;
    });

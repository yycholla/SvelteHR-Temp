...
import { buildSchema, getIntrospectionQuery, introspectionFromSchema, validateSchema } from 'graphql';
...
describe('GraphQL Schema Contract Testing', () => {
  ...
  describe('Schema Structure Validation', () => {
    ...
    test('should have proper input and payload types', () => {
      ...
      const inputTypes = typeNames.filter(name => name.endsWith('Input') || name.endsWith('Patch'));
    ...
    test('should have all required query fields', () => {
      ...
      if (queryType && 'fields' in queryType && queryType.fields) {
        ...
        TEST_CONFIG.requiredQueries.forEach(requiredQuery => {
          const hasField = queryFieldNames.some(fieldName =>
    ...
    test('should have all required mutation fields', () => {
      ...
      if (mutationType && 'fields' in mutationType && mutationType.fields) {
        ...
        TEST_CONFIG.requiredMutations.forEach(requiredMutation => {
          const hasField = mutationFieldNames.some(fieldName =>
    ...
    test('should have proper input and payload structure', () => {
      ...
      if (mutationType && 'fields' in mutationType && mutationType.fields) {
        mutationType.fields.forEach(mutation => {
          ...
          const hasInputArg = mutation.args.some(arg =>
            arg.name === 'input' || arg.name.endsWith('Input')
          );
      ...
      TEST_CONFIG.expectedTypes.forEach(typeName => {
        ...
        if (type && 'fields' in type && type.fields) {
          const hasIdField = type.fields.some(field =>
            field.name === 'id' && field.type.name === 'ID'
          );
  ...
  describe('Deprecation Management', () => {
    test('should track deprecated fields', () => {
      ...
      deprecatedFields.forEach(deprecated => {
        const isAllowed = TEST_CONFIG.allowedDeprecations.some(allowed =>
          allowed.type === deprecated.type && allowed.field === deprecated.field
        );

        expect(isAllowed || deprecated.reason,
      ...
      const deeplyNestedTypes = schema.types.filter(type => {
        if ('fields' in type && type.fields) {
          return type.fields.some(field => {
            ...
            return field.type.name?.includes('Connection') &&
                   field.type.name.length > 50; // Arbitrary complexity indicator
          });
        }
        return false;
      });

...
import { startPerformanceMonitoring, stopPerformanceMonitoring } from '../utils/performance-monitor';
...
afterAll(() => {
  ...
  console.log(`- Success rate: ${Math.round(report.performanceTargets.passed / (report.performanceTargets.passed + report.performanceTargets.failed) * 100)}%`);
});
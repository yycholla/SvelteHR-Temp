...
beforeAll(async () => {
  ...
  console.log(`Load test configuration: ${process.env.GRAPHQL_LOAD_TEST_CONCURRENT_REQUESTS || 10} concurrent requests`);
  console.log(`Performance alert threshold: ${process.env.GRAPHQL_PERFORMANCE_ALERT_THRESHOLD || 500}ms`);
});
...
afterEach(async () => {
  ...
  if (currentMetrics?.averageResponseTime > 1000) {
    console.warn(`Slow test detected: ${expect.getState().currentTestName}, avg response time: ${currentMetrics.averageResponseTime}ms`);
  }
});
...
function setupMemoryMonitoring() {
  ...
  const memoryInterval = setInterval(() => {
    ...
    if (heapIncrease > 100 * 1024 * 1024) { // 100MB increase

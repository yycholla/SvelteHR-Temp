...
test.describe('Carbon Components Accessibility', () => {
  ...
  test('Color contrast compliance', async ({ page, a11y }) => {
    ...
    const contrastResults = await a11yHelpers.checkColorContrast(page, [
      'h1', 'h2', 'h3', 'p', 'span', 'a', 'button', 'label', '.bx--label'
    ])
    ...
    if (errorMessages.length > 0) {
      for (const error of errorMessages) {
        ...
        const isAssociated = await error.evaluate(el => {
          const id = el.id
          const associatedField = id ?
            document.querySelector(`[aria-describedby*="${id}"]`) : null
      ...
      const liveRegionResult = await a11y.testLiveRegions(
        async () => {
          await submitButton.click()
        },
        'error'
  ...
  test('Reduced motion support', async ({ page, a11y }) => {
    ...
    const animationElements = await page.locator(
      '.bx--loading, .bx--toast, [class*="animate"], [class*="transition"]'
    ).all()

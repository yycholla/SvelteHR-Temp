		...
		const a11y = {
			...
			async testLiveRegions(triggerAction: () => Promise<void>, expectedMessage?: string) {
				...
				const liveRegions = await page
					...
					.all();
				const initialContent = await Promise.all(liveRegions.map((region) => region.textContent()));
				...
				// Check for updates
				const updatedContent = await Promise.all(liveRegions.map((region) => region.textContent()));

				const hasUpdates = updatedContent.some(
					(content, index) => content !== initialContent[index]
				);
	...
	customMatchers: {
		toPassA11yAudit: (results: any) => {
			...
			if (pass) {
				...
				const summary = violations.map((v: any) => `${v.id}: ${v.description}`).join('\n');

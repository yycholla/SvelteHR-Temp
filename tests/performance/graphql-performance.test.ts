...
import { performanceMonitor, markPerformance, measurePerformance, timeFunction } from '../../tests/utils/performance-monitor.js';
...
describe('GraphQL Performance Tests', () => {
  ...
  test('Complex GraphQL queries stay within acceptable limits', async () => {
    ...
    const validation = budgetValidator.validateMetric('GraphQL Response Time (Average)', complexQuery!.duration);
    console.log(`Budget validation: ${validation.message}`);

    // Should be within warning threshold at minimum
    expect(complexQuery!.duration).toBeLessThan(PERFORMANCE_BUDGETS.budgets.find(b =>
      b.name === 'GraphQL Response Time (Average)'
    )!.warning);
  });
  ...
  test('Slow queries are properly identified and flagged', async () => {
    ...
    const validation = budgetValidator.validateMetric('GraphQL Response Time (Average)', slowQuery!.duration);
  ...
  test('Performance monitoring captures GraphQL metrics correctly', async () => {
    ...
    const operations = [
      ...
      () => timeFunction('Mutation1', 'graphql', () => mockGraphQLOperations.mutationOperation({}), ['batch'])
    ];
  ...
  test('Performance budgets are enforced correctly', async () => {
    ...
    const graphqlResults = validation.results.filter(r =>
      r.budget.name.includes('GraphQL')
    );

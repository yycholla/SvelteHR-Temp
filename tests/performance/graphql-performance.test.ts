	...
	describe('GraphQL Performance Tests', () => {
		...
		test('Complex GraphQL queries stay within acceptable limits', async () => {
			...
			expect(complexQuery!.duration).toBeLessThan(
				PERFORMANCE_BUDGETS.budgets.find((b) => b.name === 'GraphQL Response Time (Average)')!
					.warning
			);
		});
		...
		test('Performance monitoring captures GraphQL metrics correctly', async () => {
			...
			const operations = [
				...
				() =>
					timeFunction('Mutation1', () => mockGraphQLOperations.mutationOperation({}), ['batch'])
			];
			...
			const avgDuration =
				batchEntries.reduce((sum, e) => sum + e.duration, 0) / batchEntries.length;

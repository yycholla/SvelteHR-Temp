...
class PerformanceReporter implements Reporter {
  ...
  onTestEnd(test: TestCase, result: TestResult) {
    ...
    if (testMetrics.duration > this.performanceBudgets.totalTest) {
      console.warn(`⚠️  Test "${testMetrics.testName}" exceeded time budget: ${testMetrics.duration}ms`);
    }
  }
  ...
  private extractStepMetrics(result: TestResult): Array<{ title: string; duration: number; category: string }> {
  ...
  private checkPerformanceBudgets(metrics: PerformanceMetrics) {
      ...
      switch (step.category) {
        case 'page-load':
          if (step.duration > this.performanceBudgets.pageLoad) {
            violations.push(`Page load "${step.title}": ${step.duration}ms > ${this.performanceBudgets.pageLoad}ms`);
          }
          break;
        case 'api-request':
          if (step.duration > this.performanceBudgets.apiRequest) {
            violations.push(`API request "${step.title}": ${step.duration}ms > ${this.performanceBudgets.apiRequest}ms`);
          }
          break;
        case 'component-render':
          if (step.duration > this.performanceBudgets.componentRender) {
            violations.push(`Component render "${step.title}": ${step.duration}ms > ${this.performanceBudgets.componentRender}ms`);
          }
          break;
      }
    });
    ...
    if (metrics.duration > this.performanceBudgets.totalTest) {
      violations.push(`Total test duration: ${metrics.duration}ms > ${this.performanceBudgets.totalTest}ms`);
    }
  ...
  private extractAdditionalMetrics(metrics: PerformanceMetrics, result: TestResult) {
    ...
    if (result.stdout) {
      const errorMatches = result.stdout.filter(output =>
  ...
  private async generatePerformanceReport(result: FullResult, totalDuration: number) {
    const report = {
      summary: {
        ...
        totalDuration,
        averageTestDuration: this.performanceMetrics.reduce((sum, m) => sum + m.duration, 0) / this.performanceMetrics.length,
  ...
  private async generateCSVReport() {
    ...
    const csvRows = this.performanceMetrics.map(metric => [
      ...
      metric.annotations.filter(a => a.includes('Performance Budget Violations')).length.toString(),
    ]);
...
<body>
  ...
  private generateCategoryBreakdown(): Record<string, { count: number; averageDuration: number }> {
    const breakdown: Record<string, { count: number; totalDuration: number; averageDuration: number }> = {};
    ...
    Object.keys(breakdown).forEach(category => {
      breakdown[category].averageDuration = breakdown[category].totalDuration / breakdown[category].count;
    });
  ...
  private generateBrowserBreakdown(): Record<string, { count: number; averageDuration: number }> {
    ...
    Object.keys(breakdown).forEach(browser => {
      breakdown[browser].averageDuration = breakdown[browser].totalDuration / breakdown[browser].count;
    });

...
class PerformanceMonitor {
  ...
  private checkPerformanceTarget(entry: PerformanceEntry): void {
    ...
    if (tags.includes('graphql') || name.includes('graphql')) {
      if (duration > PERFORMANCE_TARGETS.GRAPHQL_RESPONSE_TIME) {
        console.warn(`⚠️  GraphQL operation '${name}' exceeded target: ${duration}ms > ${PERFORMANCE_TARGETS.GRAPHQL_RESPONSE_TIME}ms`);
      }
    }
    ...
    if (tags.includes('page-load') || name.includes('page')) {
      if (duration > PERFORMANCE_TARGETS.PAGE_LOAD_TIME) {
        console.warn(`⚠️  Page load '${name}' exceeded target: ${duration}ms > ${PERFORMANCE_TARGETS.PAGE_LOAD_TIME}ms`);
      }
    }
    ...
    if (tags.includes('component') || name.includes('render')) {
      if (duration > PERFORMANCE_TARGETS.COMPONENT_RENDER_TIME) {
        console.warn(`⚠️  Component render '${name}' exceeded target: ${duration}ms > ${PERFORMANCE_TARGETS.COMPONENT_RENDER_TIME}ms`);
      }
    }
    ...
    if (tags.includes('database') || name.includes('db') || name.includes('query')) {
      if (duration > PERFORMANCE_TARGETS.DATABASE_QUERY_TIME) {
        console.warn(`⚠️  Database operation '${name}' exceeded target: ${duration}ms > ${PERFORMANCE_TARGETS.DATABASE_QUERY_TIME}ms`);
      }
    }
    ...
    if (tags.includes('api') || name.includes('api') || name.includes('request')) {
      if (duration > PERFORMANCE_TARGETS.API_REQUEST_TIME) {
        console.warn(`⚠️  API request '${name}' exceeded target: ${duration}ms > ${PERFORMANCE_TARGETS.API_REQUEST_TIME}ms`);
      }
    }
  }
  ...
  private generateReport(): PerformanceReport {
    ...
    // Extreme values
    const slowestTest = entries.reduce((prev, current) =>
      (!prev || current.duration > prev.duration) ? current : prev, null as PerformanceEntry | null
    );

    const fastestTest = entries.reduce((prev, current) =>
      (!prev || current.duration < prev.duration) ? current : prev, null as PerformanceEntry | null
    );
  ...
  private analyzePerformanceTargets() {
    ...
    this.entries.forEach(entry => {
      ...
      if (tags.includes('graphql') && duration > PERFORMANCE_TARGETS.GRAPHQL_RESPONSE_TIME) {
        targetMet = false;
        warnings.push(`GraphQL '${name}': ${duration}ms > ${PERFORMANCE_TARGETS.GRAPHQL_RESPONSE_TIME}ms`);
      }

      if (tags.includes('page-load') && duration > PERFORMANCE_TARGETS.PAGE_LOAD_TIME) {
        targetMet = false;
        warnings.push(`Page load '${name}': ${duration}ms > ${PERFORMANCE_TARGETS.PAGE_LOAD_TIME}ms`);
      }

      if (tags.includes('component') && duration > PERFORMANCE_TARGETS.COMPONENT_RENDER_TIME) {
        targetMet = false;
        warnings.push(`Component '${name}': ${duration}ms > ${PERFORMANCE_TARGETS.COMPONENT_RENDER_TIME}ms`);
      }

      if (tags.includes('database') && duration > PERFORMANCE_TARGETS.DATABASE_QUERY_TIME) {
        targetMet = false;
        warnings.push(`Database '${name}': ${duration}ms > ${PERFORMANCE_TARGETS.DATABASE_QUERY_TIME}ms`);
      }
...
export function measurePerformance(name: string, tags?: string[], metadata?: Record<string, any>): number {
...
export class PerformanceAssertions {
  ...
  static assertWithinTarget(
    ...
    if (duration > target * 0.8) { // Warn if within 80% of target
  ...
  static assertGraphQLPerformance(duration: number, operationName: string): void {
    this.assertWithinTarget(duration, PERFORMANCE_TARGETS.GRAPHQL_RESPONSE_TIME, `GraphQL ${operationName}`);
  }
  ...
  static assertComponentPerformance(duration: number, componentName: string): void {
    this.assertWithinTarget(duration, PERFORMANCE_TARGETS.COMPONENT_RENDER_TIME, `Component ${componentName}`);
  }
  ...
  static assertDatabasePerformance(duration: number, queryName: string): void {
    this.assertWithinTarget(duration, PERFORMANCE_TARGETS.DATABASE_QUERY_TIME, `Database ${queryName}`);
  }
  ...
  static assertSuitePerformance(report: PerformanceReport): void {
    ...
    const successRate = report.performanceTargets.passed /

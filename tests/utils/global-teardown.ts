...
function generateReadablePerformanceReport(report: any): string {
	...
	if (report.slowestOperation) {
		...
		lines.push(
			`• Slowest: ${report.slowestOperation.name} (${report.slowestOperation.duration}ms)`
		);
		if (report.fastestOperation) {
			lines.push(
				`• Fastest: ${report.fastestOperation.name} (${report.fastestOperation.duration}ms)`
			);
		}
		lines.push('');
	}
...
async function cleanupTempFiles() {
	const tempPaths = ['./test-results/temp', './playwright-report/temp', './coverage/temp'];

...
describe('Performance Integration Tests', () => {
  ...
  test('should handle concurrent user actions without blocking', async () => {
    ...
    await expect(page.locator('[data-testid="loading-spinner"]')).not.toBeVisible({ timeout: 1000 });
  });
  ...
  test('should optimize image and asset loading', async () => {
    ...
    page.on('response', response => {
      if (response.url().includes('.jpg') || response.url().includes('.png') || response.url().includes('.css') || response.url().includes('.js')) {
        resources.push({
          url: response.url(),
  ...
  test('should handle large dataset exports efficiently', async () => {
    ...
    await expect(page.locator('[data-testid="export-status"]')).toContainText('Processing', { timeout: 5000 });

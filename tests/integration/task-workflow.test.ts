...
describe('Task Management Workflow Integration', () => {
	...
	test('should create new task with assignment by manager', async () => {
		...
		await managerPage.fill(
			...
			'Compile Q3 performance metrics and analysis'
		);
		...
		// Verify task details
		await expect(managerPage.locator('[data-testid="task-title"]')).toContainText(
			'Complete quarterly report'
		);
		await expect(managerPage.locator('[data-testid="task-status"]')).toContainText('Pending');
		await expect(managerPage.locator('[data-testid="assigned-to"]')).toContainText(
	...
	test('should notify assigned employee of new task', async () => {
		...
		await expect(employeePage.locator('[data-testid="notification-item"]')).toContainText(
			'New task assigned: Complete quarterly report'
		);

		// Click notification to navigate to task
		await employeePage.click(
			'[data-testid="notification-item"]:has-text("Complete quarterly report")'
		);
		await employeePage.waitForURL(/tasks\/[^\/]+$/);

		// Verify task details from employee perspective
		await expect(employeePage.locator('[data-testid="task-title"]')).toContainText(
			'Complete quarterly report'
		);
	...
	test('should display task in employee task list with filtering', async () => {
		...
		await expect(employeePage.locator('[data-testid="task-list"]')).toContainText(
			'Complete quarterly report'
		);
		...
		await expect(
			employeePage.locator('[data-testid="task-item"]:has-text("Complete quarterly report")')
		).toBeVisible();

		// Test sorting
		await employeePage.selectOption('select[data-testid="sort-by"]', 'dueDate');
		await expect(employeePage.locator('[data-testid="task-item"]').first()).toContainText(
			'Complete quarterly report'
		);
	});
	...
	test('should allow employee to start working on task', async () => {
		...
		await expect(employeePage.locator('[data-testid="task-timeline"]')).toContainText(
			'Started by employee@mountaincarerx.com'
		);
	...
	test('should track task progress and time logging', async () => {
		...
		await expect(employeePage.locator('[data-testid="progress-bar"]')).toHaveAttribute(
			'aria-valuenow',
			'25'
		);
	...
	test('should complete subtasks and update parent task progress', async () => {
		...
		await expect(
			employeePage.locator('[data-testid="subtask-status"]:has-text("Completed")')
		).toHaveCount(1);
		...
		await employeePage.click(
			'[data-testid="subtask-item"]:has-text("Analyze performance metrics")'
		);
		...
		await employeePage.fill(
			...
			'Completed analysis and drafted report'
		);
		await employeePage.click('button[data-testid="save-time-log"]');
	});
	...
	test('should notify manager of task progress updates', async () => {
		...
		await expect(managerPage.locator('[data-testid="notification-item"]')).toContainText(
			'Task progress updated: Complete quarterly report (50%)'
		);
		...
		await expect(managerPage.locator('[data-testid="progress-bar"]')).toHaveAttribute(
			'aria-valuenow',
			'50'
		);
	...
	test('should allow manager to add comments and feedback', async () => {
		...
		await managerPage.fill(
			...
			'Great progress! Please include competitor analysis in final report.'
		);
		await managerPage.click('button[data-testid="post-comment"]');

		// Comment should appear in timeline
		await expect(managerPage.locator('[data-testid="comment-timeline"]')).toContainText(
			'Great progress! Please include competitor analysis'
		);
		...
		await expect(managerPage.locator('[data-testid="timeline"]')).toContainText(
			'Due date extended to September 28, 2025'
		);
	});
	...
	test('should notify employee of manager comments and changes', async () => {
		...
		await employeePage.click('[data-testid="notifications-menu"]');
		await expect(employeePage.locator('[data-testid="notification-item"]')).toContainText(
			'Manager commented on: Complete quarterly report'
		);
		await expect(employeePage.locator('[data-testid="notification-item"]')).toContainText(
			'Task updated: Complete quarterly report'
		);
		...
		await expect(employeePage.locator('[data-testid="comment-timeline"]')).toContainText(
			'Great progress! Please include competitor analysis'
		);

		// Verify due date change
		await expect(employeePage.locator('[data-testid="due-date"]')).toContainText(
			'September 28, 2025'
		);
	});
	...
	test('should handle task completion and approval workflow', async () => {
		...
		await employeePage.fill(
			'textarea[data-testid="completion-notes"]',
			'Quarterly report completed with competitor analysis included'
		);
		...
		await expect(employeePage.locator('[data-testid="task-status"]')).toContainText(
	...
	test('should notify manager of task completion for review', async () => {
		...
		await managerPage.click('[data-testid="notifications-menu"]');
		await expect(managerPage.locator('[data-testid="notification-item"]')).toContainText(
			'Task completed: Complete quarterly report - Review needed'
		);
		...
		await expect(managerPage.locator('[data-testid="task-status"]')).toContainText(
			'Completed - Pending Review'
		);
		await expect(managerPage.locator('[data-testid="deliverable-file"]')).toContainText(
			'Q3_Report.pdf'
		);
		await expect(managerPage.locator('[data-testid="completion-notes"]')).toContainText(
	...
	test('should allow manager to approve or request revisions', async () => {
		...
		await managerPage.fill(
			'textarea[data-testid="approval-feedback"]',
			'Excellent work! Report meets all requirements and provides valuable insights.'
		);
		...
		await expect(managerPage.locator('[data-testid="timeline"]')).toContainText(
			'Task approved by manager@mountaincarerx.com'
		);
	});
	...
	test('should update employee dashboard with completed task metrics', async () => {
		...
		await employeePage.click('[data-testid="notifications-menu"]');
		await expect(employeePage.locator('[data-testid="notification-item"]')).toContainText(
			'Task approved: Complete quarterly report'
		);
		...
		await expect(employeePage.locator('[data-testid="recent-activity"]')).toContainText(
	...
	test('should generate task completion report for manager dashboard', async () => {
		...
		await expect(managerPage.locator('[data-testid="average-completion-time"]')).toContainText(
			'6 hours'
		);
		await expect(managerPage.locator('[data-testid="team-performance-average"]')).toContainText(
			'5.0'
		);
		...
		await expect(managerPage.locator('[data-testid="task-report-table"]')).toContainText(
			'Complete quarterly report'
		);

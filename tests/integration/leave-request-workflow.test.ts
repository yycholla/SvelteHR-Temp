...
describe('Leave Request Workflow Integration', () => {
  ...
  test('should display employee leave balances and history', async () => {
    ...
    for (const leaveType of leaveTypes) {
      await expect(employeePage.locator(`[data-testid="balance-${leaveType.toLowerCase()}"]`)).toBeVisible();
    }
    
    // Verify vacation balance details
    await expect(employeePage.locator('[data-testid="balance-vacation"]')).toContainText('15 days remaining');
    await expect(employeePage.locator('[data-testid="balance-vacation"]')).toContainText('20 days total');
    
    // Check leave history
    await expect(employeePage.locator('[data-testid="leave-history"]')).toBeVisible();
    await expect(employeePage.locator('[data-testid="leave-history"]')).toContainText('Previous Requests');
  });
  ...
  test('should submit vacation leave request with validation', async () => {
    ...
    await expect(employeePage.locator('[data-testid="remaining-balance"]')).toContainText('10 days remaining');
  ...
  test('should validate leave request conflicts and availability', async () => {
    ...
    await expect(employeePage.locator('[data-testid="conflict-warning"]')).toContainText('Overlaps with existing request');
  ...
  test('should check team availability and coverage', async () => {
    ...
    await expect(employeePage.locator('[data-testid="coverage-suggestion"]')).toContainText('Consider assigning tasks to');
  });
  ...
  test('should notify direct manager of pending leave request', async () => {
    ...
    await expect(managerPage.locator('[data-testid="notification-item"]')).toContainText('Leave request pending approval');
    await expect(managerPage.locator('[data-testid="notification-item"]')).toContainText('employee@mountaincarerx.com');
  ...
  test('should allow manager to review request details and approve', async () => {
    ...
    await expect(managerPage.locator('[data-testid="employee-details"]')).toContainText('employee@mountaincarerx.com');
    await expect(managerPage.locator('[data-testid="leave-balance"]')).toContainText('15 days remaining');
    await expect(managerPage.locator('[data-testid="emergency-contact"]')).toContainText('Jane Doe - 555-123-4567');
    ...
    await managerPage.fill('textarea[data-testid="approval-comments"]', 'Approved. Team coverage has been arranged.');
    await managerPage.selectOption('select[data-testid="coverage-contact"]', 'backup-employee@mountaincarerx.com');
    ...
    await expect(managerPage.locator('[data-testid="request-status"]')).toContainText('Manager Approved');
  });
  ...
  test('should route multi-level approval to HR for policy compliance', async () => {
    ...
    await managerPage.click('[data-testid="approve-request"]');
    await managerPage.fill('textarea[data-testid="approval-comments"]', 'Manager approval - forwarding to HR');
    await managerPage.click('button[data-testid="confirm-approval"]');
    
    // Should show "Pending HR Approval" status
    await expect(managerPage.locator('[data-testid="request-status"]')).toContainText('Pending HR Approval');
  });
  ...
  test('should notify HR of requests requiring final approval', async () => {
    ...
    await expect(hrPage.locator('[data-testid="notification-item"]')).toContainText('Extended leave request requires HR approval');
  ...
  test('should allow HR final approval with policy compliance checks', async () => {
    ...
    await hrPage.fill('textarea[data-testid="hr-approval-notes"]', 'Approved under extended leave policy. FMLA documentation required.');
    await hrPage.selectOption('select[data-testid="approval-conditions"]', 'Documentation Required');
    await hrPage.click('button[data-testid="final-approve"]');
    
    // Should show final approval status
    await expect(hrPage.locator('[data-testid="request-status"]')).toContainText('Approved - Final');
  });
  ...
  test('should notify employee of approval and update leave balance', async () => {
    ...
    await expect(employeePage.locator('[data-testid="notification-item"]')).toContainText('Leave request approved: October 1-5, 2025');
    await expect(employeePage.locator('[data-testid="notification-item"]')).toContainText('Extended leave approved: December 1-20, 2025');
    
    // Verify updated leave balances
    await employeePage.goto('http://localhost:5173/leave');
    await expect(employeePage.locator('[data-testid="balance-vacation"]')).toContainText('10 days remaining'); // 15 - 5
    
    // Check approved requests in calendar
    await expect(employeePage.locator('[data-testid="approved-leave"]')).toContainText('October 1-5, 2025');
    await expect(employeePage.locator('[data-testid="approved-leave"]')).toContainText('December 1-20, 2025');
  });
  ...
  test('should handle emergency leave request with immediate approval', async () => {
    ...
    await employeePage.fill('input[name="emergencyDetails"]', 'Unable to work due to severe symptoms');
    ...
    await expect(employeePage.locator('[data-testid="request-status"]')).toContainText('Emergency Approved');
    
    // Medical certificate reminder
    await expect(employeePage.locator('[data-testid="medical-cert-reminder"]')).toContainText('Medical certificate required within 3 days');
  });
  ...
  test('should integrate approved leave with team calendar', async () => {
    ...
    await expect(managerPage.locator('[data-testid="calendar-event"]')).toContainText('employee@mountaincarerx.com - Vacation');
    ...
    await expect(managerPage.locator('[data-testid="coverage-details"]')).toContainText('Coverage: backup-employee@mountaincarerx.com');
  ...
  test('should handle leave cancellation and balance restoration', async () => {
    ...
    await employeePage.fill('textarea[data-testid="cancellation-reason"]', 'Personal circumstances changed');
    await employeePage.click('button[data-testid="confirm-cancellation"]');
    
    // Should restore leave balance
    await expect(employeePage.locator('[data-testid="cancellation-success"]')).toContainText('Leave cancelled and balance restored');
    
    // Verify balance update
    await expect(employeePage.locator('[data-testid="balance-extended"]')).toContainText('15 days restored');
  });
  ...
  test('should generate leave utilization reports for management', async () => {
    ...
    await expect(hrPage.locator('[data-testid="department-breakdown"]')).toContainText('IT Department: 65% utilization');
    
    // Monthly leave patterns
    await expect(hrPage.locator('[data-testid="monthly-patterns"]')).toBeVisible();
    await expect(hrPage.locator('[data-testid="peak-months"]')).toContainText('December: High volume');
    ...
    await expect(hrPage.locator('[data-testid="export-queued"]')).toContainText('Report generation started');
  });
});
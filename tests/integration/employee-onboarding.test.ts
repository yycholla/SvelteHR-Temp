...
describe('Employee Onboarding Workflow Integration', () => {
	...
	test('should create new employee account by HR admin', async () => {
		...
		await expect(hrPage.locator('[data-testid="success-message"]')).toContainText(
	...
	test('should automatically generate onboarding tasks for new employee', async () => {
		...
		await expect(hrPage.locator('[data-testid="task-status"]:has-text("Pending")')).toHaveCount(
	...
	test('should send welcome email with login credentials to new employee', async () => {
		...
		await expect(hrPage.locator('[data-testid="notification-list"]')).toContainText(
			'Welcome to MountainHR - john.doe@mountaincarerx.com'
		);

		// Check notification details
		await hrPage.click('[data-testid="notification-item"]:has-text("Welcome to MountainHR")');
		await expect(hrPage.locator('[data-testid="notification-details"]')).toContainText(
	...
	test('should allow new employee to login with temporary credentials', async () => {
		...
		await expect(employeePage.locator('[data-testid="welcome-message"]')).toContainText(
			'Welcome, John!'
		);
	});
	...
	test('should handle document upload and verification workflow', async () => {
		...
		await expect(employeePage.locator('[data-testid="verification-pending"]')).toContainText(
			'Under review by HR'
		);
	});
	...
	test('should notify HR admin of pending document reviews', async () => {
		...
		await expect(hrPage.locator('[data-testid="notification-item"]')).toContainText(
			'John Doe submitted onboarding documents'
		);

		// Navigate to document review
		await hrPage.click(
			'[data-testid="notification-item"]:has-text("submitted onboarding documents")'
		);
	...
	test('should allow HR to approve documents and update employee status', async () => {
		...
		await expect(
			hrPage.locator('[data-testid="document-status"]:has-text("Approved")')
		).toHaveCount(2);
	});
	...
	test('should update employee onboarding progress after HR approval', async () => {
		...
		await expect(employeePage.locator('[data-testid="approval-notification"]')).toContainText(
			'Documents approved'
		);
		...
		await expect(
			employeePage.locator(
				'[data-testid="submit-documents-task"] [data-testid="task-status-complete"]'
			)
		).toBeVisible();
	});
	...
	test('should schedule and track orientation session attendance', async () => {
		...
		await expect(employeePage.locator('[data-testid="session-details"]')).toContainText(
			'September 16, 2025'
		);
	});
	...
	test('should complete full onboarding workflow and activate employee', async () => {
		...
		await expect(employeePage.locator('[data-testid="welcome-complete"]')).toContainText(
			'Onboarding complete!'
		);
	...
	test('should update employee status to active after onboarding completion', async () => {
		...
		await expect(employeeRow.locator('[data-testid="onboarding-status"]')).toContainText(
			'Completed'
		);

		// Onboarding completion date should be set
		await employeeRow.click();
		await expect(hrPage.locator('[data-testid="onboarding-completed-date"]')).toContainText(
			'September 10, 2025'
		);
	});
});

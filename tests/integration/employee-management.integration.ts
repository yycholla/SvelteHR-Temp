...
import { createTestContext, cleanupTestData, TestUser, TestEmployee, TestEmployeeData } from '../utils/test-helpers';
...
describe('Employee Management Integration Tests', () => {
  ...
  describe('Performance and Load Testing', () => {
    test('should handle bulk employee operations within performance targets', async () => {
      ...
      const employeePromises = Array(50).fill(null).map((_, index) => {
        const createEmployeeMutation = `
          mutation CreateEmployee($input: CreateEmployeeInput!) {
            createEmployee(input: $input) {
              id

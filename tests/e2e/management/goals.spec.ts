...
test.describe('Management Goals & OKRs Tracking Page', () => {
  ...
  test('should display goals and OKRs data table', async ({ page }) => {
    ...
    // Check table headers
    await expect(page.locator('th')).toContainText(['Goal', 'Owner', 'Type', 'Progress', 'Due Date', 'Status', 'Actions']);
  ...
  test('should create new team goal with key results', async ({ page }) => {
    ...
    await page.fill('[data-testid="goal-description"]', 'Increase customer satisfaction scores and reduce support ticket resolution time');
    ...
    // Add key results
    await page.fill('[data-testid="key-result-1-title"]', 'Achieve 90% customer satisfaction score');
    await page.fill('[data-testid="key-result-1-target"]', '90');
    await page.selectOption('[data-testid="key-result-1-unit"]', 'percentage');

    await page.fill('[data-testid="key-result-2-title"]', 'Reduce average response time to under 2 hours');
    await page.fill('[data-testid="key-result-2-target"]', '2');
    await page.selectOption('[data-testid="key-result-2-unit"]', 'hours');

    await page.fill('[data-testid="key-result-3-title"]', 'Implement 5 new customer feedback improvements');
    ...
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Goal created successfully');

    // Modal should close and new goal should appear in table
    await expect(page.locator('[data-testid="goal-modal"]')).not.toBeVisible();
    await expect(page.locator('[data-testid="goal-row"]')).toContainText('Improve Customer Satisfaction');
  });
  ...
  test('should update goal progress and key results', async ({ page }) => {
    ...
    // Add progress notes
    await page.fill('[data-testid="progress-notes"]', 'Good progress on customer satisfaction metrics. Response time needs improvement.');
    ...
    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Goal updated successfully');
  ...
  test('should create individual employee goal', async ({ page }) => {
    ...
    await page.fill('[data-testid="goal-description"]', 'Complete professional certification to advance technical skills');
    ...
    await page.fill('[data-testid="key-result-1-title"]', 'Pass certification exam with score >= 85%');
    ...
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Goal created successfully');
    await expect(page.locator('[data-testid="goal-row"]')).toContainText('Complete Advanced Certification');
  });
  ...
  test('should validate required fields in goal creation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="title-error"]')).toContainText('Goal title is required');
    await expect(page.locator('[data-testid="description-error"]')).toContainText('Goal description is required');
    await expect(page.locator('[data-testid="type-error"]')).toContainText('Goal type is required');
    await expect(page.locator('[data-testid="owner-error"]')).toContainText('Goal owner is required');
    ...
    await expect(page.locator('[data-testid="key-results-error"]')).toContainText('At least one key result is required');
  ...
  test('should delete goal with confirmation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="confirmation-message"]')).toContainText('Are you sure you want to delete this goal?');
    await expect(page.locator('[data-testid="warning-message"]')).toContainText('This will also delete all associated key results');

    // Confirm deletion
    await page.click('[data-testid="confirm-delete"]');

    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Goal deleted successfully');

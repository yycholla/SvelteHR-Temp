...
test.describe('Management Leave Approvals Page', () => {
  ...
  test('should display pending leave requests for manager approval', async ({ page }) => {
    ...
    // Check table headers
    await expect(page.locator('th')).toContainText(['Employee', 'Leave Type', 'Dates', 'Days', 'Status', 'Actions']);
  ...
  test('should approve leave request with manager comments', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="modal-title"]')).toContainText('Approve Leave Request');
    ...
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Leave request approved successfully');
  ...
  test('should deny leave request with required manager comments', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="comments-error"]')).toContainText('Manager comments are required for denial');

    // Add required manager comments
    await page.fill('[data-testid="manager-comments"]', 'Cannot approve due to project deadline conflicts');

    // Confirm denial
    await page.click('[data-testid="confirm-deny"]');

    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Leave request denied');
  ...
  test('should handle empty state when no pending requests', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="empty-state"]')).toContainText('No pending leave requests found');

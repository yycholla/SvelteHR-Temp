  ...
  return new Promise((resolve) => {
    ...
    setTimeout(() => {
      ...
      const slowestOperation = operations.reduce((prev, current) =>
        prev.duration > current.duration ? prev : current, operations[0])?.name || '';
...
test.describe('Performance Validation', () => {
  ...
  test('Dashboard page loads within performance budget', async ({ page }) => {
    ...
    console.log(`📊 Page load metrics:`, {
      ...
      firstContentfulPaint: loadMetrics.firstContentfulPaint ? `${loadMetrics.firstContentfulPaint}ms` : 'N/A'
    });

    // Validate performance budgets
    expect(loadMetrics.navigationTime, 'Page navigation time should be under budget')
      .toBeLessThan(PERFORMANCE_BUDGETS.PAGE_LOAD_TIME);

    expect(loadMetrics.domContentLoaded, 'DOM content loaded should be fast')
      .toBeLessThan(PERFORMANCE_BUDGETS.PAGE_LOAD_TIME / 2);

    if (loadMetrics.firstContentfulPaint) {
      expect(loadMetrics.firstContentfulPaint, 'First Contentful Paint should meet standards')
  ...
  test('GraphQL operations meet response time targets', async ({ page }) => {
    ...
    if (graphqlMetrics.operations.length > 0) {
      expect(graphqlMetrics.averageDuration, 'Average GraphQL response time should meet budget')
        .toBeLessThan(PERFORMANCE_BUDGETS.GRAPHQL_RESPONSE_TIME);

      expect(graphqlMetrics.p95Duration, 'P95 GraphQL response time should meet strict budget')
  ...
  test('Memory usage stays within budget', async ({ page }) => {
    ...
    expect(finalMemory.heapUsed, 'Memory usage should stay within budget')
      .toBeLessThan(PERFORMANCE_BUDGETS.MEMORY_LIMIT);

    // Memory growth should be reasonable
    const memoryGrowth = finalMemory.heapUsed - initialMemory.heapUsed;
    expect(memoryGrowth, 'Memory growth should be controlled')
      .toBeLessThan(PERFORMANCE_BUDGETS.MEMORY_LIMIT * 0.3); // Max 30MB growth
  });
  ...
  test('Core Web Vitals meet Google standards', async ({ page }) => {
    ...
    if (webVitals.lcp) {
      expect(webVitals.lcp, 'Largest Contentful Paint should meet good standards')
        .toBeLessThan(PERFORMANCE_BUDGETS.LCP_GOOD);
    }

    if (webVitals.fid) {
      expect(webVitals.fid, 'First Input Delay should meet good standards')
        .toBeLessThan(PERFORMANCE_BUDGETS.FID_GOOD);
    }

    if (webVitals.cls) {
      expect(webVitals.cls, 'Cumulative Layout Shift should meet good standards')
        .toBeLessThan(PERFORMANCE_BUDGETS.CLS_GOOD);
    }

    if (webVitals.fcp) {
      expect(webVitals.fcp, 'First Contentful Paint should meet good standards')
        .toBeLessThan(PERFORMANCE_BUDGETS.FCP_GOOD);
    }

    if (webVitals.ttfb) {
      expect(webVitals.ttfb, 'Time to First Byte should meet good standards')
        .toBeLessThan(PERFORMANCE_BUDGETS.TTFB_GOOD);
    }
  });
  ...
  test('Export operations complete within time limits', async ({ page }) => {
    ...
    const exportButton = page.locator('[data-testid="export-button"], button:has-text("Export"), [class*="export"]');

    if (await exportButton.first().isVisible()) {
      const startTime = Date.now();

      // Start the download and capture any download events
      const downloadPromise = page.waitForEvent('download', { timeout: PERFORMANCE_BUDGETS.EXPORT_OPERATION_TIME });
      ...
      try {
        ...
        expect(exportTime, 'Export operation should complete within time budget')
          .toBeLessThan(PERFORMANCE_BUDGETS.EXPORT_OPERATION_TIME);

        // Clean up download
        await download.delete();
      } catch (error) {
        console.log('ℹ️ Export functionality may not be implemented yet or requires authentication');
      }
    } else {
  ...
  test('Real-time features have low latency', async ({ page }) => {
    ...
    for (const interaction of interactions) {
      ...
      if (await element.isVisible()) {
        ...
        try {
          ...
          expect(latency, `${interaction.action} should be responsive`)
            .toBeLessThan(PERFORMANCE_BUDGETS.REAL_TIME_UPDATE_LATENCY);
    ...
    if (latencies.length > 0) {
      ...
      expect(averageLatency, 'Average interaction latency should be low')
        .toBeLessThan(PERFORMANCE_BUDGETS.REAL_TIME_UPDATE_LATENCY);
    }
  });
  ...
  test('Performance monitoring dashboard loads and functions correctly', async ({ page }) => {
    ...
    expect(hasPerformanceMonitoring, 'Performance monitoring APIs should be available')
      .toBeTruthy();
    ...
    expect(performanceEntries.navigationEntries, 'Navigation timing should be available')
      .toBeGreaterThan(0);
  });
});
...
test.describe('Performance Auditing', () => {
  test('Run comprehensive performance audit', async ({ page }) => {
    ...
    if (auditResults.webVitals.ttfb && auditResults.webVitals.ttfb > PERFORMANCE_BUDGETS.TTFB_GOOD) {
      vitalsScore -= 10;
    }
    ...
    expect(score, 'Overall performance score should meet standards')
      .toBeGreaterThan(70);
    ...
    if (auditResults.webVitals.lcp && auditResults.webVitals.lcp > PERFORMANCE_BUDGETS.LCP_GOOD * 0.8) {

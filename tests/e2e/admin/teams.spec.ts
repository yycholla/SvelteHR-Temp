...
test.describe('Admin Teams Administration Page', () => {
  ...
  test('should display teams and departments data table', async ({ page }) => {
    ...
    // Check table headers
    await expect(page.locator('th')).toContainText(['Department', 'Team Size', 'Department Head', 'Budget', 'Status', 'Actions']);
  ...
  test('should create new department with team structure', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="modal-title"]')).toContainText('Create New Department');

    // Fill in department details
    await page.fill('[data-testid="department-name"]', 'Product Development');
    await page.fill('[data-testid="department-description"]', 'Responsible for product design, development, and innovation');
    ...
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Department created successfully');
  ...
  test('should edit existing department and update team members', async ({ page }) => {
    ...
    await page.fill('[data-testid="department-description"]', 'Updated department description with new responsibilities');
    await page.fill('[data-testid="annual-budget"]', '750000');

    // Add new team member
    await page.click('[data-testid="add-team-member"]');
    const newMemberRow = page.locator('[data-testid="member-row"]').last();
    await newMemberRow.locator('[data-testid="member-select"]').selectOption('jane.smith@postgraphile-hr.com');
    ...
    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Department updated successfully');
  ...
  test('should validate required fields in department creation', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="name-error"]')).toContainText('Department name is required');
    await expect(page.locator('[data-testid="code-error"]')).toContainText('Department code is required');
    await expect(page.locator('[data-testid="description-error"]')).toContainText('Department description is required');
    await expect(page.locator('[data-testid="head-error"]')).toContainText('Department head is required');
    ...
    await expect(page.locator('[data-testid="code-error"]')).toContainText('Department code already exists');
  ...
  test('should transfer department head with proper authorization', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="transfer-message"]')).toContainText('Confirm department head transfer');

    // Should show current and new head details
    await expect(page.locator('[data-testid="current-head"]')).toContainText(currentHead);
    await expect(page.locator('[data-testid="new-head"]')).toContainText('jane.smith@postgraphile-hr.com');

    // Confirm transfer
    await page.click('[data-testid="confirm-transfer"]');

    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Department head transferred successfully');
  ...
  test('should delete department with confirmation and data validation', async ({ page }) => {
    ...
    const departmentRow = page.locator('[data-testid="team-row"]').filter({ hasText: 'Test Department' }).first();
    await departmentRow.locator('[data-testid="delete-team"]').click();

    // Should show deletion confirmation modal
    await expect(page.locator('[data-testid="delete-confirmation"]')).toBeVisible();
    await expect(page.locator('[data-testid="confirmation-message"]')).toContainText('Are you sure you want to delete this department?');

    // Should show impact warning if department has employees
    if (await page.locator('[data-testid="impact-warning"]').isVisible()) {
      await expect(page.locator('[data-testid="impact-warning"]')).toContainText('This department has active employees');
    ...
    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Department deleted successfully');
  ...
  test('should bulk assign employees to departments', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="modal-title"]')).toContainText('Bulk Assign Employees');
    ...
    // Should show success notification
    await expect(page.locator('[data-testid="success-notification"]')).toContainText('Employees assigned successfully');

...
import { login, waitForAuthentication, verifyDashboardRedirect, TEST_USERS, performCompleteLogin } from '../helpers/auth';
...
test.describe('Already Authenticated User Redirect Behavior', () => {
  ...
  test('SHOULD FAIL: authenticated admin accessing login page redirects to admin dashboard', async ({ page }) => {
  ...
  test('SHOULD FAIL: authenticated admin accessing root page redirects to admin dashboard', async ({ page }) => {
    // Authenticate first
    await performCompleteLogin(page, TEST_USERS.admin);
  ...
  test('SHOULD FAIL: authenticated user maintains auth state across page refreshes', async ({ page }) => {
    // Authenticate
    await performCompleteLogin(page, TEST_USERS.admin);
    ...
    const tokenAfterRefresh = await page.evaluate(() => localStorage.getItem('postgraphile-jwt-token'));
  ...
  test('SHOULD FAIL: authenticated user accessing protected page directly works without redirects', async ({ page }) => {
    // Authenticate
    await performCompleteLogin(page, TEST_USERS.admin);
  ...
  test('SHOULD FAIL: multiple browser tabs maintain consistent auth state', async ({ page, context }) => {
    ...
    const secondTabToken = await secondTab.evaluate(() => localStorage.getItem('postgraphile-jwt-token'));
  ...
  test('SHOULD FAIL: auth state cleanup works properly on logout', async ({ page }) => {
    ...
    const tokenAfterLogout = await page.evaluate(() => localStorage.getItem('postgraphile-jwt-token'));

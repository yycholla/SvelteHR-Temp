...
test.describe('Successful Admin Login Flow', () => {
  ...
  test('SHOULD FAIL: admin user logs in and reaches admin dashboard without redirects', async ({ page }) => {
  ...
  test('SHOULD FAIL: admin has access to admin-specific features', async ({ page }) => {
    ...
    const hasAdminElements = await page.locator('.admin, [data-role="admin"], .admin-dashboard').count();

...
impl Guard for RequireRole {
    async fn check(&self, ctx: &Context<'_>) -> Result<()> {
        ...
        if user_ctx.has_role(&self.role) {
            ...
            Err(Error::new(format!("Requires role: {}", self.role))
                .extend_with(|_, e| {
...
impl Guard for RequirePermission {
    async fn check(&self, ctx: &Context<'_>) -> Result<()> {
        ...
        if user_ctx.has_permission(&self.permission) {
            ...
            Err(Error::new(format!("Requires permission: {}", self.permission))
                .extend_with(|_, e| {
                    e.set("code", "FORBIDDEN");
...
impl Guard for RequireAnyRole {
    async fn check(&self, ctx: &Context<'_>) -> Result<()> {
        ...
        Err(Error::new(format!("Requires one of roles: {}", self.roles.join(", ")))
            .extend_with(|_, e| {
                e.set("code", "FORBIDDEN");
...
impl Guard for RequireMinRoleLevel {
    async fn check(&self, ctx: &Context<'_>) -> Result<()> {
        ...
        if user_level >= self.min_level {
            ...
            Err(Error::new(format!("Requires minimum role level: {}", self.min_level))
                .extend_with(|_, e| {
                    e.set("code", "FORBIDDEN");

...
impl EmployeeImportMutations {
    ...
    async fn upload_employee_import(
        ...
        for (idx, result) in reader.records().enumerate() {
            ...
            let record = result.map_err(|e| {
                AppError::Validation(format!("CSV error at row {}: {}", idx + 1, e))
            })?;
            ...
            if rows_to_insert.len() >= 100 {
                import_row::Entity::insert_many(rows_to_insert)
                    .exec(&db)
                    .await?;
    ...
    async fn validate_employee_import(
        ...
        for row in rows {
            ...
            if let (Some(f), Some(l)) = (first_name, last_name) {
                if f.is_empty() {
                    validation_errors.push("First name is empty".to_string());
                }
                if l.is_empty() {
                    validation_errors.push("Last name is empty".to_string());
                }
            ...
            let home_phone = input
                .home_phone_column
                .as_ref()
                .or(input.phone_column.as_ref())
                .and_then(|col| get_val(col));
            if let Some(p) = home_phone {
                parsed_data.insert("home_phone".to_string(), json!(p));
            }

            if let Some(col) = &input.work_phone_column {
                if let Some(p) = get_val(col) {
                    parsed_data.insert("work_phone".to_string(), json!(p));
                }
            }

            if let Some(col) = &input.mobile_phone_column {
                if let Some(p) = get_val(col) {
                    parsed_data.insert("mobile_phone".to_string(), json!(p));
                }
            }

            // 7. Nickname & Social
            if let Some(col) = &input.nickname_column {
                if let Some(n) = get_val(col) {
                    parsed_data.insert("nickname".to_string(), json!(n));
                }
            }
            ...
            if let Some(col) = &input.social_media_release_column {
                if let Some(val) = get_val(col) {
                    let is_agreed =
                        ["yes", "y", "true", "1"].contains(&val.to_lowercase().as_str());
    ...
    async fn commit_employee_import(
        ...
        for row in valid_rows {
            ...
            let birth_date = parsed
                .get("birth_date")
                .and_then(|s| s.as_str())
                .map(|s| NaiveDate::parse_from_str(s, "%Y-%m-%d").unwrap());

            let _role_name = parsed
                .get("role")
                .and_then(|s| s.as_str())
                .unwrap_or("Employee");
            ...
            let social_media_release = parsed
                .get("social_media_release")
            ...
            match user.insert(&db).await {
                ...
                Err(e) => {
                    ...
                    let mut row_active: import_row::ActiveModel = row.into();
                    row_active.status = Set(ImportRowStatus::Error);
                    row_active.validation_errors =
                        Set(Some(json!([format!("Insert Error: {}", e)])));
                    row_active.update(&db).await?;
                }
            }
        }
    ...
    async fn import_employee_with_email(
        ...
        let intuit_client = client_manager.get_client().await.map_err(|e| {
            async_graphql::Error::new(format!("Failed to connect to QuickBooks: {}", e))
        })?;

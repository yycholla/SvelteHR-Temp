...
async fn test_up_migration() {
    ...
    assert!(table_exists(&db, "hr_public", "webhook_subscriptions")
        .await
        .unwrap());
    assert!(table_exists(&db, "hr_public", "webhook_events")
        .await
        .unwrap());
}
...
async fn test_indexes() {
    ...
    assert!(index_exists(&db, "idx_webhook_subscriptions_realm_id")
        .await
        .unwrap());
    assert!(index_exists(&db, "idx_webhook_subscriptions_webhook_id")
        .await
        .unwrap());
    assert!(index_exists(&db, "idx_webhook_events_subscription_id")
        .await
        .unwrap());
    assert!(index_exists(&db, "idx_webhook_events_status")
        .await
        .unwrap());
    assert!(index_exists(&db, "idx_webhook_events_entity")
        .await
        .unwrap());
}
...
async fn test_foreign_key_cascade() {
    ...
    db.execute(Statement::from_string(
        db.get_database_backend(),
        format!(
            "DELETE FROM hr_public.webhook_subscriptions WHERE id = '{}'",
            sub_id
        ),
    ))
    .await

...
async fn setup() -> Result<DatabaseConnection, DbErr> {
    ...
    let _ = db
        .execute(Statement::from_string(
            DbBackend::Postgres,
            "DROP TABLE IF EXISTS hr_public.time_entries CASCADE".to_string(),
        ))
        .await;
    let _ = db
        .execute(Statement::from_string(
            DbBackend::Postgres,
            "DROP TABLE IF EXISTS hr_public.projects CASCADE".to_string(),
        ))
        .await;
    Ok(db)
}
...
async fn test_up_migration_creates_tables() -> Result<(), DbErr> {
    ...
    let result = db
        .query_one(Statement::from_string(
         ...
         AND table_name IN ('projects', 'time_entries')"
                .to_string(),
        ))
        .await?;
...
async fn test_time_entries_columns() -> Result<(), DbErr> {
    ...
    let columns = db
        .query_all(Statement::from_string(
         ...
         WHERE table_schema = 'hr_public' AND table_name = 'time_entries'"
                .to_string(),
        ))
        .await?;

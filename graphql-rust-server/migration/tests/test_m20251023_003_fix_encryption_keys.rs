...
mod tests {
    ...
    async fn test_encryption_keys_foreign_key() {
        ...
        assert_eq!(delete_rule, "CASCADE", "FK should have ON DELETE CASCADE");
    }
    ...
    async fn test_encryption_keys_encrypt_function() {
        ...
        assert!(result.is_some(), "Function encrypt_key_data should exist");
        let row = result.unwrap();

        let return_type: String = row.try_get("", "return_type").unwrap();
        assert_eq!(return_type, "bytea", "encrypt_key_data should return BYTEA");
    ...
    async fn test_encryption_keys_decrypt_function() {
        ...
        assert!(result.is_some(), "Function decrypt_key_data should exist");
        let row = result.unwrap();

        let return_type: String = row.try_get("", "return_type").unwrap();
        assert_eq!(return_type, "bytea", "decrypt_key_data should return BYTEA");

...
async fn test_indexes() {
    ...
    assert!(index_exists(&db, "idx_batch_operation_items_batch_id").await.unwrap());
    assert!(index_exists(&db, "idx_batch_operation_items_status").await.unwrap());
}
...
async fn test_foreign_key_cascade() {
    ...
    db.execute(Statement::from_string(
        db.get_database_backend(),
        format!("DELETE FROM hr_public.batch_operations WHERE id = '{}'", batch_id),
    )).await.unwrap();

...
const ES = SS,
	kS = ({
		...
		const [g, y] = $.useState({ expandedItems: new Map() }),
			...
			A = $.useCallback(
				(D) =>
					!n || !D.action || (D.action.startTime <= n.maximum && D.action.endTime >= n.minimum),
				[n]
			),
			U = $.useCallback(
				(D) => {
				...
				e.map((v, x) =>
					w.jsx(
						'div',
						{
							...
							onMouseDown: (E) =>
								d({ clientX: E.clientX, clientY: E.clientY, offset: v, index: x }),
...
class v1 {
	generatePlaywrightRequestCall(e, n) {
		...
		(['delete', 'get', 'head', 'post', 'put', 'patch'].includes(r) ||
			...
			n && (c.data = n),
			e.headers.length &&
				(c.headers = Object.fromEntries(e.headers.map((p) => [p.name, p.value]))));
		const u = [`'${l}'`];
		return (
...
class S1 {
	generatePlaywrightRequestCall(e, n) {
		...
		(['delete', 'get', 'head', 'post', 'put', 'patch'].includes(u) ||
			...
			n && (l.Data = n),
			e.headers.length &&
				(l.Headers = Object.fromEntries(e.headers.map((g) => [g.name, g.value]))));
		const d = [`"${o}"`];
		return (

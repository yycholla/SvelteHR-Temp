...
export class NPlusOneDetector {
    ...
    const visitor = {
      Field: {
        enter: (node: FieldNode, key: any, parent: any, path: any, ancestors: any[]) => {
          ...
          originalComplexity += this.calculateFieldComplexity(node, isListField, hasNestedSelection);
        },
        leave: () => {
          fieldPath.pop();
        }
  ...
  private isNPlusOnePattern(
    ...
    if (isListField && field.selectionSet && field.selectionSet.selections.length > 0) {
      const hasObjectSelections = field.selectionSet.selections.some(selection =>
  ...
  private isPostGraphileConnectionPattern(field: FieldNode, fieldPath: string[]): boolean {
    ...
    if (fieldName.endsWith('Connection') && field.selectionSet) {
      const hasEdgesWithNestedNodes = field.selectionSet.selections.some(selection =>
        selection.kind === Kind.FIELD &&
        selection.name.value === 'edges' &&
        selection.selectionSet &&
        selection.selectionSet.selections.some(edgeSelection =>
  ...
  private estimateCallCount(
    ...
    const listFieldsInPath = fieldPath.filter(field =>
      field.endsWith('s') || field.endsWith('Connection')
    ).length;
...
const ${batchKey}Loader = new DataLoader(async (keys) => {
  ...
  private calculateImprovementEstimate(pattern: NPlusOnePattern): number {
    const baseImprovement = Math.min(90, (pattern.estimatedCallCount - 1) / pattern.estimatedCallCount * 100);

    // Adjust based on severity and nesting
    switch (pattern.severity) {
      case 'critical': return Math.min(95, baseImprovement + 20);
      case 'high': return Math.min(90, baseImprovement + 15);
      case 'medium': return Math.min(80, baseImprovement + 10);
      default: return baseImprovement;
    }
  }
   ...
   */
  private calculateOptimizedComplexity(originalComplexity: number, patterns: NPlusOnePattern[]): number {
    let reduction = 0;

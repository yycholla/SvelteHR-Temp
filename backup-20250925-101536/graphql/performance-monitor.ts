...
export class GraphQLPerformanceMonitor {
  ...
  recordMetrics(metrics: GraphQLPerformanceMetrics, document?: DocumentNode): void {
    ...
    if (this.config.excludeIntrospectionQueries &&
  ...
  getQueryProfile(operationName?: string, queryHash?: string): QueryPerformanceProfile | undefined {
    ...
    if (operationName) {
      return Array.from(this.queryProfiles.values())
        .find(profile => profile.operationName === operationName);
    }
   ...
   */
  getAllQueryProfiles(sortBy: 'execution_time' | 'complexity' | 'frequency' | 'error_rate' = 'execution_time'): QueryPerformanceProfile[] {
  ...
  getCurrentMetrics(): {
    ...
    return {
      ...
      slowQueriesCount: profiles.filter(p =>
        p.averageExecutionTime > this.config.thresholds.warningTime
      ).length,
      complexQueriesCount: profiles.filter(p =>
        p.complexityStats.average > this.config.thresholds.complexityWarning
      ).length
    };
  }
  ...
  private updateQueryProfile(metrics: GraphQLPerformanceMetrics & { timestamp: number; queryHash: string }): void {
    ...
    if (existing) {
      ...
      const updatedProfile: QueryPerformanceProfile = {
        ...
        complexityStats: {
          average: (existing.complexityStats.average * existing.executionCount + metrics.complexity) / newCount,
      ...
      const newProfile: QueryPerformanceProfile = {
        ...
        percentiles: { p50: metrics.executionTime, p95: metrics.executionTime, p99: metrics.executionTime },
        complexityStats: {
  ...
  private checkPerformanceAlerts(metrics: GraphQLPerformanceMetrics & { timestamp: number; queryHash: string }): void {
    ...
    if (metrics.executionTime > this.config.thresholds.criticalTime) {
      alerts.push(this.createAlert('critical', 'slow_query', metrics,
      ...
      alerts.push(this.createAlert('warning', 'slow_query', metrics,
    ...
    if (metrics.complexity > this.config.thresholds.complexityCritical) {
      alerts.push(this.createAlert('critical', 'high_complexity', metrics,
      ...
      alerts.push(this.createAlert('warning', 'high_complexity', metrics,
    ...
    if (metrics.cacheHitRatio < (this.config.thresholds.cacheHitRateWarning / 100)) {
      alerts.push(this.createAlert('warning', 'low_cache_hit', metrics,
  ...
  private getMetricsForQuery(queryHash: string, limit?: number): (GraphQLPerformanceMetrics & { timestamp: number })[] {
    const filtered = this.rawMetrics.filter(m => (m as any).queryHash === queryHash);
    return limit ? filtered.slice(-limit) : filtered;
  }

  private getMetricsInRange(start: Date, end: Date): (GraphQLPerformanceMetrics & { timestamp: number })[] {
    return this.rawMetrics.filter(m =>
      m.timestamp >= start.getTime() && m.timestamp <= end.getTime()
    );
  }

  private getRecentMetrics(timeframe: number): (GraphQLPerformanceMetrics & { timestamp: number })[] {
    const cutoff = Date.now() - timeframe;
  ...
  private calculateOverallCacheHitRate(profiles: QueryPerformanceProfile[]): number {
    ...
    const totalRequests = profiles.reduce((sum, p) => sum + p.cacheStats.hits + p.cacheStats.misses, 0);
  ...
  private identifyCacheOptimizations(profiles: QueryPerformanceProfile[]): string[] {
    ...
    const frequentSlowQueries = profiles.filter(p =>
      p.executionCount > 10 &&
      p.cacheStats.hitRate < 0.5 &&
      p.averageExecutionTime > 100
    );

    if (frequentSlowQueries.length > 0) {
      opportunities.push(`${frequentSlowQueries.length} frequently executed queries have low cache hit rates`);
    }

    // Find queries that could benefit from longer cache TTL
    const shortCachedQueries = profiles.filter(p =>
      p.cacheStats.hitRate > 0.8 && p.executionCount > 50
    );

    if (shortCachedQueries.length > 0) {
      opportunities.push(`${shortCachedQueries.length} queries have high cache hit rates and could benefit from extended TTL`);
    }

    return opportunities;
  }

  private generateRecommendations(profiles: QueryPerformanceProfile[], metrics: GraphQLPerformanceMetrics[]): PerformanceRecommendation[] {
    const recommendations: PerformanceRecommendation[] = [];

    // Query optimization recommendations
    const slowQueries = profiles.filter(p => p.averageExecutionTime > this.config.thresholds.warningTime);
    ...
    const lowCacheHitRate = this.calculateOverallCacheHitRate(profiles) < this.config.thresholds.cacheHitRateWarning;
    if (lowCacheHitRate) {
      recommendations.push({
        category: 'caching',
        priority: 'medium',
    ...
    const highComplexityQueries = profiles.filter(p => p.complexityStats.average > this.config.thresholds.complexityWarning);

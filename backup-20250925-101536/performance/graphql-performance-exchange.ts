...
export function createPerformanceExchange(
  ...
  if (!mergedConfig.enabled) {
    ...
    return ({ forward }) => (ops$) => forward(ops$);
  }
      ...
      tap((result: OperationResult) => {
        ...
        if (mergedConfig.trackAllOperations) {
          ...
          const metadata = {
            ...
            variablesSize: context.variables ?
              JSON.stringify(context.variables).length : 0,
            responseSize: result.data ?
              JSON.stringify(result.data).length : 0
          };
          ...
          const performanceMetric: Omit<PerformanceMetric, 'id' | 'timestamp'> = {
            ...
            status: result.error ? 'error' :
                   duration > mergedConfig.slowQueryThreshold ? 'warning' : 'success',
            metadata,
            tags: [
              'graphql',
              'urql',
              ...
              ...(result.error ? ['error'] : []),
              ...(context.complexity && context.complexity > mergedConfig.complexityThreshold ? ['complex'] : [])
            ]
          };
        ...
        if (mergedConfig.enableComplexityAnalysis &&
            context.complexity &&
  ...
  async testOperation<T>(
    ...
    for (let i = 0; i < iterations; i++) {
      try {
        ...
        if (logResults) {
          const status = duration > maxDuration ? '❌' :
                        duration > maxDuration * 0.8 ? '⚠️' : '✅';
    ...
    if (testResult.averageDuration > maxDuration) {
      console.warn(`⚠️ Average duration (${testResult.averageDuration}ms) exceeds target (${maxDuration}ms)`);
    }

...
class ClientPerformanceMonitor {
  ...
  recordMetric(metric: Omit<PerformanceMetric, 'id' | 'timestamp'>): void {
    ...
    if (fullMetric.status === 'warning' || fullMetric.status === 'error') {
      console.warn(`🐌 Performance ${fullMetric.status}: ${fullMetric.name} took ${fullMetric.duration}ms`);
    }
  }
   ...
   */
  trackPageLoad(pageName: string, loadTime: number, navigationTiming?: PerformanceNavigationTiming): void {
  ...
  private checkPerformanceBudget(metric: PerformanceMetric): void {
    ...
    if (budgetExceeded) {
      const severity = metric.duration > budget * 2 ? 'critical' :
                      metric.duration > budget * 1.5 ? 'high' : 'medium';
    ...
    return derived(
      ...
      ([metrics, memory]) => {
        ...
        const performanceScore = metrics.length > 0 ?
          Math.round((successfulMetrics / metrics.length) * 100) : 100;

        return {
          totalMetrics: metrics.length,
          averagePageLoad: pageLoadMetrics.length > 0 ?
            pageLoadMetrics.reduce((sum, m) => sum + m.duration, 0) / pageLoadMetrics.length : 0,
          averageGraphQLResponse: graphqlMetrics.length > 0 ?
            graphqlMetrics.reduce((sum, m) => sum + m.duration, 0) / graphqlMetrics.length : 0,
...
export function trackExport(type: string, recordCount: number, duration: number, fileSize?: number): void {

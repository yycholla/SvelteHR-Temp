...
async function migrateAuthStore() {
	...
	const files = await glob('src/**/*.{svelte,ts}', {
		ignore: ['src/lib/stores/auth.ts', 'src/lib/stores/auth.svelte.ts']
	});
	...
	for (const file of files) {
		...
		content = content.replace(
			/(?<!function\s|[\w\.])hasMinimumRoleLevel\(/g,
			'auth.hasMinimumRoleLevel('
		);

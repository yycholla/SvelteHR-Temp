...
<script lang="ts">
	...
	import { Calendar, Check, X, Clock, AlertCircle, Users, TrendingUp, FileText, Search, Filter } from 'lucide-svelte';
...
<svelte:head>
	...
	<meta name="description" content="Review and manage pending leave requests from your team members" />
</svelte:head>
										...
										<div class="flex items-start space-x-4 flex-1">
											...
											<div class="flex-1 space-y-2">
												<div class="flex items-center gap-3">
													...
													>
														{leaveTypeOptions.find(t => t.value === request.leaveType)?.label || request.leaveType}
													</Badge>
												</div>
...
<Dialog bind:open={showApprovalModal}>
	<DialogContent>
			...
			<div class="space-y-4">
				<div class="bg-gray-50 p-4 rounded-lg">
					...
					<div class="text-sm space-y-1">
						...
						<p><strong>Leave Type:</strong> {leaveTypeOptions.find(t => t.value === currentRequest.leaveType)?.label}</p>
						<p><strong>Dates:</strong> {formatDateRange(currentRequest.startDate, currentRequest.endDate)}</p>
		...
		<DialogFooter>
			<Button variant="outline" onclick={() => { showApprovalModal = false; }}>Cancel</Button>
...
<Dialog bind:open={showDenialModal}>
	<DialogContent>
			...
			<div class="space-y-4">
				...
				<div>
					...
					{#if !managerComments.trim()}
						<p class="text-sm text-red-600 mt-1">A reason for denial is required</p>
					{/if}
				</div>
			</div>
		{/if}
		...
		<DialogFooter>
			<Button variant="outline" onclick={() => { showDenialModal = false; }}>Cancel</Button>
			<Button
				variant="destructive"

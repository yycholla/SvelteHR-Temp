<script lang="ts">
	...
	function isCompleted(contentId: string) {
		return progressData.some((p: any) => p.trainingContentId === contentId && p.status === 'COMPLETED');
	}
...
<div class="min-h-screen bg-background flex flex-col">
	...
	<header class="border-b bg-card px-4 py-3 sticky top-0 z-10">
		<div class="container mx-auto max-w-7xl flex items-center justify-between gap-4">
			...
			<Button variant="ghost" size="icon" class="lg:hidden" onclick={() => mobileMenuOpen = !mobileMenuOpen}>
				{#if mobileMenuOpen}
					<X class="h-5 w-5" />
				{:else}
					<Menu class="h-5 w-5" />
	...
	<div class="flex-1 container mx-auto max-w-7xl p-4 lg:p-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
		...
		{#if mobileMenuOpen}
			...
			></button>
			<div class="fixed inset-y-0 right-0 z-50 w-3/4 max-w-sm border-l bg-background shadow-lg p-4 lg:hidden transition-transform duration-300 ease-in-out overflow-y-auto">
		...
		<div class="lg:col-span-3">
			{#if activeContent}
				...
				<Card.Root class="min-h-[600px] flex flex-col">
					<Card.Header>
						<div class="flex items-center gap-2 text-muted-foreground text-sm mb-2 uppercase tracking-wider font-medium">
							<Icon class="h-4 w-4" />
							{activeContent.type}
						</div>
					...
					<Card.Content class="flex-1 space-y-6">
							...
							<div class="aspect-video w-full bg-black rounded-lg overflow-hidden shadow-sm">
								...
								{:else}
									...
									<div class="flex items-center justify-center h-full">
										<a href={activeContent.data} target="_blank" rel="noreferrer" class="flex flex-col items-center gap-4 text-white hover:scale-105 transition-transform">
						...
						<p class="text-sm text-muted-foreground text-center mt-4">
							Source: <a href={activeContent.data} target="_blank" rel="noreferrer" class="underline text-primary hover:text-primary/80">{activeContent.data}</a>
						</p>
						{:else if activeContent.type === 'IMAGE'}
							<div class="rounded-lg overflow-hidden border bg-muted/10">
								<img src={activeContent.data} alt={activeContent.title} class="w-full h-auto max-h-[600px] object-contain mx-auto" />
							</div>
						{:else if activeContent.type === 'URL' || activeContent.type === 'DOCUMENT'}
							<div class="flex flex-col items-center justify-center p-12 border-2 border-dashed rounded-xl bg-muted/5 text-center space-y-4">
								...
								<Button href={activeContent.data} target="_blank" rel="noreferrer" variant="outline" class="gap-2">
					...
					<Card.Footer class="border-t p-6 flex flex-col sm:flex-row justify-between gap-4 items-center bg-muted/5">
						...
						>
							...
							<Button type="submit" variant={isCompleted(activeContent.id) ? "secondary" : "default"} class="w-full sm:w-auto min-w-[200px]">

...
export const load: PageServerLoad = async ({ url, locals, fetch, cookies }) => {
	...
	requireAuth({ url, locals, fetch } as any, { minTier: AccessTier.ALL });
	...
	try {
		...
		// Create GraphQL client and query activity logs
		...
		if (response.errors && response.errors.length > 0) {
			logger.warn('[Audit] GraphQL errors fetching activity logs', {

	...
	<div class="mx-auto max-w-7xl space-y-8">
		...
		<div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
			...
			<div class="space-y-6 lg:col-span-2">
				...
				<div class="flex flex-wrap items-center gap-4 rounded-lg border bg-card p-4 text-card-foreground shadow-sm">
					<div class="flex items-center gap-2">
					...
					<div class="flex items-center gap-2">
						...
						<div class="flex items-center gap-1.5">
							{#if task.taskType?.colorCode}
								<div class="h-3 w-3 rounded-full" style="background-color: {task.taskType.colorCode}"></div>
							{:else}
							...
							{#each task.subtasks as subtask (subtask.id)}
								<div class="group flex items-start space-x-3">
									<button
										...
										onclick={() => {
											const newStatus = (subtask.status === 'DONE' || subtask.status === 'COMPLETED') ? 'TODO' : 'DONE';
											handleStatusChange(subtask.id, newStatus as TaskStatus);
										}}
										aria-label={subtask.status === 'DONE' || subtask.status === 'COMPLETED' ? "Mark as incomplete" : "Mark as complete"}
									>
							...
							<div class="space-y-2">
								{#each task.linkedResources as resource}
									{#if resource.resourceType === 'document'}
										<div class="flex items-center gap-3 p-3 rounded-md border bg-secondary/20 hover:bg-secondary/40 transition-colors">
											<div class="h-10 w-10 rounded bg-blue-100 flex items-center justify-center text-blue-600">
												<FileText class="h-5 w-5" />
											</div>
											<div class="flex flex-col flex-1 min-w-0">
												<span class="text-sm font-medium truncate">{resource.resourceTitle}</span>
												<span class="text-xs text-muted-foreground">Document • {formatDate(resource.createdAt.toString())}</span>
											</div>
											<Button variant="ghost" size="icon" href={`/dashboard/documents/${resource.resourceId}`} title="View Document">
												<Link2 class="h-4 w-4" />
											</Button>
										</div>
									{/if}
							...
							{#if data.auditTrail && data.auditTrail.length > 0}
								...
								{#each data.auditTrail as entry}
									<div class="relative py-4 pl-12">
										<div class="absolute left-2 top-5 z-10 flex h-6 w-6 items-center justify-center rounded-full border-2 border-border bg-background">
										...
										<div class="text-sm text-muted-foreground">
											...
											{entry.action}
											<div class="mt-1 text-xs">{formatDistanceToNow(new Date(entry.createdAt))} ago</div>
										</div>
									</div>
								{/each}
							{:else}
				...
				<div class="rounded-lg border bg-card text-card-foreground shadow-sm">
					...
					<div class="p-6">
						<div class="flex flex-wrap gap-2">
							{#if task.taskType}
								<span class="inline-flex items-center rounded-full border border-transparent bg-secondary px-2.5 py-0.5 text-xs font-semibold text-secondary-foreground transition-colors hover:bg-secondary/80">
									{task.taskType.name}
								</span>
							{/if}
							
							{#each tags as tag}
								<span class="inline-flex items-center rounded-full border border-transparent bg-primary/10 text-primary px-2.5 py-0.5 text-xs font-semibold transition-colors">
									{tag}
									<button class="ml-1 hover:text-destructive focus:outline-none" onclick={() => removeTag(tag)}>
										<X class="h-3 w-3" />
									</button>
								</span>
							{/each}

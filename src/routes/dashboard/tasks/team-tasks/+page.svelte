	...
	<Card.Root>
		<Card.Content class="pt-6">
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
				...
				<!-- Assignee Filter -->
				<Select.Root selected={{ value: selectedAssignee }} onSelectedChange={(v) => { selectedAssignee = v?.value || ''; updateFilters(); }}>
					<Select.Trigger>
						<Select.Value placeholder="All Team Members" />
					</Select.Trigger>
					<Select.Content>
						<Select.Item value="">All Team Members</Select.Item>
						{#each data.teamMembers as member}
							<Select.Item value={member.id}>{member.displayName}</Select.Item>
						{/each}
					</Select.Content>
				</Select.Root>

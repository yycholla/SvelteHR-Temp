  ...
  try {
    ...
    switch (endpoint) {
      ...
      case 'health':
        ...
        if (healthStats.errorRate > 0.05) { // 5%
          healthStatus.status = 'degraded';
          healthStatus.score -= 25;
          healthStatus.issues.push('High error rate');
        }

        if (healthStats.slowRequestsCount > healthStats.totalRequests * 0.1) { // 10%
          healthStatus.status = 'degraded';
        ...
        return json({
          ...
          data: {
            ...
            summary: {
              ...
              averageActualDuration: testResults.reduce((sum, r) => sum + r.actualDuration, 0) / iterations,

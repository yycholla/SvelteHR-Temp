  ...
  try {
    ...
    if (metricType) {
      metrics = metrics.filter(m => 
        m.tags.metric_type === metricType || m.name.includes(metricType)
      );
    }
...
function logCriticalMetrics(metrics: PerformanceMetric[]) {
  metrics.forEach(metric => {
    ...
    if (metric.name === 'api_response' && parseInt(metric.tags.status) >= 400) {
      console.error(`API error: ${metric.tags.method} ${metric.tags.endpoint} - ${metric.tags.status}`);
    }
  });
}
...
function getSlowestOperations(metrics: PerformanceMetric[]) {
  const operationMetrics = metrics.filter(m => 
    m.name === 'navigation_timing' || 
    m.name === 'graphql_query' || 
    m.name === 'api_response'
  );

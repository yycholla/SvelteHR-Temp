	...
	async ({ cookies }) => {
		...
		if (token) {
			...
			try {
				...
				return {
					initialDepartments: departmentsResponse.status === 'fulfilled' ? departmentsResponse.value : { departments: [], count: 0 },
					error: departmentsResponse.status === 'rejected' ? departmentsResponse.reason?.message : null
				};
			} catch (error) {

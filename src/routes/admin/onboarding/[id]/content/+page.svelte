<script lang="ts">
	...
	$effect(() => {
		if (data?.contents) {
			const newItems = [...data.contents].sort((a: any, b: any) => a.sequenceOrder - b.sequenceOrder);
	...
	async function handleSaveBlock() {
		...
		if (selectedBlock.formTemplateId) formData.append('formTemplateId', selectedBlock.formTemplateId);
		else formData.append('formTemplateId', ''); 

		if (selectedBlock.inlineFormElements) formData.append('inlineFormElements', JSON.stringify(selectedBlock.inlineFormElements));
		if (selectedBlock.textContent) { /* handled implicitly by update action if present */ }
...
<div class="h-screen flex flex-col bg-background overflow-hidden">
	<Tooltip.Provider>
		<!-- Top App Bar -->
		<header class="h-14 border-b flex items-center px-4 bg-background shrink-0 z-20 justify-between">
	...
	<div class="flex-1 flex overflow-hidden">
		...
		<div class="flex-1 overflow-hidden relative">
			{#if selectedBlock}
				...
				/>
			{:else}
				<div class="h-full flex flex-col items-center justify-center text-muted-foreground bg-muted/10">

...
export const load: PageServerLoad = async (event) => {
	...
	try {
		const [moduleResult, formsResult] = await Promise.all([
			graphqlClient.query<{ onboardingModule?: OnboardingModule | null }>(GET_ONBOARDING_MODULE_QUERY, { id: moduleId }),
			graphqlClient.query<{ onboardingFormsByModule?: OnboardingForm[] }>(GET_FORMS_BY_MODULE, { onboardingModuleId: moduleId })
		]);
...
export const actions: Actions = {
	createForm: async (event) => {
		...
		try {
			const graphqlClient = createGraphQLClient(event);
			const formsResult = await graphqlClient.query<{ onboardingFormsByModule?: unknown[] }>(GET_FORMS_BY_MODULE, { onboardingModuleId: params.id });
			const sequenceOrder = formsResult?.onboardingFormsByModule?.length || 0;
			const input = { onboardingModuleId: params.id, title, description, sequenceOrder, isRequired };
			const result = await graphqlClient.mutate<{ createOnboardingForm: { id: string } }>(CREATE_ONBOARDING_FORM, { input });
			return { success: true, form: result.createOnboardingForm, formId: result.createOnboardingForm.id };
		} catch (err) {
	...
	reorderForms: async (event) => {
		...
		try {
			const graphqlClient = createGraphQLClient(event);
			await graphqlClient.mutate(REORDER_ONBOARDING_FORMS, { onboardingModuleId: params.id, formIds });
		} catch (err) {

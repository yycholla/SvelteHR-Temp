<script lang="ts">
	...
	import { SyncStatusCards, ActionableErrorDisplay } from '$lib/components/integrations';
	import {
	...
	let lastSyncResult = $state<SyncResponse | null>(null);
	...
	let selectedConflictStrategy = $state<ConflictStrategy>('LAST_WRITE_WINS');
	...
	async function confirmBidirectionalSync() {
		...
		try {
			if (entityType === 'ALL') {
				...
				const employeeResult = await syncEmployeesBidirectional(
					'INCREMENTAL',
					selectedConflictStrategy
				);
				if (isSyncSuccessful(employeeResult)) {
					combinedResult.pushed_count += employeeResult.pushed_count;
					combinedResult.pulled_count += employeeResult.pulled_count;
					combinedResult.conflicts_count += employeeResult.conflicts_count;
					allErrors.push(...employeeResult.errors);
				} else {
					allErrors.push(
						'error' in employeeResult ? employeeResult.error : 'Failed to sync employees'
					);
				}

				// Sync departments
				const deptResult = await syncDepartmentsBidirectional(
					'INCREMENTAL',
					selectedConflictStrategy
				);
				if (isSyncSuccessful(deptResult)) {
					combinedResult.pushed_count += deptResult.pushed_count;
					combinedResult.pulled_count += deptResult.pulled_count;
					combinedResult.conflicts_count += deptResult.conflicts_count;
					allErrors.push(...deptResult.errors);
				} else {
					allErrors.push('error' in deptResult ? deptResult.error : 'Failed to sync departments');
				}

				combinedResult.completed_at = new Date().toISOString();
				combinedResult.errors = allErrors;
				combinedResult.success = allErrors.length === 0;
				combinedResult.status = allErrors.length > 0 ? 'COMPLETED_WITH_ERRORS' : 'COMPLETED';
				combinedResult.message = `Synced employees and departments: ${combinedResult.pushed_count} pushed, ${combinedResult.pulled_count} pulled, ${combinedResult.conflicts_count} conflicts resolved`;

				lastSyncResult = combinedResult;
				syncErrors = allErrors;

				if (allErrors.length === 0) {
					showSuccess(combinedResult.message);
				} else {
					errorStore.add({
						message: `Sync completed with ${allErrors.length} error(s)`,
						type: 'error'
					});
				}
				...
				const result =
					entityType === 'EMPLOYEE'
						? await syncEmployeesBidirectional('INCREMENTAL', selectedConflictStrategy)
						: await syncDepartmentsBidirectional('INCREMENTAL', selectedConflictStrategy);
			...
			const errorMsg =
				error instanceof Error ? error.message : 'Failed to perform bidirectional sync';
			syncErrors = [errorMsg];
			errorStore.add({
				message: errorMsg,
				type: 'error'
	...
	function handleRetrySync() {
		// Re-trigger the last sync if we know what it was
		if (lastSyncResult) {
			const entityType = lastSyncResult.entity_type === 'EMPLOYEE' ? 'EMPLOYEE' : 'DEPARTMENT';
			openBidirectionalSyncDialog(entityType);
		}
	}
</script>
...
{#if data.intuitConnected}
	<div class="flex flex-col h-full overflow-hidden bg-background">
		...
		<div class="flex-1 overflow-auto bg-muted/5 p-6 space-y-6">
			<!-- Metrics Overview -->
			<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
				<div class="p-6 border rounded-lg bg-background flex flex-col justify-between h-32">
			...
			<!-- Sync Status Cards -->
			<SyncStatusCards syncResult={lastSyncResult} loading={syncingBidirectional} />
			...
			{#if syncErrors.length > 0}
				<ActionableErrorDisplay
					...
					onDismiss={(index) => {
						syncErrors = syncErrors.filter((_, i) => i !== index);
					}}
				/>
			{/if}
			...
			{#if perms.canTriggerBidirectionalSync}
				<div class="border rounded-lg bg-background p-6">
					...
					<div class="mt-4 text-xs text-muted-foreground bg-muted/50 p-3 rounded border">
						<strong>Note:</strong> Automatically detects changes on both sides and resolves
						conflicts using the {selectedConflictStrategy.replace('_', ' ').toLowerCase()} strategy.
					</div>
				</div>
			{/if}
			...
			<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
				...
				<div class="space-y-2">
					...
					<a
						...
						class="block p-3 rounded-lg border bg-background hover:bg-accent/50 transition-colors"
					>
				...
				{#if pendingBidirectionalSync}
					...
					<div class="p-3 bg-blue-50 border border-blue-200 rounded text-sm">
						<p class="text-blue-900">
							<strong>Two-way sync</strong> detects changes on both sides and automatically resolves
							conflicts using
							<strong>{selectedConflictStrategy.replace('_', ' ').toLowerCase()}</strong> strategy.
						</p>
						{#if pendingBidirectionalSync === 'ALL'}

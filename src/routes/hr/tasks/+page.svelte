					...
					<div class="border border-gray-200 rounded-lg p-4 hover:bg-gray-50 transition-colors">
						<div class="flex items-start justify-between">
							<div class="flex-1 min-w-0">
								...
								{#if task.description}
									<p class="text-sm text-gray-600 mb-2 truncate">{task.description}</p>
								{/if}
							...
							<div class="flex items-center gap-2 ml-4">
								<Button variant="ghost" size="sm" onclick={() => handleViewTask(task)} title="View task">
									<Eye class="w-4 h-4" />
								</Button>
								<Button variant="ghost" size="sm" onclick={() => handleEditTask(task)} title="Edit task">
									<Edit class="w-4 h-4" />
								</Button>
								<Button variant="ghost" size="sm" onclick={() => handleDeleteTask(task)} title="Delete task" class="text-red-600 hover:text-red-700">

<script lang="ts">
	...
	const { form, errors, constraints, enhance, submitting, message } = superForm(data.form, {
		validators: data.template
			? zodClient(createFormSchema(data.template.template_schema))
			: undefined,
		resetForm: false,
		invalidateAll: true,
		onResult: ({ result }) => {
...
<div class="create-leave-request-page">
	...
	<div class="page-header mb-8 flex items-center gap-4">
		...
		<div class="flex items-center gap-3">
			...
			<div>
				<h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Request Time Off</h1>
				<p class="text-gray-600 dark:text-gray-400">Submit a new leave request for approval</p>
			</div>
		</div>
	</div>
	...
	{#if $form.startDate && $form.endDate && leaveDuration}
		<div
			class="duration-display mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4 dark:border-blue-800 dark:bg-blue-900/20"
		>
				...
				<div class="grid grid-cols-1 gap-6 md:grid-cols-2">
					<div>
						...
						<select
							...
							required
							class="w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-none"
						>
						...
						{#if $errors.leaveType}
							<p class="mt-1 text-sm text-destructive">{$errors.leaveType}</p>
						{/if}
					</div>
					...
					<div>
						...
						/>
						{#if $errors.startDate}
							<p class="mt-1 text-sm text-destructive">{$errors.startDate}</p>
						{/if}
					</div>
					...
					<div class="md:col-span-2">
						...
						<textarea
							...
							required
							rows="4"
							class="resize-vertical w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm shadow-sm focus-visible:ring-1 focus-visible:ring-ring focus-visible:outline-none"

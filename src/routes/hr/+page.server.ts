...
export const load: PageServerLoad = async ({ cookies }) => {
  ...
  try {
    ...
    const employees = employeeData.status === 'fulfilled' 
      ? employeeData.value
      : { employees: [], departments: [], roles: [], pagination: { totalCount: 0 } };
    
    // Process task data  
    const tasks = taskData.status === 'fulfilled'
      ? taskData.value
      : { tasks: [], employees: [], totalCount: 0 };
    
    // Process announcements
    const announcements = announcementData.status === 'fulfilled'
    ...
    const leaves = leaveData.status === 'fulfilled'
      ? leaveData.value 

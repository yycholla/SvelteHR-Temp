<script lang="ts">
	...
	$effect(() => {
		console.log('🔍 Frontend employee data check:', {
			...
			firstEmployeeValues: employeesData.data?.[0]
				? Object.entries(employeesData.data[0]).slice(0, 10)
				: []
		});
	});
...
<div class="container mx-auto px-4 py-8">
	<div class="mb-8">
		<h1 class="mb-2 text-3xl font-bold text-gray-900 dark:text-white">Employee Management</h1>
	...
	<div class="hr-employees-content">
		...
		{#if departmentsData.length > 0}
			<div class="mb-8 rounded-lg bg-white p-6 shadow dark:bg-gray-800">
				...
				<DepartmentChart departments={departmentsData} showPercentage={true} showProgress={true} />
			</div>
		{/if}
		...
		<div class="rounded-lg bg-white p-6 shadow dark:bg-gray-800">
			<div class="mb-6 flex items-center justify-between">
				...
				<Button onclick={handleAddEmployee} class="flex items-center gap-2">
					<Plus class="h-4 w-4" />
					Add Employee
				</Button>
			</div>
			...
			<!-- Search and Filters -->
			<div class="mb-6 flex flex-col gap-4 rounded-lg bg-gray-50 p-4 sm:flex-row dark:bg-gray-700">
				<!-- Search Input -->

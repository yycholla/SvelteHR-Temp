<script lang="ts">
  ...
  import { departmentService, departments, isLoadingDepartments, departmentError } from '$lib/services/departmentService';
  ...
  if (showActions && ($currentUser && (hasPermission('department:update') || hasPermission('department:delete')))) {
    columns.push({
      key: 'actions',
      label: 'Actions',
      sortable: false,

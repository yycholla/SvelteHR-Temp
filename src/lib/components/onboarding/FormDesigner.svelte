<script lang="ts">
	...
	function addElement(type: FormFieldType | 'TEXT_BLOCK' | 'HEADER' | 'DIVIDER' | 'QUOTE' | 'MEDIA_IMAGE' | 'MEDIA_VIDEO' | 'MEDIA_DOCUMENT') {
...
<div class="flex h-full bg-muted/5 group/designer overflow-hidden">
	...
	<div class="w-16 bg-background border-r flex flex-col items-center py-4 gap-4 shrink-0 z-20">
		{#each toolGroups as group}
			<HoverCard.Root openDelay={0} closeDelay={100}>
				...
				<HoverCard.Content class="w-64 p-0 overflow-hidden" side="right" align="start" sideOffset={12}>
					...
					<div class="p-2 grid gap-1">
						{#each group.tools as tool}
							...
							>
								<div class="h-8 w-8 rounded-sm bg-background border flex items-center justify-center text-muted-foreground group-hover/btn:border-primary/20 group-hover/btn:text-primary transition-colors">
		...
		<Tooltip.Root>
			<Tooltip.Trigger class="focus:outline-none">
				<div tabindex="0" role="button">
					<Badge variant="secondary" class="h-6 w-6 p-0 flex items-center justify-center rounded-full text-[10px]">
						{elements.length}
					</Badge>
				</div>
			</Tooltip.Trigger>
	...
	<div class="flex-1 flex flex-col relative min-w-0">
		<!-- Canvas Toolbar -->
		<div class="h-12 border-b bg-background/50 backdrop-blur flex items-center justify-between px-4 shrink-0">
		...
		<div class="flex-1 overflow-y-auto bg-muted/20 p-8 flex justify-center">
			<div class="w-full max-w-3xl bg-background rounded-xl border shadow-sm min-h-[800px] h-fit flex flex-col transition-all duration-300 {isPreviewMode ? 'ring-4 ring-offset-4 ring-muted' : ''}">
				...
				<div class="px-12 pb-12 flex-1">
					{#if elements.length === 0}
						<div class="h-full flex flex-col items-center justify-center text-muted-foreground opacity-40 py-20">
							<div class="bg-muted p-6 rounded-full mb-4">
								<Layout class="h-10 w-10" />
							</div>
							<p class="text-lg font-medium">Start Building</p>
							<p class="text-sm max-w-[200px] text-center mt-2">Hover over the tools on the left to add elements to your form.</p>
						</div>
					{:else}
						<div 
							...
							use:dndzone={{ 
								items: dndItems, 
								flipDurationMs: 200, 
								dropTargetStyle: { outline: '2px solid hsl(var(--primary))', borderRadius: '0.5rem', opacity: '0.5' },
								...
								<div animate:flip={{ duration: 200 }} class="relative group/item">
									...
									<div 
										...
										{!isPreviewMode && selectedIndex === index 
											...
											: !isPreviewMode ? 'hover:bg-accent/5 border border-transparent hover:border-border/50' : ''}"
										onclick={(e) => {
											if (!isPreviewMode) {
												e.stopPropagation();
												selectedIndex = index;
										...
										role="button"
										tabindex="0"
										onkeydown={(e) => e.key === 'Enter' && !isPreviewMode && (selectedIndex = index)}
									>
										<!-- Drag Handle (Left) -->
										{#if !isPreviewMode}
											<div class="absolute -left-8 top-1/2 -translate-y-1/2 p-1.5 rounded cursor-move text-muted-foreground/40 hover:text-foreground opacity-0 group-hover/item:opacity-100 transition-opacity">
												<GripVertical class="h-4 w-4" />
											</div>
										{/if}

										<div class="p-4 pointer-events-none"> <!-- Disable interaction inside preview to prevent drag issues -->
											<FormElementPreview
												element={item}
												{index}
	...
	{#if !isPreviewMode && selectedIndex !== null && elements[selectedIndex]}
		<div class="w-80 bg-background border-l flex flex-col shrink-0 z-10 transition-all duration-300 slide-in-from-right-4">

<script lang="ts">
  ...
  function getStatusVariant(status: string): 'default' | 'secondary' | 'success' | 'warning' | 'danger' {
    switch (status) {
      case 'PreHire': return 'secondary';
...
<div class="onboarding-dashboard">
  ...
  <div class="stats-grid">
    <Card padding="md" class="stat-card">
      <div class="stat-content">
        ...
        <div class="stat-icon active">
          <i class="icon-user-check w-6 h-6"></i>
        </div>
      </div>
    </Card>
    ...
    <Card padding="md" class="stat-card">
      <div class="stat-content">
        ...
        <div class="stat-icon completed">
          <i class="icon-check-circle w-6 h-6"></i>
        </div>
      </div>
    </Card>
      ...
      <div class="stat-content">
        <div class="stat-value">
          {$activeOnboardingInstances.length > 0 
            ? Math.round($activeOnboardingInstances.reduce((acc, curr) => acc + curr.completion_percentage, 0) / $activeOnboardingInstances.length)
            : 0}%
        </div>
        <div class="stat-label">Average Progress</div>
        <div class="stat-icon progress">
          <i class="icon-trending-up w-6 h-6"></i>
        </div>
      </div>
    </Card>
    ...
    <Card padding="md" class="stat-card">
      <div class="stat-content">
        <div class="stat-value">
          {$activeOnboardingInstances.filter(i => 
            i.expected_completion_date && new Date(i.expected_completion_date) < new Date()
          ).length}
        </div>
        <div class="stat-label">Overdue</div>
        <div class="stat-icon overdue">
          <i class="icon-alert-triangle w-6 h-6"></i>
        </div>
      </div>
    </Card>
  </div>
    ...
    {:else if currentTabData.length === 0}
      <Card padding="lg" class="empty-state">
        <div class="empty-content">
          <i class="icon-users text-gray-400 w-12 h-12 mx-auto"></i>
          <h3 class="mt-4 text-lg font-medium text-gray-900">
            {selectedTab === 'active' ? 'No active onboarding' : 
             selectedTab === 'completed' ? 'No completed onboarding' : 'No onboarding instances found'}
          </h3>
          <p class="mt-2 text-sm text-gray-600">
            {selectedTab === 'active' ? 'There are no employees currently going through onboarding.' :
             selectedTab === 'completed' ? 'No employees have completed onboarding recently.' :
...
<style lang="postcss">
  ...
  .stat-icon {
    @apply absolute top-0 right-0 p-3 rounded-lg;
  }
  ...
  /* Card Actions */
  .card-actions {
    @apply flex items-center space-x-2 mt-4 pt-4 border-t;
  }

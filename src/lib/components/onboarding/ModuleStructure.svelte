...
<div class="flex flex-col h-full bg-muted/10 border-l w-80">
	...
	>
		{#each items as item (item.id)}
			...
			<div animate:flip={{ duration: 300 }} class="relative group">
				...
				>
					...
					<div
						class="h-8 w-8 rounded-md bg-muted flex items-center justify-center shrink-0 text-muted-foreground border"
					>
						<Icon class="h-4 w-4" />
					</div>
		...
		{#if items.length === 0}
			<div
				class="text-center py-12 px-4 text-sm text-muted-foreground border-2 border-dashed rounded-lg"
			>
				<p>No steps yet.</p>

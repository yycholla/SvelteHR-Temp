<script lang="ts">
	...
	const options = $derived(element.elementType === 'FIELD' ? (element.validation?.options || []) : []);
	
	// Content blocks
	const content = $derived(
		element.elementType === 'TEXT_BLOCK' || element.elementType === 'QUOTE' || element.elementType === 'HEADER' 
		? element.content 
		: ''
	);
...
<div class="w-full">
		...
		<div class="space-y-2">
			...
			{:else if fieldType === 'CHECKBOX'}
				<div class="flex items-center space-x-2">
					<Checkbox disabled />
					<Label class="text-sm font-normal leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
						{placeholder || label}
					</Label>
				</div>
			{/if}
	...
	{:else if element.elementType === 'MEDIA'}
		<div class="rounded-lg border bg-muted/10 overflow-hidden">
			{#if mediaType === 'image'}
				{#if mediaUrl}
					<img src={mediaUrl} alt={mediaAlt || 'Form image'} class="w-full h-auto object-cover max-h-[400px]" />
				{:else}
			...
			{:else if mediaType === 'document'}
				<div class="p-6 flex items-center justify-center bg-muted/20">
					<div class="flex items-center gap-3 p-4 rounded-md border bg-background shadow-sm w-full max-w-sm">
						<div class="h-10 w-10 rounded bg-primary/10 flex items-center justify-center text-primary">
							<FileText class="h-5 w-5" />
						</div>
						<div class="flex-1 min-w-0">
							<div class="text-sm font-medium truncate">{mediaUrl ? mediaUrl.split('/').pop() : 'Document.pdf'}</div>

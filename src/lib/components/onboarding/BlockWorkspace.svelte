<script lang="ts">
	...
	$effect(() => {
		...
		formMode = block.formTemplateId
			? 'template'
			: block.inlineFormElements?.length > 0
				? 'custom'
				: 'template';
	});
		...
		<div class="flex-1 flex flex-col bg-white dark:bg-zinc-950 overflow-hidden">
			...
			<div class="flex-1 overflow-y-auto p-8">
				<div class="max-w-3xl mx-auto w-full">
					...
					{#if block.type === 'TEXT'}
						<div
							class="prose dark:prose-invert max-w-none text-muted-foreground leading-relaxed whitespace-pre-wrap"
						>
							{block.textContent || 'Start typing in the sidebar to add content...'}
						</div>
					{:else if block.type === 'DOCUMENT'}
						<div
							class="flex flex-col items-center justify-center border-2 border-dashed rounded-none p-12 bg-muted/5 text-center"
						>
							<div
								class="w-16 h-16 rounded-full bg-primary/5 flex items-center justify-center mb-4"
							>
								<FileText class="w-8 h-8 text-primary" />
							</div>
							{#if block.documentUrl}
								<h3 class="font-medium text-lg mb-2">Document Attached</h3>
								<p class="text-sm text-muted-foreground max-w-xs mb-6 break-all">
									{block.documentUrl}
								</p>
							...
							{:else}
								...
								<p class="text-sm text-muted-foreground">
									Add a document URL in the properties panel.
								</p>
							{/if}
						</div>
					{/if}

<script lang="ts">
  ...
  import type { CarbonColumn, BatchAction, ToolbarAction } from '../../../contracts/component-interface'
  ...
  // Sort logic
  $: sortedData = sortKey ? [...filteredData].sort((a, b) => {
...
>
  ...
  {:else}
    <DataTable
      ...
      on:update={(e) => handleSelect(e.detail.selectedRowIds.map(id => carbonRows.find(row => row.id === id)).filter(Boolean))}
    >
      <Toolbar>
        <ToolbarContent>
          {#if searchable}
          ...
          {#if batchActions.length > 0 && selectedRowIds.length > 0}
            <ToolbarMenu>
              {#each batchActions as action}
                <ToolbarMenuItem
                  primaryFocus
                  on:click={() => handleBatchAction(action.key)}
                >
                  {action.label}
                </ToolbarMenuItem>
              {/each}
        ...
        <table class="bx--data-table">
            ...
            <tr>
              ...
              {#each columns as column}
                <th
                  ...
                  class:bx--table-sort--ascending={sortKey === column.key && sortDirection === 'asc'}
                  class:bx--table-sort--descending={sortKey === column.key && sortDirection === 'desc'}
                  ...
                  {#if sortable && column.sortable !== false}
                    ...
                    >
                      ...
                      <svg class="bx--table-sort__icon" width="16" height="16" viewBox="0 0 16 16">
                        <path d="M12.3 9.3L8.5 13.1c-.1.1-.4.1-.5 0L4.2 9.3c-.2-.2-.2-.5 0-.7s.5-.2.7 0L8 11.6l3.1-3.1c.2-.2.5-.2.7 0s.2.6 0 .8z"/>
                        <path d="M4.2 6.7l3.8-3.8c.1-.1.4-.1.5 0l3.8 3.8c.2.2.2.5 0 .7s-.5.2-.7 0L8 4.4 4.9 7.5c-.2.2-.5.2-.7 0s-.2-.6 0-.8z"/>
                      </svg>
                    </button>
                  {:else}
                    <span class="bx--table-header-label">{column.label}</span>
          ...
          <tbody>
            ...
            {#if carbonRows.length === 0}
              <tr>
                <td colspan={columns.length + (selectable ? 1 : 0)} class="bx--table-cell carbon-empty-state">
          ...
          on:update={(e) => {
            ...
            if (accessibility.paginationAnnouncements) {
              ...
              announceToScreenReader(
                `Page changed. Showing ${currentRows} of ${total} items`
              )
            }
          }}
        />

...
<div class="rounded-2xl border border-border/40 bg-background/20 shadow-xl backdrop-blur-md">
	<div class="p-6">
		<div class="mb-4 flex items-center justify-between">
			<h3 class="text-lg font-semibold text-foreground">Filters</h3>
			{#if hasActiveFilters}
				<Button variant="ghost" size="sm" onclick={clearAllFilters} class="rounded-lg text-sm">
		...
		<div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
			<!-- Department Filters -->
			<div>
				<h4 class="mb-3 font-medium text-foreground">Department</h4>
				<div class="space-y-3">
					{#each departments as department}
						<div class="flex items-center space-x-3">
							...
							/>
							<Label for="dept-{department.id}" class="flex cursor-pointer items-center space-x-2">
			...
			<!-- Status Filters -->
			<div>
				<h4 class="mb-3 font-medium text-foreground">Status</h4>
				<div class="space-y-3">
					{#each statusOptions as status}
						<div class="flex items-center space-x-3">
							...
							/>
							<Label for="status-{status.value}" class="flex cursor-pointer items-center space-x-2">

		...
		compoundVariants: [
			{
				...
				attached: true,
				class:
					'[&>*:first-child]:rounded-b-none [&>*:last-child]:rounded-t-none [&>*:not(:first-child):not(:last-child)]:rounded-none [&>*:not(:first-child)]:border-t-0 [&>*:not(:first-child)]:border-l [&>*:not(:first-child)]:mt-[-1px] [&>*:not(:first-child)]:ml-0'
			},
			{
				attached: true,
...
<script lang="ts">
	...
	function handleKeydown(event: KeyboardEvent) {
		...
		const buttons = ref?.querySelectorAll(
			'button:not([disabled])'
...
<div
	...
	class={cn(
		buttonGroupVariants({ orientation, attached, size, variant, fullWidth, disabled }),
		className
	)}
	{role}

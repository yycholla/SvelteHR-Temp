...
<script lang="ts">
	...
	let selectedValues = $derived(multiple 
	...
	let groupedOptions = $derived(filteredOptions.reduce((acc, option) => {
...
>
			...
			{#if multiple && selectedValues.length > 1}
				<div class="flex flex-wrap gap-1">
					{#each selectedValues.slice(0, 3) as selectedValue}
						...
						<span class="inline-flex items-center gap-1 px-2 py-0.5 bg-secondary-100 text-secondary-700 rounded text-xs">
					...
					{#if selectedValues.length > 3}
						<span class="text-secondary-500 text-xs">+{selectedValues.length - 3} more</span>
					{/if}
				</div>
			{:else}
				<span class={cn(
	...
	{#if isOpen}
		<div
			...
			class="absolute top-full mt-1 w-full z-50 rounded-md border bg-popover shadow-lg"
			role="listbox"
		...
		>
			{#if searchable}
				<div class="p-2 border-b">
					<input
						...
						class="w-full px-3 py-1.5 text-sm border border-border rounded focus:outline-none focus:ring-1 focus:ring-primary-500"
					/>
				</div>
			{/if}
			...
			<div class="max-h-60 overflow-auto p-1">
				...
				{:else if filteredOptions.length === 0}
					<div class="py-4 text-center text-secondary-500 text-sm">
						{emptyText}
					</div>
				{:else}
					{#each Object.entries(groupedOptions) as [groupName, groupOptions], groupIndex}
						{#if groupName !== 'default'}
							<div class="px-2 py-1 text-xs font-medium text-secondary-600 bg-secondary-50">
								{groupName}
							</div>
						{/if}
						
						{#each groupOptions as option, optionIndex}
							{@const globalIndex = Object.entries(groupedOptions).slice(0, groupIndex).reduce((acc, [, opts]) => acc + opts.length, 0) + optionIndex}
							<button
								type="button"
								class={cn(
									"w-full flex items-center justify-between px-2 py-1.5 text-sm rounded text-left",
							...
							>
								<div class="flex items-center gap-2 flex-1 min-w-0">
									{#if option.icon}
										<svelte:component this={option.icon} class="w-4 h-4 flex-shrink-0" />
									{/if}

<script lang="ts" module>
	...
	export const selectableCardVariants = tv({
		...
		variants: {
			variant: {
				default:
					'card-default hover:shadow-md hover:border-primary-200 dark:hover:border-primary-700',
				outline:
					'card-outline hover:shadow-sm hover:border-primary-300 dark:hover:border-primary-600',
...
<script lang="ts">
	...
	let ariaChecked = $derived(
		selectionMode === 'checkbox' || selectionMode === 'radio' ? selected : undefined
	);
...
<style>
	...
	:global(.dark) [data-slot='selectable-card']:focus-visible {
		box-shadow:
			0 0 0 2px var(--color-primary-500),
			0 0 0 4px var(--color-secondary-900);
	}

...
>
	...
	{#if showHeader}
		<div class="card-header p-6 pb-4">
			<!-- Title skeleton -->
			<div class={cn('skeleton-base mb-2 h-6 w-3/4', animated && 'animate-pulse')}></div>

			<!-- Subtitle skeleton -->
			<div class={cn('skeleton-base h-4 w-1/2', animated && 'animate-pulse')}></div>
		</div>
	{/if}

	<div class={cn('card-content', showHeader ? 'p-6 pt-0' : 'p-6')}>
		{#if variant === 'profile'}
			<!-- Profile variant: Avatar + info -->
			<div class="mb-4 flex items-start space-x-4">
				<div
					class={cn(
						'skeleton-base h-12 w-12 flex-shrink-0 rounded-full',
						animated && 'animate-pulse'
					)}
				></div>
				<div class="flex-1 space-y-2">
					<div class={cn('skeleton-base h-4 w-32', animated && 'animate-pulse')}></div>
					<div class={cn('skeleton-base h-3 w-24', animated && 'animate-pulse')}></div>
				</div>
			</div>
		{:else if variant === 'list'}
			<!-- List variant: Multiple items -->
			{#each Array(3) as _, i}
				<div class="mb-3 flex items-center space-x-3 last:mb-0">
					<div class={cn('skeleton-base h-8 w-8 rounded', animated && 'animate-pulse')}></div>
		...
		{:else if variant === 'content'}
			...
			<div class="space-y-3">
				{#each lineWidths as width}
					<div class={cn('skeleton-base h-4', width, animated && 'animate-pulse')}></div>
				{/each}
			</div>
		{:else}
			...
			<div class="space-y-2">
				{#each lineWidths as width, i}
					<div
						class={cn('skeleton-base', i === 0 ? 'h-4' : 'h-3', width, animated && 'animate-pulse')}
					></div>
				{/each}
			</div>
		{/if}
	...
	{#if showFooter}
		<div class="card-footer flex items-center justify-between p-6 pt-0">
			<div class={cn('skeleton-base h-8 w-20', animated && 'animate-pulse')}></div>
			<div class={cn('skeleton-base h-8 w-16', animated && 'animate-pulse')}></div>
		</div>
	{/if}
</div>

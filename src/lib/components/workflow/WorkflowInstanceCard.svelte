<script lang="ts">
  ...
  export let instance: WorkflowInstance;
  
  const dispatch = createEventDispatcher();
  
  $: statusColor = getStatusColor(instance.status);
  $: categoryColor = getCategoryColor(instance.workflowDefinitionByWorkflowDefinitionId?.category);
  $: duration = calculateDuration(instance.startedAt, instance.completedAt);
...
<div class="workflow-instance-card bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow p-6">
  <!-- Header -->
  <div class="flex justify-between items-start mb-4">
    ...
    <span class={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColor}`}>
      <span class="mr-1">{@html getStatusIcon(instance.status)}</span>
      {instance.status}
    </span>
  </div>
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
    <div>
      ...
      <dd class="text-sm text-gray-900 mt-1">{formatDate(instance.createdAt)}</dd>
    </div>
    
    {#if instance.startedAt}
      <div>
        <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">Started</dt>
        <dd class="text-sm text-gray-900 mt-1">{formatDate(instance.startedAt)}</dd>
      </div>
    {/if}
    
    {#if instance.completedAt}
      <div>
        <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">Completed</dt>
        <dd class="text-sm text-gray-900 mt-1">{formatDate(instance.completedAt)}</dd>
      </div>
    {/if}
    ...
    {#if duration}
      <div>
        ...
        <dd class="text-sm text-gray-900 mt-1">{duration}</dd>
      </div>
    {/if}
  </div>
  ...
  {#if instance.userByTriggeredByUserId}
    <div class="mb-4">
      <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide">Triggered By</dt>
  ...
  {#if instance.triggerData && typeof instance.triggerData === 'object'}
    <div class="mb-4">
      <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Trigger Data</dt>
      <div class="bg-gray-50 rounded-md p-3">
        <pre class="text-xs text-gray-700 overflow-x-auto">{JSON.stringify(instance.triggerData, null, 2)}</pre>
      </div>
    </div>
  {/if}

...
<div class={`workflow-task-card bg-white rounded-lg shadow-sm border transition-all duration-200 p-6 ${isOverdue ? 'border-red-300 bg-red-50' : 'border-gray-200 hover:shadow-md'}`}>
  <!-- Header -->
  ...
  <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
    ...
    <div>
      ...
      <dd class="mt-1">
        <span class={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${priorityColor}`}>
    ...
    <div>
      ...
      <dd class="text-sm text-gray-900 mt-1">{task.taskType}</dd>
    </div>
    ...
    {#if task.dueDate}
      <div>
        ...
        <dd class={`text-sm mt-1 ${isOverdue ? 'text-red-600 font-medium' : 'text-gray-900'}`}>
    ...
    {#if task.completedAt}
      <div>
        ...
        <dd class="text-sm text-gray-900 mt-1">{formatDate(task.completedAt)}</dd>
      </div>
    {/if}
  </div>
  ...
  <div class="mb-4">
    <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Workflow</dt>
    <div class="bg-gray-50 rounded-md p-3">
      <p class="text-sm font-medium text-gray-900">
        {task.workflowInstanceByWorkflowInstanceId?.workflowDefinitionByWorkflowDefinitionId?.name || 'Unknown Workflow'}
      </p>
      {#if task.workflowInstanceByWorkflowInstanceId?.workflowDefinitionByWorkflowDefinitionId?.category}
        <p class="text-xs text-gray-500">
          Category: {task.workflowInstanceByWorkflowInstanceId.workflowDefinitionByWorkflowDefinitionId.category}
        </p>
      {/if}
    </div>
  </div>
  ...
  {#if task.userByAssignedToId}
    <div class="mb-4">
      <dt class="text-xs font-medium text-gray-500 uppercase tracking-wide mb-2">Assigned To</dt>

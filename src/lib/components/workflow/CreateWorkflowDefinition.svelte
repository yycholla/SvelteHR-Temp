...
<div class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center p-4 z-50">
  <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
    ...
    <form on:submit|preventDefault={handleSubmit} class="px-6 py-4 space-y-6">
      ...
      <!-- Configuration -->
      <div>
        <h4 class="text-sm font-medium text-gray-900 mb-4">Configuration</h4>
      ...
      <div>
        ...
        <textarea
          ...
          rows="4"
          class={`mt-1 block w-full border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 font-mono text-sm ${isValidJson.triggerConditions ? 'border-gray-300' : 'border-red-300'}`}
          placeholder={`{"condition": "example"}`}
        ></textarea>
        {#if !isValidJson.triggerConditions}
          <p class="text-sm text-red-600 mt-1">Invalid JSON format</p>
        {/if}
      </div>
      ...
      <div>
        ...
        <textarea
          ...
          rows="6"
          class={`mt-1 block w-full border rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 font-mono text-sm ${isValidJson.definition ? 'border-gray-300' : 'border-red-300'}`}

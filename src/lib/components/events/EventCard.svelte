	...
	<div class="mb-2 flex items-start justify-between pl-3">
		<div class="flex-1">
			...
			{#if !compact}
				<div class="mt-1 flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
					...
					<!-- Event Status Badge -->
					<span
						class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {getEventStatusColor(
							event.status
						)}"
					>
						{event.status}
		...
		{#if showRsvp && userRsvpStatus}
			<div class="ml-4 flex-shrink-0">
				<span
					class="inline-flex items-center rounded-full px-3 py-1 text-xs font-medium {getRsvpStatusColor(
						userRsvpStatus
					)}"
				>
		...
		<div class="flex items-center justify-between border-t pt-3 pl-3">
			...
			{#if event.eventAttendeesByEventId && event.eventAttendeesByEventId.nodes.length > 0}
				<div class="flex items-center text-sm text-muted-foreground">
					...
					<span
						>{event.eventAttendeesByEventId.nodes.length} attendee{event.eventAttendeesByEventId
							.nodes.length !== 1
							? 's'
							: ''}</span
					>

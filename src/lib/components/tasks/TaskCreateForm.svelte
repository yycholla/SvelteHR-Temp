...
<form class="space-y-4" onsubmit={(e) => { e.preventDefault(); }}>
  ...
  <div class="space-y-2">
    ...
    <Textarea id="description" bind:value={$form.description} placeholder="Optional details" class="rounded-xl" />
  </div>

  <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
    <div class="space-y-2">
      <Label for="status">Status</Label>
      <select id="status" bind:value={$form.status} class="w-full px-3 py-2 rounded-xl bg-background border-border focus:bg-background transition-all duration-200">

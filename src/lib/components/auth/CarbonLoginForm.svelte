<script lang="ts">
  ...
  // Event handlers
  export let onSubmit: ((credentials: LoginCredentials) => Promise<boolean>) | undefined = undefined;
  ...
  const validateForm = (): boolean => {
    ...
    if (errorsChanged) {
      ...
      if (accessibility.announceValidation && Object.keys(formErrors).length > 0) {
        ...
        announceToScreenReader(`Form has ${errorCount} validation error${errorCount === 1 ? '' : 's'}`);
      }
    }

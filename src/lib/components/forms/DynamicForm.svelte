<script lang="ts">
	...
	$: layoutClass =
		schema.ui_config?.layout === 'three_column'
			? 'grid-cols-1 lg:grid-cols-3'
			: schema.ui_config?.layout === 'single_column'
				? 'grid-cols-1'
...
<div class="dynamic-form" class:show-progress={schema.ui_config?.show_progress}>
	{#if schema.ui_config?.show_progress}
		<div class="form-progress mb-6">
			<div class="progress-bar h-2 rounded-full bg-gray-200">
				<div class="progress-fill h-2 rounded-full bg-primary" style="width: 0%"></div>
			</div>
		</div>
	{/if}
						...
						{:else if shouldShowField(field)}
							...
							>
								...
								{#if field.ui_config?.help_text}
									<p class="field-help-text mt-1 text-xs text-gray-500 dark:text-gray-400">
	...
	{#if showSubmitButton}
		<div
			class="form-actions mt-8 flex justify-end gap-4 border-t border-gray-200 pt-6 dark:border-gray-700"
		>
			{#if onCancel}

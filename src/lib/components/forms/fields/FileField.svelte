		...
		>
			{#if files.length === 0}
				<div class="drop-zone-empty">
					...
					<p class="text-sm text-gray-600 dark:text-gray-400">
						<Button type="button" variant="link" class="h-auto p-0 font-medium">
							Choose files
						</Button>
						or drag and drop
					</p>
			...
			{:else}
				<div class="selected-files">
					<p class="mb-2 text-sm font-medium text-gray-700 dark:text-gray-300">Selected Files:</p>
					<div class="file-list space-y-2">
						{#each files as file, index}
							<div class="file-item">
								<div class="file-info">
									<FileIcon class="h-4 w-4 text-gray-500" />
									<span class="file-name truncate text-sm">{file.name}</span>

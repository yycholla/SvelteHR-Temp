<script lang="ts">
	...
	$: colWidth = $dashboardLayout
		? (gridWidth - GRID_GAP * ($dashboardLayout.gridCols + 1)) / $dashboardLayout.gridCols
		: 100;
	...
	function handleDragEnd(event: MouseEvent) {
		...
		if (
			isPositionOccupied(newPosition.x, newPosition.y, newPosition.w, newPosition.h, draggedCard.id)
		) {
...
<div
	class="dashboard-grid relative h-full w-full overflow-auto bg-muted/30"
	bind:this={gridContainer}
>
	{#if $dashboardLayout && visibleCards.length > 0}
		<div class="relative h-full w-full p-4" style="min-height: {contentHeight}px;">
			{#each visibleCards as card (card.id)}
				<div
					data-card-id={card.id}
					style={getCardStyle(card)}
					class="dashboard-card-container relative transition-all duration-200 {$isDragging &&
				...
				>
					...
					{#if $isEditing}
						...
						<div class="pointer-events-none absolute inset-0">
							<!-- Corner handles -->
							<div
								class="pointer-events-auto absolute h-3 w-3 cursor-nw-resize rounded-full border-2 border-white bg-primary shadow-sm"
								style="top: -6px; left: -6px;"
								on:mousedown={(e) => handleResizeStart(e, card, 'nw')}
							></div>
							<div
								class="pointer-events-auto absolute h-3 w-3 cursor-ne-resize rounded-full border-2 border-white bg-primary shadow-sm"
								style="top: -6px; right: -6px;"
								on:mousedown={(e) => handleResizeStart(e, card, 'ne')}
							></div>
							<div
								class="pointer-events-auto absolute h-3 w-3 cursor-sw-resize rounded-full border-2 border-white bg-primary shadow-sm"
								style="bottom: -6px; left: -6px;"
								on:mousedown={(e) => handleResizeStart(e, card, 'sw')}
							></div>
							<div
								class="pointer-events-auto absolute h-3 w-3 cursor-se-resize rounded-full border-2 border-white bg-primary shadow-sm"
							...
							<!-- Edge handles -->
							<div
								class="pointer-events-auto absolute h-2 w-full cursor-n-resize"
								style="top: -4px; left: 0;"
								on:mousedown={(e) => handleResizeStart(e, card, 'n')}
							></div>
							<div
								class="pointer-events-auto absolute h-2 w-full cursor-s-resize"
								style="bottom: -4px; left: 0;"
								on:mousedown={(e) => handleResizeStart(e, card, 's')}
							></div>
							<div
								class="pointer-events-auto absolute h-full w-2 cursor-w-resize"
								style="top: 0; left: -4px;"
								on:mousedown={(e) => handleResizeStart(e, card, 'w')}
							></div>
							<div
								class="pointer-events-auto absolute h-full w-2 cursor-e-resize"
		...
		<div class="flex h-full items-center justify-center text-center">
			<div class="space-y-4">
				...
				<p class="max-w-md text-sm text-muted-foreground">
					{#if $isEditing}
						Add some cards to get started. Use the card library to find widgets that suit your
						needs.
					{:else}

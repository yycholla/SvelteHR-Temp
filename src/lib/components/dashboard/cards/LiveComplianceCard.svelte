<script lang="ts">
	...
	function getComplianceStatus(score: number) {
		...
		if (score >= 85)
			return { color: 'text-blue-600', bg: 'bg-blue-100', icon: Shield, status: 'Good' };
		if (score >= 70)
			return {
		...
		<div class="h-full space-y-3">
			...
			<div class="flex items-center justify-between">
				<div class="flex items-center space-x-2">
					<div class="h-8 w-8 {statusInfo.bg} flex items-center justify-center rounded-full">
			...
			<div class="grid min-h-0 flex-1 grid-cols-3 gap-2">
				...
				<div class="rounded-lg bg-green-50 p-2 text-center">
					<CheckCircle class="mx-auto mb-1 h-4 w-4 text-green-600" />
				...
				<div class="rounded-lg bg-red-50 p-2 text-center">
					<AlertTriangle class="mx-auto mb-1 h-4 w-4 text-red-600" />
				...
				<div class="rounded-lg bg-yellow-50 p-2 text-center">
					<Clock class="mx-auto mb-1 h-4 w-4 text-yellow-600" />
			...
			{#if complianceData.recentViolations.length > 0}
				<div class="border-t pt-2">
					<div class="mb-1 text-xs font-medium text-muted-foreground">Recent Issues</div>
					<div class="max-h-16 space-y-1 overflow-hidden">
						{#each complianceData.recentViolations.slice(0, 2) as violation}
							<div class="flex items-center space-x-2 text-xs">
								<AlertTriangle class="h-3 w-3 flex-shrink-0 text-red-500" />
								<span class="truncate"
									>{violation.title || violation.description || 'Compliance issue'}</span
								>
							</div>
						{/each}

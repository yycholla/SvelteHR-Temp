...
class CarbonDashboardTileTestUtils {
	...
	async getTrendDirection() {
		...
		const hasPositive = await trend.evaluate((el) =>
			el.classList.contains('carbon-trend--positive')
		);
		const hasNegative = await trend.evaluate((el) =>
			el.classList.contains('carbon-trend--negative')
		);
...
test.describe('CarbonDashboardTile Component Contract', () => {
	...
	test.describe('Color Variants', () => {
		...
		for (const color of colors) {
			test(`should handle ${color} color variant`, async ({ page }) => {
				...
				const icon = await utils.getTileIcon();
				const hasColorClass = await icon.evaluate(
					(el, color) => el.classList.contains(`carbon-tile--${color}`),
					color
				);
	...
	test.describe('Trend Indicators', () => {
		test('should handle positive trends', async () => {
			...
			const trend = await utils.getTrend();
			const color = await trend.evaluate((el) => window.getComputedStyle(el).color);
	...
	test.describe('Interactive Behavior', () => {
		test('should handle interactive tiles', async ({ page }) => {
			...
			// Should be clickable
			const tile = await utils.getTileElement();
			const isClickable = await tile.evaluate(
				(el) => el.querySelector('.bx--tile--clickable') !== null
			);
	...
	test.describe('Carbon Design Integration', () => {
		...
		test('should apply Carbon design tokens', async () => {
			...
			// Check spacing
			const gap = await tile.evaluate(
				(el) => window.getComputedStyle(el.querySelector('.carbon-tile-content')).gap
			);
        ...
        `
			...
			await page.evaluate(() => document.documentElement.setAttribute('data-carbon-theme', 'g100'));

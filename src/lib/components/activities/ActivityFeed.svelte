<script lang="ts">
	...
	// Purpose: Chronological activity feed display
...
<div class="activity-feed">
		...
		{#if groupByDate && groupedActivities}
			...
			{#each Array.from(groupedActivities.entries()) as [date, dateActivities]}
				<div class="date-group mb-6">
					<!-- Date Header -->
					<div class="sticky top-0 z-10 mb-3 bg-white py-2">
						...
						{#each dateActivities as activity (activity.id)}
							...
							>
								...
								<div class="flex-1 pb-4">
									<!-- Activity Message -->
									<p class="text-sm text-gray-900 group-hover:text-blue-600">
										{#if showUserInfo && activity.employee}
											<span class="font-medium text-gray-900">{activity.employee.displayName}</span>
											{' '}
										{/if}
		...
		{:else}
			...
			<div class="space-y-3">
				{#each displayActivities as activity (activity.id)}
					<div
						class="activity-item group relative flex gap-3 rounded-lg border border-gray-200 bg-white p-3 transition-all hover:shadow-md"
						class:cursor-pointer={onActivityClick}
						class:hover:border-blue-300={onActivityClick}
						class:compact
						role={onActivityClick ? 'button' : 'article'}
						tabindex={onActivityClick ? 0 : undefined}
						onclick={() => handleActivityClick(activity)}
	...
	{:else}
		...
		<div class="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-gray-300 bg-gray-50 p-12">
			...
			<h3 class="mb-2 text-lg font-medium text-gray-900">No activity</h3>
...
<style>
	...
	.date-group .activity-item:not(:last-child)::after {
		content: '';
		@apply absolute left-4 top-10 h-[calc(100%+0.75rem)] w-px bg-gray-200;
	}
</style>

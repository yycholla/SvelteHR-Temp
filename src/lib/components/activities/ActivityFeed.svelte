					...
					<div class="space-y-3 pl-4">
						{#each dateActivities as activity (activity.id)}
							<div
								...
								class:is-rollback={activity.isRollback && showRollbackIndicators}
							>
								<div
								...
								>
									...
									<div class="flex-shrink-0">
										<div
											class="flex h-8 w-8 items-center justify-center rounded-full {getActivityActionColor(
												activity.action
											)}"
										>
											...
											{:else}
												<span class="text-sm"
													>{getActivityIcon(activity.resourceType, activity.action)}</span
												>
											{/if}
										</div>
									</div>
		...
		{:else}
			...
			<div class="space-y-3">
				{#each displayActivities as activity (activity.id)}
					...
					>
						<div
							class="activity-item group relative flex gap-3 rounded-lg bg-card p-3 transition-all hover:shadow-md {activity.isRollback &&
							showRollbackIndicators
								? ''
								: 'border border-border'}"
							class:cursor-pointer={onActivityClick}
							class:hover:border-primary={onActivityClick &&
								!(activity.isRollback && showRollbackIndicators)}
							class:compact
							role={onActivityClick ? 'button' : 'article'}
							{...onActivityClick ? { tabindex: 0 } : {}}
							onclick={() => handleActivityClick(activity)}
						...
						>
							...
							<div class="flex-shrink-0">
								<div
									class="flex h-10 w-10 items-center justify-center rounded-full {getActivityActionColor(
										activity.action
									)}"
								>
									{#if activity.isRollback && showRollbackIndicators}

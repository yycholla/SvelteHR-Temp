...
export const employeeStatusSchema = z.enum(['PreHire', 'Onboarding', 'Active', 'Terminated']);
...
export const nestedDepartmentSchema = z
	.object({
		ID: z.number(),
		Name: z.string(),
...
export const departmentSchema = z.union([departmentApiSchema, departmentTransformedSchema]);
...
export const employeeSchema = z
	.union([flatEmployeeSchema, nestedEmployeeSchema])
	.transform((emp) => {
		...
		if (!isNested) {
			...
			return {
				...
				location:
					[flatEmp.addressCity, flatEmp.addressState].filter(Boolean).join(', ') || 'Remote',
			...
			return {
				...
				location:
					[contact.AddressCity, contact.AddressState].filter(Boolean).join(', ') || 'Remote',
...
const employeeListResponseBackendSchema = z
	.object({

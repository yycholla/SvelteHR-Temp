...
export class AuthenticationOperations {
  ...
  async login(credentials: LoginCredentials): Promise<AuthenticationResult> {
    ...
    class AuthRetryHandler {
      ...
      private isAuthError(error: unknown): boolean {
        ...
        return errorString.includes('credential') ||
    ...
    return new Promise<AuthenticationResult>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Token refresh timeout'),
          {
            type: 'TIMEOUT_ERROR',
  ...
  async verifyToken(): Promise<AuthenticationResult> {
    ...
    return new Promise<AuthenticationResult>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Token verification timeout'),
          {
            type: 'TIMEOUT_ERROR',
 ...
 */
export function extractPermissions(roles: Array<{ name: string; permissions: Array<{ resource: string; action: string }> }>): string[] {
 ...
 */
export async function createUserSessionFromAuth(authResult: AuthenticationResult): Promise<UserSession> {

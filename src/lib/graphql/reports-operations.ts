...
export class ReportsOperations {
  ...
  private generateMockReports(count: number): HRReport[] {
    ...
    return Array.from({ length: count }, (_, i) => ({
      ...
      lastRunAt: i % 2 === 0 ? new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000).toISOString() : null,
...
export async function getReportAnalytics(params: {
  ...
  return {
    ...
    departmentUsage: [
      ...
      { department: 'Finance', reportCount: 12, lastActivity: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString() },
      { department: 'Engineering', reportCount: 8, lastActivity: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString() }
    ],
    ...
    popularReports: [
      { id: 'report-1', title: 'Monthly Employee Summary', runCount: 45, lastRun: new Date().toISOString(), avgDuration: 32.5 },
      { id: 'report-2', title: 'Payroll Analysis', runCount: 38, lastRun: new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), avgDuration: 28.1 }
    ],
    performanceMetrics: {

...
export const GET_COMPLETE_EMPLOYEE_DASHBOARD = `
  query GetCompleteEmployeeDashboard($userId: UUID!) {
    ...
    allLeaveRequests(condition: { employeeId: $userId }) {
      totalCount
      nodes {
        id
        status
    ...
    allAttendanceRecords(condition: { employeeId: $userId }, orderBy: DATE_DESC, first: 30) {
      totalCount
      nodes {
        id
        date
...
export function calculateAttendanceRate(attendanceRecords: any[]): number {
  ...
  const presentDays = attendanceRecords.filter(record =>
    record.status === 'present' || record.status === 'late' || (record.totalHours && record.totalHours > 0)
  ).length;
...
export function calculateRemainingVacationDays(user: any, leaveBalances?: any[]): number {
  ...
  const yearsEmployed = Math.max(0, (Date.now() - hireDate.getTime()) / (1000 * 60 * 60 * 24 * 365));
...
export async function getCompleteDashboardData(userId: string) {
  try {
    ...
    const pendingRequests = leaveRequests.filter(req => req.status === 'pending' || req.status === 'submitted').length;
    const taskCount = 0; // No tasks system implemented yet

    // Calculate department context
    const userDepartment = getUserDepartment(user);
    const departmentColleagues = allUsers.filter(u => u.isActive && getUserDepartment(u) === userDepartment);

    // Calculate company tenure
    const hireDate = user.hireDate ? new Date(user.hireDate) : new Date();
    const tenureInMonths = Math.floor((Date.now() - hireDate.getTime()) / (1000 * 60 * 60 * 24 * 30));

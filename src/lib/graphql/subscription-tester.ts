...
export class GraphQLSubscriptionTester {
  ...
  async testConnectionResilience(
    ...
    const resililienceRunner = new SubscriptionResilienceTestRunner(
      this.client,
      this.config
    );
  ...
  async testSubscriptionPerformance(
    ...
    const performanceRunner = new SubscriptionPerformanceTestRunner(
      this.client,
      this.config
    );
  ...
  async testMessageOrdering(
    ...
    const orderingRunner = new SubscriptionOrderingTestRunner(
      this.client,
      this.config
    );
  ...
  async stopAllTests(): Promise<void> {
    const stopPromises = Array.from(this.activeTests.values()).map(runner =>
      runner.stop()
    );
...
class SubscriptionTestRunner {
  ...
  private handleSubscriptionMessage(result: OperationResult): void {
    ...
    const totalLatency = this.result.performanceMetrics.averageLatency * (this.result.totalMessages - 1) + latency;

...
export const GET_SCHEDULED_REPORTS = gql`
  query GetScheduledReports(
    $first: Int = 50
    $orderBy: [TeamReportsOrderBy!] = [UPDATED_AT_DESC]
  ) {
    teamReports(
...
export const scheduleOptions = [
  ...
  { value: '0 9 1 * *', label: 'Monthly (1st at 9 AM)', description: '1st of every month at 9:00 AM' },
  { value: '0 9 1 1,4,7,10 *', label: 'Quarterly (1st at 9 AM)', description: '1st of Jan, Apr, Jul, Oct at 9:00 AM' },
  { value: '0 9 * * 1-5', label: 'Daily (Weekdays at 9 AM)', description: 'Monday to Friday at 9:00 AM' },
...
export function generateReportTemplate(reportType: ReportType, parameters: any = {}): ReportTemplate {
...
export function calculateReportStatistics(reports: TeamReport[]) {
  ...
  return {
    ...
    rates: {
      ...
      scheduleUtilization: totalReports > 0 ? Math.round((scheduledReports / totalReports) * 100) : 0
    },
    breakdowns: {
    ...
    performance: {
      ...
      mostPopularType: typeBreakdown.reduce((max, type) =>
        type.count > max.count ? type : max, typeBreakdown[0]
      )
    }
  };
}

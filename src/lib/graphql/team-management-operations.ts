...
export const GET_TEAM_HIERARCHY = gql`
  query GetTeamHierarchy($rootDepartmentId: UUID) {
    departments(
      condition: { parentDepartmentId: $rootDepartmentId }
      orderBy: [NAME_ASC]
    ) {
      nodes {
        id
...
export function categorizeTeamSize(employeeCount: number): typeof teamSizeCategories[0] {
  return teamSizeCategories.find(cat =>
    employeeCount >= cat.min && employeeCount <= cat.max
...
export function getDepartmentTypeInfo(name: string): typeof departmentTypes[0] {
  ...
  return departmentTypes.find(type =>
    lowerName.includes(type.value) || lowerName.includes(type.label.toLowerCase())
  ) || departmentTypes[5]; // Default to operations
}
...
export function calculateTeamStats(departments: Department[]) {
  ...
  const totalEmployees = departments.reduce((sum, dept) =>
    sum + (dept.employees?.totalCount || 0), 0
  );
  const activeEmployees = departments.reduce((sum, dept) =>
    sum + (dept.activeEmployees?.totalCount || 0), 0
  );

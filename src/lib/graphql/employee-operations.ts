...
export const GET_EMPLOYEES_QUERY = gql`
  query GetEmployees($first: Int, $after: Cursor, $filter: EmployeeFilter, $orderBy: [EmployeesOrderBy!]) {
    employees(first: $first, after: $after, filter: $filter, orderBy: $orderBy) {
      nodes {
        id
        email
        displayName
...
export class EmployeeOperations {
        ...
        const unsubscribe = this.client.subscribe(
          ...
          (result) => {
            if (result.error) {
              ...
              console.log(`Loaded ${result.data.employees.nodes.length} employees`);
              resolve(result.data.employees);
              unsubscribe();
            }
          }
        );
    ...
    return new Promise<Employee>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Employee fetch timeout'),
          {
            type: 'TIMEOUT_ERROR',
  ...
  async getEmployeeDashboard(params: {
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Employee dashboard timeout'),
          {
            type: 'TIMEOUT_ERROR',
  ...
  async createEmployee(params: {
    ...
    return new Promise<Employee>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Employee creation timeout'),
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Employee creation is taking longer than expected. Please check if the employee was created.'
          }
        );
        reject(errorResponse);
        unsubscribe();
  ...
  async updateEmployee(params: {
    ...
    return new Promise<Employee>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Employee update timeout'),
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Employee update is taking longer than expected. Please verify the changes were saved.'
          }
        );
        reject(errorResponse);
        unsubscribe();
  ...
  async deactivateEmployee(params: {
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Employee deactivation timeout'),
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Employee deactivation is taking longer than expected. Please verify the change was applied.'
          }
        );
        reject(errorResponse);
        unsubscribe();
...
export function canViewEmployee(employee: Employee, userCredentials: UserCredentials): boolean {
  ...
  if (userCredentials.permissions.includes('*') || userCredentials.permissions.includes('employees:read')) {
    return true;
  }

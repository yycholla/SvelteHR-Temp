...
export const performanceRatings = [
  { value: 1, label: 'Needs Improvement', description: 'Does not meet expectations', color: 'red', icon: '⚠️' },
  { value: 2, label: 'Below Expectations', description: 'Partially meets expectations', color: 'orange', icon: '📉' },
  { value: 3, label: 'Meets Expectations', description: 'Satisfactory performance', color: 'yellow', icon: '✅' },
  { value: 4, label: 'Exceeds Expectations', description: 'Strong performance', color: 'blue', icon: '⭐' },
  { value: 5, label: 'Outstanding', description: 'Exceptional performance', color: 'green', icon: '🏆' }
];
...
export function generateReviewAnalytics(reviews: PerformanceReview[]) {
  ...
  const avgGoalsAchievement = completedReviews.length > 0
    ? Math.round((completedReviews.reduce((sum, r) => sum + (r.goalsAchievement || 0), 0) / completedReviews.length) * 10) / 10
    : 0;
  const avgCollaboration = completedReviews.length > 0
    ? Math.round((completedReviews.reduce((sum, r) => sum + (r.collaboration || 0), 0) / completedReviews.length) * 10) / 10
    : 0;
  const avgCommunication = completedReviews.length > 0
    ? Math.round((completedReviews.reduce((sum, r) => sum + (r.communication || 0), 0) / completedReviews.length) * 10) / 10
    : 0;
  const avgLeadership = completedReviews.length > 0
    ? Math.round((completedReviews.reduce((sum, r) => sum + (r.leadership || 0), 0) / completedReviews.length) * 10) / 10
    : 0;
...
export class PerformanceOperations {
          ...
          try {
            ...
            const result = await Promise.race([
              ...
              new Promise<never>((_, reject) =>
                setTimeout(() => reject(new Error('Performance reviews query timeout')), request.timeoutMs)
              )
            ]);
            ...
            if (this.attempts > request.maxRetries) {
              ...
              const errorResponse = createErrorResponse(error, {
                ...
                userMessage: 'Unable to load performance reviews. Please try again or contact support.'
              });
          ...
          (result) => {
            if (result.error) {
              ...
              const errorResponse = createErrorResponse(result.error, {
                type: 'GRAPHQL_ERROR',
                userMessage: 'Unable to load performance reviews. Please check your permissions and try again.'
              });
              reject(errorResponse);
              unsubscribe();
            } else if (result.data?.performanceReviews) {
              console.log(`Loaded ${result.data.performanceReviews.nodes.length} performance reviews`);
    ...
    return new Promise<PerformanceReview>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Performance review fetch timeout'),
          {
            type: 'TIMEOUT_ERROR',
    ...
    return new Promise<PerformanceReview>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          ...
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Performance review creation is taking longer than expected. Please check if it was created.'
          }
        );
        reject(errorResponse);
        unsubscribe();
      ...
      const unsubscribe = this.client.subscribe(
        ...
        (result) => {
          ...
          if (result.error) {
            ...
            const errorResponse = createErrorResponse(result.error, {
              type: 'VALIDATION_ERROR',
              userMessage: 'Unable to create performance review. Please check the information and try again.'
            });
            reject(errorResponse);
            unsubscribe();
          } else if (result.data?.createPerformanceReview?.performanceReview) {
            console.log(`Created performance review: ${result.data.createPerformanceReview.performanceReview.id}`);
  ...
  async updatePerformanceReview(params: {
    ...
    return new Promise<PerformanceReview>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Performance review update timeout'),
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Performance review update is taking longer than expected. Please verify the changes were saved.'
          }
        );
        reject(errorResponse);
        unsubscribe();
      ...
      const unsubscribe = this.client.subscribe(
        ...
        (result) => {
          ...
          if (result.error) {
            ...
            const errorResponse = createErrorResponse(result.error, {
              type: 'VALIDATION_ERROR',
              userMessage: 'Unable to update performance review. Please check the information and try again.'
            });
            reject(errorResponse);
            unsubscribe();
          } else if (result.data?.updatePerformanceReview?.performanceReview) {
            console.log(`Updated performance review: ${result.data.updatePerformanceReview.performanceReview.id}`);
  ...
  async getPerformanceStatistics(params: {
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Performance statistics timeout'),
          {
            type: 'TIMEOUT_ERROR',
    ...
    return new Promise((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          ...
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Review deletion is taking longer than expected. Please verify the action completed.'
          }
        );
        reject(errorResponse);
        unsubscribe();
        ...
        (result) => {
          ...
          if (result.error) {
            const errorResponse = createErrorResponse(result.error, {
              ...
              userMessage: 'Unable to delete performance review. Please check your permissions and try again.'
            });
            reject(errorResponse);
            unsubscribe();
          } else if (result.data) {
 ...
 */
export function canManagePerformanceReview(review: PerformanceReview, userCredentials: UserCredentials): boolean {
  // Admin can manage all reviews
  if (userCredentials.permissions.includes('*') || userCredentials.permissions.includes('performance:write')) {
    return true;
  }

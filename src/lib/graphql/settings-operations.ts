...
export const colorSchemeOptions = [
  { value: 'blue', label: 'Ocean Blue', color: '#3B82F6', description: 'Classic professional blue' },
  { value: 'green', label: 'Forest Green', color: '#10B981', description: 'Calm and natural green' },
  { value: 'purple', label: 'Royal Purple', color: '#8B5CF6', description: 'Creative and modern purple' },
  { value: 'orange', label: 'Sunset Orange', color: '#F59E0B', description: 'Energetic and warm orange' }
];
...
export class SettingsOperations {
    ...
    return new Promise<any>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Profile update timeout'),
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Profile update is taking longer than expected. Please check if changes were saved.'
          }
        );
  ...
  async updateUserPreferences(params: {
    ...
    return new Promise<any>((resolve, reject) => {
      const timeoutId = setTimeout(() => {
        const errorResponse = createErrorResponse(
          new Error('Preferences update timeout'),
          {
            type: 'TIMEOUT_ERROR',
            userMessage: 'Preferences update is taking longer than expected. Please verify changes were saved.'
          }
        );
        reject(errorResponse);
      }, dataRequest.timeoutMs);

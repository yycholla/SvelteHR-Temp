	...
	return {
		...
		async markNotificationAsRead(notificationId: string): Promise<void> {
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to mark notification as read'
					);
				}
		...
		async markAllNotificationsAsRead(): Promise<void> {
			...
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to mark all notifications as read'
					);
				}
		...
		async deleteNotification(notificationId: string): Promise<void> {
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to delete notification'
					);
				}
		...
		async updateNotificationPreferences(
			...
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to update notification preferences'
					);
				}
		...
		async getNotificationAnalytics(timeRange?: { startDate: string; endDate: string }) {
			update((state) => ({ ...state, isLoading: true, error: null }));

			try {
				const result = await client
					.query(GET_NOTIFICATION_ANALYTICS_QUERY, {
						timeRange
					})
					.toPromise();
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load notification analytics'
					);
				}
		...
		async bulkMarkAsRead(notificationIds: string[]): Promise<void> {
			...
			try {
				const markReadPromises = notificationIds.map((id) => this.markNotificationAsRead(id));
		...
		getRecentNotifications(hours: number = 24): Notification[] {
			...
			return currentState.notifications.filter(
				(notification) => new Date(notification.createdAt) >= cutoffTime
			);
		},
...
export const unreadNotifications = derived(
	notificationService,
	($notificationService) => $notificationService.unreadNotifications
);

export const unreadCount = derived(
	notificationService,
	($notificationService) => $notificationService.unreadCount
);

export const notificationPreferences = derived(
	notificationService,
	($notificationService) => $notificationService.preferences
);

export const isLoadingNotifications = derived(
	notificationService,
	($notificationService) => $notificationService.isLoading
);
...
export const notificationsPagination = derived(
	notificationService,
	($notificationService) => $notificationService.pagination
);

export const notificationsFilters = derived(
	notificationService,
	($notificationService) => $notificationService.filters
);

export const notificationsSorting = derived(
	notificationService,
	($notificationService) => $notificationService.sorting
);

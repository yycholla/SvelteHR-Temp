...
// Derived stores
export const myPendingRequests = derived(myLeaveRequests, ($myLeaveRequests) =>
	$myLeaveRequests.filter((req) => req.status === 'PENDING')
);

export const myApprovedRequests = derived(myLeaveRequests, ($myLeaveRequests) =>
	$myLeaveRequests.filter((req) => req.status === 'APPROVED')
);

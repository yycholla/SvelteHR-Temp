...
const createPermissionsService = () => {
	...
	return {
		...
		evaluateCustomCondition(condition: PermissionCondition, context?: PermissionContext): boolean {
		...
		isSuperAdmin(user: User): boolean {
			return (
				user.role_assignments?.some(
					(ra) =>
						(ra.userRoleByRoleId?.name.toLowerCase().includes('admin') ||
							ra.userRoleByRoleId?.name === 'hr_admin') &&
		...
		isHRManager(user: User): boolean {
			return (
				user.role_assignments?.some(
					(ra) => ra.userRoleByRoleId?.name.toLowerCase() === 'hr_manager'
				) || false
			);
		},
		...
		isManager(user: User): boolean {
			return (
				user.role_assignments?.some((ra) =>
					['manager', 'hr_manager', 'hr_admin', 'admin'].includes(
						ra.userRoleByRoleId?.name.toLowerCase() || ''
					)
				) || false
			);
		},
		...
		getHighestRoleLevel(user: User): number {
			return Math.max(
				...(user.role_assignments?.map((ra) => ra.userRoleByRoleId?.level || 0) || [0])
			);
		},
...
export const userPermissions = derived([currentUser], ([user]) =>
	user ? permissionsService.getEffectivePermissions(user) : []
);

export const userRoles = derived([currentUser], ([user]) =>
	user ? permissionsService.getUserRoles(user) : []
);

export const isAdmin = derived([currentUser], ([user]) =>
	user ? permissionsService.isSuperAdmin(user) : false
);

export const isHRManager = derived([currentUser], ([user]) =>
	user ? permissionsService.isHRManager(user) : false
);

export const isManager = derived([currentUser], ([user]) =>
	user ? permissionsService.isManager(user) : false
);

	...
	return {
		...
		async loadDepartmentHierarchy() {
			...
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load department hierarchy'
					);
				}
		...
		async getDepartmentDetails(departmentId: string): Promise<Department> {
			...
			try {
				const result = await client
					.query(GET_DEPARTMENT_DETAILS_QUERY, {
						id: departmentId
					})
					.toPromise();
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load department details'
					);
				}
		...
		async updateDepartment(
			...
			try {
				...
				update((state) => ({
					...
					currentDepartment:
						state.currentDepartment?.id === departmentId
		...
		async archiveDepartment(departmentId: string, reason?: string): Promise<void> {
			...
			try {
				const result = await client
					.mutation(ARCHIVE_DEPARTMENT_MUTATION, {
						id: departmentId,
						reason
					})
					.toPromise();
		...
		async assignDepartmentHead(departmentId: string, managerId: string): Promise<void> {
			...
			try {
				const result = await client
					.mutation(ASSIGN_DEPARTMENT_HEAD_MUTATION, {
						departmentId,
						managerId
					})
					.toPromise();
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to assign department head'
					);
				}
		...
		async transferEmployee(transfer: EmployeeTransferInput): Promise<void> {
			...
			try {
				const result = await client
					.mutation(TRANSFER_EMPLOYEE_MUTATION, {
						transfer
					})
					.toPromise();
				...
				const currentState = get({ subscribe });
				if (
					currentState.currentDepartment?.id === transfer.fromDepartmentId ||
		...
		async getDepartmentBudget(departmentId: string): Promise<DepartmentBudgetInfo> {
			...
			try {
				const result = await client
					.query(GET_DEPARTMENT_BUDGET_QUERY, {
						departmentId
					})
					.toPromise();
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load department budget'
					);
				}
		...
		async updateDepartmentBudget(
			...
			try {
				const result = await client
					.mutation(UPDATE_DEPARTMENT_BUDGET_MUTATION, {
						departmentId,
						...budgetUpdates
					})
					.toPromise();
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to update department budget'
					);
				}
		...
		async getDepartmentAnalytics(
			...
			try {
				const result = await client
					.query(GET_DEPARTMENT_ANALYTICS_QUERY, {
						departmentId,
						...timeRange
					})
					.toPromise();
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load department analytics'
					);
				}
		...
		getChildDepartments(parentId: string): Department[] {
			const currentState = get({ subscribe });
			return currentState.departmentHierarchy.filter(
				(dept) => dept.parentDepartment?.id === parentId
			);
		},
...
export const departments = derived(
	departmentService,
	($departmentService) => $departmentService.departments
);

export const currentDepartment = derived(
	departmentService,
	($departmentService) => $departmentService.currentDepartment
);

export const departmentHierarchy = derived(
	departmentService,
	($departmentService) => $departmentService.departmentHierarchy
);

export const isLoadingDepartments = derived(
	departmentService,
	($departmentService) => $departmentService.isLoading
);

export const departmentError = derived(
	departmentService,
	($departmentService) => $departmentService.error
);
...
export const departmentsPagination = derived(
	departmentService,
	($departmentService) => $departmentService.pagination
);

export const departmentsFilters = derived(
	departmentService,
	($departmentService) => $departmentService.filters
);

export const departmentsSorting = derived(
	departmentService,
	($departmentService) => $departmentService.sorting
);

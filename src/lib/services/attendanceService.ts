	...
	return {
			...
			try {
				...
				update((state) => ({
					...
					todaysSummary: {
						...
						status: clockOutRecord.isEarlyDeparture
		...
		async loadAttendanceRecords(options?: {
			...
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load attendance records'
					);
				}
		...
		async createWorkSchedule(input: WorkScheduleInput): Promise<void> {
			...
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to create work schedule'
					);
				}
		...
		async loadOvertimeReport(filter: {
			...
			try {
				...
				if (result.error) {
					throw new Error(
						result.error.graphQLErrors[0]?.message || 'Failed to load overtime report'
					);
				}
		...
		getAttendanceRate(days: number = 30): number {
			...
			const recentRecords = currentState.attendanceRecords.filter(
		...
		getAverageHoursPerDay(): number {
			...
			const totalHours = currentState.attendanceRecords.reduce(
		...
		getTodaysAttendanceStats() {
			...
			const todaysRecords = currentState.attendanceRecords.filter(
		...
		getWeeklyAttendanceStats() {
			...
			const presentDays = weeklyRecords.filter(
				(record) =>
					record.status === AttendanceStatus.PRESENT || record.status === AttendanceStatus.LATE
			).length;

			// Calculate expected working days (weekdays only)
			const expectedWorkingDays = this.getWorkingDaysInPeriod(
				weekStart,
				now > weekEnd ? weekEnd : now
			);
			const attendanceRate =
				expectedWorkingDays > 0 ? (presentDays / expectedWorkingDays) * 100 : 0;
			...
			return {
				...
				summary: {
					...
					earlyDepartureCount: weeklyRecords.filter(
...
export const attendanceRecords = derived(
	attendanceService,
	($attendanceService) => $attendanceService.attendanceRecords
);

export const currentAttendanceRecord = derived(
	attendanceService,
	($attendanceService) => $attendanceService.currentRecord
);

export const isCurrentlyClockedIn = derived(
	attendanceService,
	($attendanceService) => $attendanceService.isCurrentlyClockedIn
);

export const currentBreak = derived(
	attendanceService,
	($attendanceService) => $attendanceService.currentBreak
);

export const todaysSummary = derived(
	attendanceService,
	($attendanceService) => $attendanceService.todaysSummary
);

export const overtimeAlert = derived(
	attendanceService,
	($attendanceService) => $attendanceService.overtimeAlert
);

export const isLoadingAttendance = derived(
	attendanceService,
	($attendanceService) => $attendanceService.isLoading
);

export const attendanceError = derived(
	attendanceService,
	($attendanceService) => $attendanceService.error
);

export const currentWorkDuration = derived([attendanceService], ([$attendanceService]) =>
	attendanceService.getCurrentWorkDuration()
);

export const currentBreakDuration = derived([attendanceService], ([$attendanceService]) =>
	attendanceService.getBreakDuration()
);

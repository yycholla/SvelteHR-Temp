...
export class PermissionsService {
  ...
  checkAnyPermission(permissions: string[]): PermissionCheck {
    ...
    return {
      ...
      granted,
      reason: granted ? 'At least one permission granted' : 'None of the required permissions granted'
    };
  }
   ...
   */
  checkResourceAccess(resource: string, action: 'read' | 'write' | 'create' | 'update' | 'delete' | 'manage'): PermissionCheck {
    let granted = false;
    let reason = '';
    ...
    switch (action) {
      case 'read':
        ...
        reason = granted ? `Read access to ${resource} granted` : `Read access to ${resource} denied`;
        break;
      case 'write':
      case 'create':
      case 'update':
        granted = this.rbacManager.canWrite(resource);
        reason = granted ? `Write access to ${resource} granted` : `Write access to ${resource} denied`;
        break;
      case 'delete':
        granted = this.rbacManager.canDelete(resource);
        reason = granted ? `Delete access to ${resource} granted` : `Delete access to ${resource} denied`;
        break;
      case 'manage':
        granted = this.rbacManager.canManage(resource);
        reason = granted ? `Management access to ${resource} granted` : `Management access to ${resource} denied`;
        break;
    }

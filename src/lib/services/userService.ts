...
const createUserService = () => {
	...
	return {
		...
		async createUser(input: CreateUserInput): Promise<any> {
			...
			try {
				...
				const result = await client
					.mutation(CREATE_USER, { input: { user: userInput } })
					.toPromise();
		...
		async bulkDeactivateUsers(userIds: string[], reason?: string): Promise<void> {
			...
			try {
				const deactivatePromises = userIds.map((userId) => this.deactivateUser(userId, reason));
		...
		getUsersByRole(roleName: string): User[] {
			const currentState = get({ subscribe });
			return currentState.users.filter((user) => user.roles.some((role) => role.name === roleName));
		},
...
export const activeUsers = derived(users, ($users) => $users.filter((user) => user.isActive));

export const inactiveUsers = derived(users, ($users) => $users.filter((user) => !user.isActive));
...
export const getNewHiresCount = () => {
	...
	return derived(
		users,
		($users) => $users.filter((user) => new Date(user.createdAt) >= thirtyDaysAgo).length
	);
};

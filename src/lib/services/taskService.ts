  ...
  return {
    ...
    async loadDashboardTasks() {
      ...
      try {
        ...
        if (result.error) {
          throw new Error(result.error.graphQLErrors[0]?.message || 'Failed to load dashboard tasks');
        }
        ...
        update(state => ({
          ...state,
          dashboardTasks: {
            upcoming: upcomingTasks.filter((task: Task) => !task.isOverdue && task.status !== TaskStatus.COMPLETED),
      ...
      try {
        ...
        update(state => ({
          ...
          quickStats: {
            ...
            myTasksCount: newTask.assignedTo ? state.quickStats.myTasksCount + 1 : state.quickStats.myTasksCount
          },
          isLoading: false
        }));
    ...
    async updateTask(taskId: string, input: UpdateTaskInput): Promise<Task> {
      ...
      try {
        const result = await client.mutation(UPDATE_TASK_MUTATION, {
          id: taskId,
          input
        }).toPromise();
...
export const highPriorityTasks = derived(tasks, $tasks => 
  $tasks.filter(task => task.priority === TaskPriority.HIGH || task.priority === TaskPriority.URGENT)
);

...
const createOnboardingService = () => {
  ...
  return {
    ...
    async updateInstanceStatus(instanceId: string, status: 'PreHire' | 'Onboarding' | 'Active' | 'Terminated'): Promise<void> {
      update(state => ({ ...state, isLoading: true, error: null }));
...
export const currentOnboardingInstance = derived(onboardingService, $service => $service.currentInstance);

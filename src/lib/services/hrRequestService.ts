...
const createHRRequestService = () => {
  ...
  return {
    ...
    async getHRRequestDetails(requestId: string): Promise<HRRequest> {
      ...
      try {
        const result = await client.query(GET_HR_REQUEST_DETAILS_QUERY, { 
          id: requestId 
        }).toPromise();

        if (result.error) {
          throw new Error(result.error.graphQLErrors[0]?.message || 'Failed to load request details');
        }
    ...
    async updateHRRequest(requestId: string, input: UpdateHRRequestInput): Promise<HRRequest> {
      ...
      try {
        const result = await client.mutation(UPDATE_HR_REQUEST_MUTATION, {
          id: requestId,
          input
        }).toPromise();
    ...
    async processHRRequest(requestId: string, input: ProcessHRRequestInput): Promise<HRRequest> {
      ...
      try {
        const result = await client.mutation(PROCESS_HR_REQUEST_MUTATION, {
          id: requestId,
          input
        }).toPromise();
    ...
    async assignHRRequest(requestId: string, assigneeId: string, comment?: string): Promise<HRRequest> {
      update(state => ({ ...state, isLoading: true, error: null }));

      try {
        const result = await client.mutation(ASSIGN_HR_REQUEST_MUTATION, {
          id: requestId,
          assigneeId,
          comment
        }).toPromise();
        ...
        update(state => ({
          ...
          assignedRequests: assigneeId === 'current_user' 
    ...
    async uploadDocument(requestId: string, file: File, description?: string): Promise<Document> {
      ...
      try {
        ...
        const result = await client.mutation(UPLOAD_HR_DOCUMENT_MUTATION, {
          requestId,
          file: formData
        }).toPromise();
    ...
    async getHRRequestAnalytics(filters?: {
      ...
      try {
        const result = await client.query(GET_HR_REQUEST_ANALYTICS_QUERY, {
          filters
        }).toPromise();

        if (result.error) {
          throw new Error(result.error.graphQLErrors[0]?.message || 'Failed to load HR request analytics');
        }
    ...
    async bulkUpdateStatus(requestIds: string[], action: ProcessHRRequestInput['action']): Promise<void> {
      update(state => ({ ...state, isLoading: true, error: null }));
    ...
    getOverdueRequests(): HRRequest[] {
      ...
      const now = new Date();
      return currentState.requests.filter(request => 
        request.dueDate && new Date(request.dueDate) < now && 
    ...
    getHighPriorityRequests(): HRRequest[] {
      const currentState = get({ subscribe });
      return currentState.requests.filter(request => request.priority === 'HIGH' || request.priority === 'URGENT');
    },
...
export const hrRequests = derived(hrRequestService, $hrRequestService => $hrRequestService.requests);

export const currentHRRequest = derived(hrRequestService, $hrRequestService => $hrRequestService.currentRequest);

export const myHRRequests = derived(hrRequestService, $hrRequestService => $hrRequestService.myRequests);

export const assignedHRRequests = derived(hrRequestService, $hrRequestService => $hrRequestService.assignedRequests);

export const isLoadingHRRequests = derived(hrRequestService, $hrRequestService => $hrRequestService.isLoading);

export const hrRequestError = derived(hrRequestService, $hrRequestService => $hrRequestService.error);
...
export const hrRequestsPagination = derived(hrRequestService, $hrRequestService => $hrRequestService.pagination);

export const hrRequestsFilters = derived(hrRequestService, $hrRequestService => $hrRequestService.filters);

export const hrRequestsSorting = derived(hrRequestService, $hrRequestService => $hrRequestService.sorting);

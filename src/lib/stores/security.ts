		...
		try {
			...
			if (auditResult.data && securityResult.data) {
				...
				const failedLogins = securityEvents.filter(
					(event) => event.eventType === 'login_attempt' && event.isSuspicious
				).length;
	...
	loadSecurityMetrics: async (): Promise<void> => {
		...
		try {
			...
			if (metricsResult.data) {
				...
				const loginAttempts = recentSecurityEvents.filter(
					(event: any) => event.eventType === 'LOGIN' || event.eventType === 'AUTHENTICATION'
				);
				...
				const metrics: SecurityMetrics = {
					...
					dataAccess: {
						sensitiveDataAccess,
						recentAccess: auditLogs
							.filter((log: any) =>
								['compensation', 'personal_information', 'contact_information'].includes(
									log.tableName
								)
							)
							.map((log: any) => ({

...
export const currentWorkflowDefinition = derived(workflowStore, ($workflow) => $workflow.currentDefinition);
export const currentWorkflowInstance = derived(workflowStore, ($workflow) => $workflow.currentInstance);
...
export const workflowStats = derived(workflowStore, ($workflow) => {
  ...
  const runningInstances = $workflow.instances.filter(instance => instance.status === 'running').length;
  const completedInstances = $workflow.instances.filter(instance => instance.status === 'completed').length;
  const failedInstances = $workflow.instances.filter(instance => instance.status === 'failed').length;
  ...
  createDefinition: async (input: CreateWorkflowDefinitionInput): Promise<boolean> => {
    ...
    try {
      ...
      if (result.data?.createWorkflowDefinition?.workflowDefinition) {
        ...
        workflowStore.update(state => ({
          ...state,
          definitions: [result.data.createWorkflowDefinition.workflowDefinition, ...state.definitions],
          isLoading: false
        }));
        return true;
      } else {

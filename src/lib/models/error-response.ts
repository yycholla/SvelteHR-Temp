...
import type { ErrorResponse as ErrorResponseContract, ErrorType, ErrorSeverity } from '$lib/types/graphql-contracts';
...
export class ErrorResponse {
  ...
  constructor(config: {
    ...
    this.userMessage = config.userMessage || this.generateUserMessage(config.type, config.originalError);
    ...
    this.technicalDetails = this.buildTechnicalDetails(config.originalError, config.technicalDetails);
  ...
  toLogEntry(): Record<string, unknown> {
    return {
      ...
      originalErrorType: this.originalError instanceof Error ? this.originalError.constructor.name : typeof this.originalError
    };
  }
...
function detectErrorType(error: unknown): ErrorType {
  ...
  if (errorString.includes('permission') || errorString.includes('forbidden') || errorString.includes('access denied')) {

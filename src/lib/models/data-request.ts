...
export class DataRequest<TVariables = Record<string, unknown>, TData = unknown> {
  ...
  get isTimedOut(): boolean {
    if (this._status !== DataRequestStatus.PENDING && this._status !== DataRequestStatus.IN_PROGRESS) {
      return false;
    }
   ...
   */
  cancel(): void {
    if (this._status === DataRequestStatus.SUCCESS ||
        this._status === DataRequestStatus.ERROR ||
        this._status === DataRequestStatus.TIMEOUT) {
  ...
  private validateConfig(config: {
    ...
    if (config.timeoutMs && (typeof config.timeoutMs !== 'number' || config.timeoutMs <= 0 || config.timeoutMs > 5000)) {
 ...
 */
export function createDataRequest<TVariables = Record<string, unknown>, TData = unknown>(
  config: {
    operationName: string;
    variables: TVariables;
    userCredentials: UserCredentials;

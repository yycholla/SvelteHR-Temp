...
export class ApplicationPage {
  ...
  get isLoading(): boolean {
    return this._loadingState === PageLoadingState.LOADING ||
  ...
  get canRetry(): boolean {
    return this._errorState.canRetry &&
  ...
  completeLoading(operationResults?: Record<string, unknown>): void {
    ...
    if (this._performance.loadStartTime) {
      ...
      this._performance.totalLoadTime = this._performance.loadEndTime - this._performance.loadStartTime;
    }
  ...
  retry(): void {
    if (!this.canRetry) {
      throw new Error(`Cannot retry: attempts=${this._errorState.retryAttempts}, canRetry=${this._errorState.canRetry}`);
    }
   ...
   */
  private buildRetryConfiguration(config?: Partial<PageRetryConfiguration>): PageRetryConfiguration {
  ...
  private updatePerformanceMetrics(): void {
    ...
    const totalRequests = this._performance.retryCount + Object.keys(this._performance.operationTimes).length;
...
export function createApplicationPage(config: {
  ...
  const metadata: PageMetadata = {
    ...
    isProtected: (config.requiredPermissions?.length || 0) > 0 || (config.requiredRoles?.length || 0) > 0,

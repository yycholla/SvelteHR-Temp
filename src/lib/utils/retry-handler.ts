...
export function shouldRetry(error: ErrorResponse, attempt: number, config: RetryConfig): boolean {
 ...
 */
export function createSvelteRetryHandler(config: Partial<RetryConfig> = {}): {
	...
	const getState = (): RetryHandlerSvelteState => {
		...
		const now = Date.now();
		const timeToNextRetry = state.nextRetryAt ? Math.max(0, state.nextRetryAt.getTime() - now) : 0;
	...
	const retry = async <T>(operation: () => Promise<T>, error: ErrorResponse): Promise<T | null> => {
...
export function logRetryAttempt(attempt: number, delay: number, error: ErrorResponse): void {
	if (import.meta.env.DEV) {
		...
		logger.info('Error:', error.userMessage);
...
export function logRetrySuccess(attempt: number): void {
	if (import.meta.env.DEV) {
		logger.info(`✅ Retry Success after ${attempt} attempts`);
	}
}
...
export function logMaxAttemptsReached(error: ErrorResponse): void {
	if (import.meta.env.DEV) {
		...
		logger.error('Final Error:', error.userMessage);

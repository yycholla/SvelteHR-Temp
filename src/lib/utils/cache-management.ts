...
export function validateCachePolicy(policy: CachePolicy): boolean {
  if (policy.ttlMinutes > GRAPHQL_OPERATION_CONSTANTS.MAX_CACHE_TTL_MINUTES) {
    console.warn(`Cache TTL ${policy.ttlMinutes} exceeds maximum allowed ${GRAPHQL_OPERATION_CONSTANTS.MAX_CACHE_TTL_MINUTES} minutes`);
    return false;
  }
...
export class CacheInvalidator {
  ...
  async invalidateByKeys(keys: string[]): Promise<boolean> {
    ...
    try {
      ...
      keys.forEach(key => {
        this.client!.reexecuteOperation(this.client!.createRequestOperation('query', {
...
export class CacheWarmer {
   ...
   */
  async warmCache(operations: Array<{
    operationName: string;
    query: any;
    variables: any;
    priority: number;
    ...
    for (const op of sortedOps) {
      try {
        await this.client.query(op.query, op.variables, {
 ...
 */
export function createRequestPolicy(cachePolicy: CachePolicy): 'cache-first' | 'cache-and-network' | 'network-only' {
...
export function validateCacheConfiguration(config: CacheConfig): string[] {
  ...
  if (config.defaultTTL > GRAPHQL_OPERATION_CONSTANTS.MAX_CACHE_TTL_MINUTES) {
    errors.push(`Default TTL ${config.defaultTTL} exceeds maximum ${GRAPHQL_OPERATION_CONSTANTS.MAX_CACHE_TTL_MINUTES} minutes`);
  }

  if (config.maxTTL > GRAPHQL_OPERATION_CONSTANTS.MAX_CACHE_TTL_MINUTES) {
    errors.push(`Max TTL ${config.maxTTL} exceeds maximum ${GRAPHQL_OPERATION_CONSTANTS.MAX_CACHE_TTL_MINUTES} minutes`);
  }

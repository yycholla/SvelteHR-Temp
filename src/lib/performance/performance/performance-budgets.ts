...
export class PerformanceBudgetValidator {
   ...
   */
  validateMetric(budgetName: string, value: number): {
    passed: boolean;
   ...
   */
  generateRecommendations(violations: Array<ReturnType<PerformanceBudgetValidator['validateMetric']>>): string[] {
      ...
      switch (budget.category) {
        case 'response-time':
          if (budget.name.includes('GraphQL')) {
            recommendations.push('Optimize GraphQL queries by reducing complexity and implementing better caching');
            recommendations.push('Consider database indexing improvements for frequently queried fields');
          } else if (budget.name.includes('Page Load')) {
            recommendations.push('Implement code splitting and lazy loading for better page load performance');
        ...
        case 'resource':
          if (budget.name.includes('Memory')) {
            recommendations.push('Check for memory leaks in components and implement proper cleanup');

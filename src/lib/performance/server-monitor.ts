...
class ServerPerformanceMonitor {
    ...
    return async ({ event, resolve }: { event: RequestEvent; resolve: any }) => {
      ...
      try {
        ...
        if (duration > this.config.slowRequestThreshold) {
          console.warn(`🐌 Slow request: ${event.request.method} ${event.url.pathname} took ${Math.round(duration)}ms`);
        }
  ...
  recordGraphQLOperation(
    ...
    if (duration > 200) { // GraphQL budget
  ...
  recordDatabaseQuery(
    ...
    if (duration > 100) { // Database budget
      console.warn(`🐌 Slow database query took ${Math.round(duration)}ms:`, this.sanitizeQuery(query));
    }
  }
  ...
  getStatistics(timeframe: number = 60000): {
    ...
    const slowRequestsCount = recentMetrics.filter(m =>
    ...
    const memoryUsage = {
      ...
      average: memoryMetrics.length > 0 ?
        memoryMetrics.reduce((sum, m) => sum + m, 0) / memoryMetrics.length : 0
    };
...
export function withDatabasePerformanceTracking<T extends any[], R>(
  ...
  return async (...args: T): Promise<R> => {
    ...
    const query = args.find(arg => typeof arg === 'string' && arg.includes('SELECT')) as string || 'Unknown query';
...
export function createServerPerformanceMonitor(config?: Partial<ServerPerformanceConfig>): ServerPerformanceMonitor {
...
export function analyzePerformanceTrends(
  ...
  const older = metrics.filter(m => now - m.timestamp >= timeWindow / 2 && now - m.timestamp < timeWindow);
  ...
  const trends = {
    responseTime: responseTimeChange < -0.1 ? 'improving' :
                 responseTimeChange > 0.1 ? 'degrading' : 'stable',
    errorRate: recentErrorRate < olderErrorRate * 0.9 ? 'improving' :
              recentErrorRate > olderErrorRate * 1.1 ? 'degrading' : 'stable',
    throughput: throughputChange > 0.1 ? 'improving' :
               throughputChange < -0.1 ? 'degrading' : 'stable'
  };
  ...
  if (trends.errorRate === 'degrading') {
    insights.push(`Error rate has increased from ${(olderErrorRate * 100).toFixed(1)}% to ${(recentErrorRate * 100).toFixed(1)}%`);
  }
  if (trends.throughput === 'degrading') {
    insights.push(`Request throughput has decreased by ${Math.round(Math.abs(throughputChange) * 100)}%`);
  }
  ...
  const trends = {
    responseTime: responseTimeChange < -0.1 ? 'improving' as const :
                 responseTimeChange > 0.1 ? 'degrading' as const : 'stable' as const,
    errorRate: recentErrorRate < olderErrorRate * 0.9 ? 'improving' as const :
              recentErrorRate > olderErrorRate * 1.1 ? 'degrading' as const : 'stable' as const,
    throughput: throughputChange > 0.1 ? 'improving' as const :
               throughputChange < -0.1 ? 'degrading' as const : 'stable' as const
  };

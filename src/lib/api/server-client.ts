...
export async function loadEmployeeData(
	...
	try {
		...
		// Process employees
		let employees =
			employeesResult.status === 'fulfilled' && employeesResult.value.success
				? employeesResult.value.data?.data || []
				: [];

		// Process departments
		const departments =
			departmentsResult.status === 'fulfilled' && departmentsResult.value.success
				? departmentsResult.value.data?.data || []
				: [];

		// Get current user context for role enrichment
		const currentUserRoles =
			authResult.status === 'fulfilled' && authResult.value.success
			...
			employees = employees.map((emp: any) => {
				if (emp.id === currentUserId) {
					...
					return {
						...
						job_title:
							emp.job_title && emp.job_title.trim() !== ''
								? emp.job_title
								: currentUserRoles[0]?.display_name || currentUserRoles[0]?.name,
		...
		const totalCount =
			employeesResult.status === 'fulfilled' && employeesResult.value.success
				? (employeesResult.value.data as any)?.total || 0
				: 0;
...
export async function loadLeaveData(
	...
	try {
		...
		const leaves =
			leavesResult.status === 'fulfilled' && leavesResult.value.success
		...
		const totalCount =
			leavesResult.status === 'fulfilled' && leavesResult.value.success
				? (leavesResult.value.data as any)?.total || 0
				: 0;
...
export async function loadTaskData(
	...
	try {
		...
		console.log('👥 Employee API result:', {
			...
			dataLength:
				employeesResult.status === 'fulfilled' && employeesResult.value.success
					? employeesResult.value.data?.data?.length || 0
					: 0,
			error:
		...
		const totalCount =
			tasksResult.status === 'fulfilled' && tasksResult.value.success
				? (tasksResult.value.data as any)?.total || 0
				: 0;
...
export async function loadDocumentData(
	...
	try {
		...
		const totalCount =
			documentsResult.status === 'fulfilled' && documentsResult.value.success
				? (documentsResult.value.data as any)?.total || 0
				: 0;
...
export async function loadAnnouncementData(
	...
	try {
		...
		const announcements = announcementsResult.success ? announcementsResult.data?.data || [] : [];

		const totalCount = announcementsResult.success ? (announcementsResult.data as any)?.total || 0 : 0;
...
export async function loadHRRequestData(
	...
	try {
		...
		const hrRequests =
			requestsResult.status === 'fulfilled' && requestsResult.value.success
		...
		const totalCount =
			requestsResult.status === 'fulfilled' && requestsResult.value.success
				? (requestsResult.value.data as any)?.total || 0
				: 0;
...
export async function loadActivityLogData(
	...
	try {
		...
		const activityLogs = logsResult.success ? logsResult.data?.data || [] : [];

		const totalCount = logsResult.success ? (logsResult.data as any)?.total || 0 : 0;

...
export const DepartmentService = {
  ...
  async getStats(id: string) {
    return apiClient.query<Department>('departments')
...
export const RoleService = {
  ...
  async addPermission(roleId: string, permissionId: string) {
    return apiClient.post(`/organization/roles/${roleId}/permissions`, { permission_id: permissionId });
  },
...
export const TaskService = {
  ...
  async list(options: {
    ...
    return apiClient.query<Task>('tasks')
      .filter({
        ...(options.status && { status: options.status }),
...
export const LeaveService = {
  ...
  async listRequests(options: { employee_id?: string; status?: string } = {}) {
    return apiClient.query<LeaveRequest>('leave/requests')
      .filter(options)
...
export const DocumentService = {
  ...
  async list(options: { category?: string; entity_type?: string; entity_id?: string } = {}) {
    return apiClient.query<Document>('documents')
      .filter(options)
      .include(['uploaded_by'])
      .sort('created_at', 'DESC')
      .execute();
...
export const NotificationService = {
  ...
  async list(options: { unread_only?: boolean } = {}) {
    return apiClient.query<Notification>('notifications')
...
export const PerformanceService = {
  ...
  async listReviews(options: { employee_id?: string; status?: string } = {}) {
    return apiClient.query<PerformanceReview>('performance/reviews')
      .filter(options)
...
export const HRRequestService = {
  ...
  async list(options: { status?: string; type?: string } = {}) {
    return apiClient.query<HRRequest>('hr-requests')
      .filter(options)
...
export const ComplianceService = {
  ...
  async list(options: { status?: string; employee_id?: string } = {}) {
    return apiClient.query<ComplianceItem>('compliance')
      .filter(options)
...
export const AttendanceService = {
  ...
  async list(options: { employee_id?: string; date?: string } = {}) {
    return apiClient.query('attendance')
      .filter(options)
      .include(['employee'])
      .sort('date', 'DESC')
      .execute();

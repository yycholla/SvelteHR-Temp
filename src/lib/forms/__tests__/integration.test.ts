...
describe('Form System Integration', () => {
	...
	describe('Context Manager Integration', () => {
		it('creates contextual forms with proper templates', async () => {
			...
			const onboardingTemplate = await contextManager.loadContextualForm(
				'employee_onboarding',
				'emp-new-123'
			);
	...
	describe('Complex Validation Scenarios', () => {
		it('handles forms with conditional fields and complex validation rules', async () => {
			...
			const fullDayLeave = {
				...
				durationType: 'full_day',
				reason:
					'Summer vacation with family to celebrate our anniversary and spend quality time together'
			};
	...
	describe('Error Handling Integration', () => {
		...
		it('handles context manager errors properly', async () => {
			...
			await expect(contextManager.loadContextualForm('invalid-context')).rejects.toThrow(
				'No template found for context type: invalid-context'
			);
		});
	});

...
describe('Form Schema Builder', () => {
	describe('createFormSchema', () => {
		it('creates valid Zod schema from form definition', () => {
			...
			// Invalid email
			expect(() => zodSchema.parse({ email: 'invalid', age: 25, name: 'John' })).toThrow();

			// Age too low
			expect(() => zodSchema.parse({ email: 'test@example.com', age: 17, name: 'John' })).toThrow();

			// Name too short
			expect(() => zodSchema.parse({ email: 'test@example.com', age: 25, name: 'J' })).toThrow();

			// Missing required field
			expect(() => zodSchema.parse({ age: 25, name: 'John' })).toThrow();
		});
...
describe('Complex Form Scenarios', () => {
		...
		const formSchema: FormSchema = {
			fields: [
				...
				},
				{
					...
					label: 'Accept Terms',
					required: true,
					options: [{ value: 'accepted', label: 'I accept the terms' }]
				}
			]
		};
	...
	it('validates phone numbers correctly', () => {
		...
		const invalidPhones = ['not-a-phone', 'abc-def-ghij', '++invalid', 'phone number with letters'];
...
describe('Form Template Integration', () => {
	...
	it('handles template with all field types', () => {
		...
		const { text_field, email_field, number_field, date_field, select_field, ...optionalFields } =
			validData;

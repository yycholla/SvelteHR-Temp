...
describe('FormContextManager', () => {
  ...
  describe('Contextual Form Loading', () => {
    ...
    it('throws error for unknown context type', async () => {
      await expect(
        contextManager.loadContextualForm('unknown-type')
      ).rejects.toThrow('No template found for context type: unknown-type');
    });
  ...
  describe('Draft Management', () => {
    ...
    it('throws error when saving without context', async () => {
      ...
      await expect(
        contextManager.saveDraft({ test: 'data' })
  ...
  describe('Auto-save Functionality', () => {
    ...
    it('handles auto-save errors gracefully', async () => {
      const saveDraftSpy = vi.spyOn(contextManager, 'saveDraft')
...
describe('HR Form Contexts', () => {
  ...
  it('maps context types to templates correctly', async () => {
    ...
    const onboarding = await contextManager.loadContextualForm(HR_FORM_CONTEXTS.EMPLOYEE_ONBOARDING);
    const leave = await contextManager.loadContextualForm(HR_FORM_CONTEXTS.LEAVE_REQUEST);
    const performance = await contextManager.loadContextualForm(HR_FORM_CONTEXTS.PERFORMANCE_REVIEW);

	...
	const sForm = superForm(initialData, {
		...
		onUpdate: async ({ form }) => {
			try {
				...
				const errorMessage =
					error instanceof Error ? error.message : 'Failed to create compliance item';
...
export function updateComplianceForm(
	itemId: number,
	options: {
		onSuccess?: (item: any) => void;
		onError?: (error: string) => void;
		...
		{},
		{
			...
			onUpdate: async ({ form }) => {
				try {
					...
					const errorMessage =
						error instanceof Error ? error.message : 'Failed to update compliance item';
					if (options.onError) options.onError(errorMessage);
					return { message: { type: 'error', text: errorMessage } };
				}
			}

...
function createFieldSchema(field: FormField): z.ZodType<any> {
	switch (field.type) {
		...
		case 'phone':
			return z.string().regex(/^[\+]?[\d\s\-\(\)\.]{6,}$/, 'Please enter a valid phone number');
...
function applyValidation(schema: z.ZodType<any>, validation: FieldValidation): z.ZodType<any> {
	...
	if (validation.maxValue !== undefined && schema instanceof z.ZodUnion) {
		...
		const fileSchema = schema.options.find(
			(opt) => opt instanceof z.ZodType && opt._def.typeName === 'ZodBranded'
		);
 ...
 */
function applyFormValidationRules(

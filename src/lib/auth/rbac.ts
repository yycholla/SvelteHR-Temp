...
export class RBACManager {
  ...
  getHighestRoleLevel(): number {
    ...
    const activeRoles = this.userRoles.filter(assignment => 
      assignment.isActive && 
  ...
  getHighestRoleName(): string {
    ...
    const roleAssignment = this.userRoles.find(assignment => 
      assignment.isActive && 
  ...
  getActiveRoles(): UserRoleAssignment[] {
    return this.userRoles.filter(assignment => 
      assignment.isActive && 
      (!assignment.validUntil || new Date(assignment.validUntil) > new Date())
    );
  }
  ...
  hasRole(roleName: string): boolean {
    return this.getActiveRoles().some(assignment => 
      assignment.userRoleByRoleId.name === roleName
    );
  }
...
export const createRBACManager = (userRoles: UserRoleAssignment[], currentUserId: string | null = null): RBACManager => {
  return new RBACManager(userRoles, currentUserId);
};

// Permission check functions for common scenarios
export const canViewUsers = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.VIEW_USERS);
export const canManageUsers = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.UPDATE_USERS);
export const canViewSensitiveData = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.VIEW_SENSITIVE_DATA);
export const canManageRoles = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.ASSIGN_ROLES);
export const canApproveLeave = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.APPROVE_LEAVE_REQUESTS);
export const canManageWorkflows = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.MANAGE_WORKFLOWS);
export const canManageCompliance = (rbac: RBACManager): boolean => rbac.hasPermission(PERMISSIONS.MANAGE_COMPLIANCE);

// Role-based route protection helpers
export const requireMinimumRole = (minimumLevel: number) => (rbac: RBACManager): boolean => {
  return rbac.hasMinimumRoleLevel(minimumLevel);
};

export const requirePermission = (permission: string) => (rbac: RBACManager): boolean => {
  return rbac.hasPermission(permission);
};

export const requireAnyPermission = (permissions: string[]) => (rbac: RBACManager): boolean => {

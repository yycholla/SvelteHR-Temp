CREATE MIGRATION m17qtdtovpge3rxrtwru4v4fzht2b656mgzjogllx64h52g3vvzmma
    ONTO initial
{
  ...
  CREATE SCALAR TYPE default::AuthEventType EXTENDING enum<LOGIN_ATTEMPT, LOGIN_SUCCESS, LOGIN_FAILURE, LOGOUT, TOKEN_REFRESH, SESSION_EXPIRED, PERMISSION_DENIED>;
  ...
  CREATE TYPE rbac::User EXTENDING rbac::Auditable {
      ...
      CREATE PROPERTY identity_id: std::uuid;
  ...
  CREATE TYPE compliance_performance::PerformanceReview EXTENDING portal_communication::FormEnabled {
  ...
  ALTER TYPE portal_communication::FormEnabled {
  ...
  CREATE TYPE default::AuthEvent EXTENDING default::Auditable {
      CREATE REQUIRED PROPERTY ip_address: std::str;
      CREATE INDEX ON (.ip_address);
      CREATE LINK user: rbac::User;
      CREATE INDEX ON (.user);
      CREATE REQUIRED PROPERTY event_type: default::AuthEventType;
      CREATE INDEX ON (.event_type);
      CREATE REQUIRED PROPERTY success: std::bool;
      CREATE INDEX ON ((.success, .event_type));
      CREATE REQUIRED PROPERTY timestamp: std::datetime {
          SET default := (std::datetime_current());
      };
      CREATE INDEX ON ((.timestamp, .event_type));
      CREATE REQUIRED PROPERTY details: std::json;
      CREATE REQUIRED PROPERTY user_agent: std::str;
  };
  ...
  CREATE TYPE default::PKCESession EXTENDING default::Auditable {
      CREATE REQUIRED PROPERTY verifier_hash: std::str;
      CREATE CONSTRAINT std::exclusive ON (.verifier_hash);
      CREATE REQUIRED PROPERTY challenge: std::str;
      CREATE CONSTRAINT std::exclusive ON (.challenge);
      CREATE INDEX ON (.verifier_hash);
      CREATE REQUIRED PROPERTY expires_at: std::datetime;
      CREATE PROPERTY is_used: std::bool {
          SET default := false;
      };
      CREATE INDEX ON ((.expires_at, .is_used));
      CREATE INDEX ON (.challenge);
      CREATE REQUIRED PROPERTY ip_address: std::str;
  ...
  CREATE TYPE hr_workflows::ChangeRequest EXTENDING portal_communication::FormEnabled {
  ...
  CREATE TYPE hr_workflows::HRRequest EXTENDING portal_communication::FormEnabled {
  ...
  CREATE TYPE onboarding_offboarding::ExitInterview EXTENDING portal_communication::FormEnabled {
  ...
  CREATE TYPE onboarding_offboarding::OnboardingProcess EXTENDING portal_communication::FormEnabled {
      CREATE REQUIRED LINK employee: rbac::User;
      CREATE CONSTRAINT std::exclusive ON (.employee);
  ...
  CREATE TYPE onboarding_offboarding::OnboardingTemplate EXTENDING portal_communication::FormEnabled {
  ...
  ALTER TYPE onboarding_offboarding::OnboardingStep {
      CREATE REQUIRED LINK process: onboarding_offboarding::OnboardingProcess;
  ...
  ALTER TYPE performance::ReviewTemplate {
      CREATE MULTI LINK roles: rbac::Role;

/**
 * TypeScript Interface Contracts for Standardized GraphQL Operations
 * SvelteHR GraphQL Integration Error Resolution
 *
 * These interfaces define the expected structure for all GraphQL operations
 * to ensure consistent parameter handling and error management.
 */
...
export interface CachePolicy {
  ttlMinutes: number; // Max 30 minutes
...
// Subscription Contracts (for future WebSocket support)

export interface DashboardDataUpdatesVariables {
  userId: string;
}

export interface DashboardDataUpdatesResponse {
  dashboardUpdates: {
    type: 'metrics' | 'activities' | 'tasks' | 'events';
    data: any;
    timestamp: string;

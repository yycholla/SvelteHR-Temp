...
export async function getCompleteDashboardData(
	...
	try {
		const result = await client
			.query(GET_DASHBOARD_QUERY, variables, {
...
{:else if error}
	<div class="error-container">
		...
		{#if error.isRetryable}
			<button class="btn retry" onclick={handleRetry} disabled={retryAttempts >= 3}>
...
_Implementation time estimate: 30 minutes per page + 15 minutes setup_
